# DNase subfamily enrichment
# See 6/5/17, 6/6/17, 6/7/17, 6/9/17

# Enrichment of DNase peaks (width) in TE subfamilies
subfamily_DNase_sample = read.table("DNase_peaks/subfamily_DNase_sample.txt",sep='\t')
colnames(subfamily_DNase_sample) = c("Subfamily","Sample","Length_ijk")
subfamily_DNase_sample_expand = expand.grid(Subfamily = levels(subfamily_state_sample$Subfamily),Sample = levels(subfamily_DNase_sample$Sample))
subfamily_DNase_sample_expand = join(subfamily_DNase_sample_expand,rbind(rmsk_TE_stats_subfamily,rmsk_other_stats_subfamily)[,1:3],by=c("Subfamily"))
subfamily_DNase_sample = join(subfamily_DNase_sample_expand,subfamily_DNase_sample,by=c("Subfamily","Sample"),type="left")
subfamily_DNase_sample[is.na(subfamily_DNase_sample$Length_ijk),]$Length_ijk = 0
subfamily_DNase_sample = merge(subfamily_DNase_sample,DNase_stats[,c(1,4)],by=c("Sample"))
colnames(subfamily_DNase_sample)[6] = "Length_jk"
subfamily_DNase_sample$Length_k = apply(subfamily_DNase_sample,1,function(x) mnemonics_states_TEother[x[1],]$Total)
subfamily_DNase_sample = merge(subfamily_DNase_sample,unique(subfamily_state_sample[,c(1:3,5,7:12,14)]),by=c("Subfamily","Family","Class","Sample"))
subfamily_DNase_sample = subfamily_DNase_sample[,c(1:4,8:13,5:6,14,7)]
subfamily_DNase_sample$Enrichment = log2((subfamily_DNase_sample$Length_ijk/subfamily_DNase_sample$Length_ik)/(subfamily_DNase_sample$Length_jk/subfamily_DNase_sample$Length_k))

# Total proportion of DNase peaks in subfamily
subfamily_DNase_sample$Length_percent_jk = subfamily_DNase_sample$Length_ijk/subfamily_DNase_sample$Length_jk

# Filtered with thresholds
subfamily_DNase_sample_filter = subfamily_DNase_sample[which(subfamily_DNase_sample$Length_ijk >= 600 & subfamily_DNase_sample$Length_ik > 5000),]

# Number of members overlapping DNase peak (needs matrix)
TE_DNase_peaks_members = merge(melt(aggregate(data=TE_DNase_peaks[,c(4,8:60)],.~subfamily,function(x) sum(as.numeric(x))),id.vars=c("subfamily")),melt(aggregate(data=TE_DNase_peaks[,c(4,8:60)],.~subfamily,function(x) sum(as.numeric(x) > 0)),id.vars=c("subfamily")),by=c("subfamily","variable"))
colnames(TE_DNase_peaks_members)[2:4] = c("Sample","DNase_peaks","TEs_overlapping")
TE_DNase_peaks_members = merge(TE_DNase_peaks_members,rmsk_TEother_stats_subfamily[,3:4],by.x=c("subfamily"),by.y=c("Subfamily"))
TE_DNase_peaks_members[which(TE_DNase_peaks_members$Sample %in% c("E116","E117","E123","E124","E126","E127")),]$Count = rmsk_TEother_stats_subfamily[match(TE_DNase_peaks_members[which(TE_DNase_peaks_members$Sample %in% c("E116","E117","E123","E124","E126","E127")),]$subfamily,rmsk_TEother_stats_subfamily$Subfamily),]$Count_noY

# Proportion of members overlapping DNase peak
TE_DNase_peaks_members$Percent = TE_DNase_peaks_members$TEs_overlapping/TE_DNase_peaks_members$Count

# Added to enrichment matrix
subfamily_DNase_sample = merge(subfamily_DNase_sample,TE_DNase_peaks_members,by.x=c("Subfamily","Sample"),by.y=c("subfamily","Sample"),all.x=TRUE)
subfamily_DNase_sample[which(subfamily_DNase_sample$Subfamily == "Tigger2a_Car"),c(17:18,20)] = 0
subfamily_DNase_sample[which(subfamily_DNase_sample$Subfamily == "Tigger2a_Car"),]$Count = 2

# Number of samples subfamily is enriched
subfamily_DNase_sample_counts = ddply(subfamily_DNase_sample,.(Class,Family,Subfamily),function(x) sum(x$Enrichment > 1.5 & x$Length_ijk >= 600 & x$Length_ik > 5000))

# Enrichment in Roadmap sample groups
enrichment_proportion_test$Proportion = apply(enrichment_proportion_test,1,function(x) as.numeric(x[3])/length(EID_metadata[which(EID_metadata[,x[4]] == x[2]),x[4]]))
