...
RNA_samples = as.data.frame(read.table("RNAseq/RNA_samples_agnostic.txt"))
...
colnames(RNA_TE) = c("chromosome","start","stop","subfamily","class","family","strand",as.vector(read.table("RNAseq/RNA_samples_agnostic.txt")$V1))
...
colnames(RNA_refseq_exon) = c("chromosome","start","stop","strand",as.vector(read.table("RNAseq/RNA_samples_agnostic.txt")$V1))
