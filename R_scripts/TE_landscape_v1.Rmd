...
e = ggplot(contribution_TE,aes(x=State,y=Proportion,fill=Cohort)) + geom_bar(stat="identity") + coord_flip() + ylab("Proportion of state in TEs") + theme(text=element_text(size=15),axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.ticks.x=element_blank()) + geom_hline(yintercept = 0.44918676) + scale_fill_manual(values=c("floralwhite","firebrick3"),guide=FALSE) + scale_x_discrete(limits=contribution_TE$State[1:28])

grid.arrange(a,b,c,d,e, nrow = 2, layout_matrix = rbind(c(1,1,1,1,2,2,3,7,7,7), c(4,4,4,5,5,5,6,7,7,7), rep(8,10)),heights=c(0.45,0.45,0.1))
```
