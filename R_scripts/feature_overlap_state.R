...
feature_state_mean = apply(rmsk_TE_measure[,measure_features],2,function(y) 
...
# Older code
#test = melt(as.matrix(apply(rmsk_TE_measure[,measure_features],2,function(x) apply(rmsk_TE_measure[,35:63],2,function(y) {table = aggregate(data=rmsk_TE_measure,y~x,mean);(table$y[2]-table$y[1])/mean(y)}))))
#compare_TE_state = ddply(rmsk_TE_measure,~class_update,function(z) apply(z[,measure_features],2,function(x) apply(z[,35:63],2,function(y) {table = aggregate(data=rmsk_TE_measure,y~x,mean);(table$y[2]-table$y[1])/mean(y)})))
