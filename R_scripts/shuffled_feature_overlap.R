...
feature_proportion_shuffled = ldply(rmsk_TE_shuffled,function(y) apply(y[,c(10:21,25:31)],2,function(x) length(na.omit(x))))
...
# Class 
feature_proportion_shuffled_class = ldply(rmsk_TE_shuffled,function(z) ddply(z,~class_update,function(y) apply(y[,c(10:21,25:31)],2,function(x) length(na.omit(x))/length(x))))
...
# Subfamily
feature_proportion_shuffled_subfamily = ldply(rmsk_TE_shuffled,function(z) ddply(z,~subfamily,function(y) apply(y[,c(10:21,25:31)],2,function(x) length(na.omit(x))/length(x))))
