...
grid.arrange(a,b,c,d,e, nrow=4,layout_matrix=rbind(c(1,3),c(2,3),c(4,4),c(5,5)),heights=c(0.2,0.2,0.2,0.4))
```
...
# 15_Quies (particular subfamilies? families?)
...
apply(chromosome_potential,2,function(x) chromosome_potential[which(x > 2*sd(x)+mean(x)),c("chromosome",x)])
```
...
b = ggplot(feature_state_mean,aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1),panel.background = element_rect(fill="lightgrey")) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-1,50)) + scale_y_discrete(limits=rev(levels(feature_state_mean$Feature)),labels=genic_labels) + scale_x_discrete(labels=all_state_labels) + facet_wrap(~Coding,labeller=labeller(Coding=coding_labels))
...
### TE feature overlap control (split by coding?)
...
ggplot(feature_proportion_shuffled_class_long,aes(x=Feature,y=Proportion)) + geom_boxplot() + geom_point(data=rmsk_TE_class_feature_long,aes(x=Feature,y=Proportion),size=2,color="red") + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_x_discrete(limits=levels(feature_proportion_shuffled_long$Feature)[c(17,19,18,5,7,6,8,2,4,3,10,12,11,14,16,15,13,1)]) + facet_wrap(~class_update)
...
test = lapply(metadata$Sample[c(1:11,32:35)],function(x) read.table(file=paste("/scratch/ecp/mappability/mappability_",x,"_average.txt",sep=""),sep='\t'))
names(test) = metadata$Sample[c(1:11,32:35)]
test = ldply(test)

ggplot(test,aes(x=V1,y=V2)) + geom_boxplot() + scale_x_discrete(limits=chromHMM_states) + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + ylab("Average mappability")
...
ggplot(rmsk_TE_shuffled_map,aes(x=mappability,fill=Iteration)) + geom_histogram(binwidth=0.05,position="dodge") + xlab("Mappability") + ylab("TEs") + scale_fill_manual(values=c(rep("grey",10),"red"))
```
...
# This relies on above analysis, lots of unix code
...
### Individual TEs in candidate subfamilies in state by sample (update!)
...
**Supplementary Figure 17. ChromHMM states of human-mouse orthologs.**

```{r Fig S17, echo=FALSE}
