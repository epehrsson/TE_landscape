...
# Comparison of feature overlap with other input variables
...
colnames(test) = c("Metric","Feature","Difference")
test$class_update = rep("All",100)
...
compare_TE_feature = melt(compare_TE_feature,id.vars=c("class_update","Metric"))
colnames(compare_TE_feature)[3:4] = c("Feature","Difference")
compare_TE_feature = rbind(test,compare_TE_feature)
compare_TE_feature$class_update = factor(compare_TE_feature$class_update,levels=c("DNA","LINE","LTR","SINE","SVA","Other","All"))
compare_TE_feature$Feature = factor(compare_TE_feature$Feature,levels=levels(compare_TE_feature$Feature)[c(17:19,5:9,2:4,10:12,14:16,13,1,20)])
rm(test)
...
aggregate(data=rmsk_TE_measure[,c(9,35:63)],.~class_update,mean)
sort(apply(aggregate(data=rmsk_TE_measure[,c(9,35:63)],.~class_update,mean)[,2:30],2,function(x) sd(x)/mean(x)))

# Correlation between input variables and epigenetic states
...
compare_TE_state$Metric = rep(colnames(rmsk_TE_measure[,35:63]),6)
compare_TE_state = melt(compare_TE_state,id.vars=c("class_update","Metric"))
colnames(compare_TE_state)[3:4] = c("Feature","Difference")
compare_TE_state = rbind(test,compare_TE_state)
compare_TE_state$class_update = factor(compare_TE_state$class_update,levels=c("DNA","LINE","LTR","SINE","SVA","Other","All"))
rm(test)
...
ggplot(compare_TE_state,aes(x=State,y=Feature,fill=Difference)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))
...
# Input variables by class
apply(rmsk_TE_subfamily_measure[,c(4:5,7,9,32:35)],2,function(x) unlist(kruskal.test(x~rmsk_TE_subfamily_measure$class_update))["p.value"])
aggregate(data=rmsk_TE_subfamily_measure[,c(3:5,7,9,32:35)],.~class_update,mean)

# Correlation between input variables
correlate_subfam_feature = rbind(correlate_spearman(rmsk_TE_subfamily_measure,"Count",c(5,7,9,32:35)),
                                 correlate_spearman(rmsk_TE_subfamily_measure,"Median_length",c(7,9,32:35)),
                                 correlate_spearman(rmsk_TE_subfamily_measure,"Mappability",c(9,32:35)),
                                 correlate_spearman(rmsk_TE_subfamily_measure,"Age",32:35),
                                 correlate_spearman(rmsk_TE_subfamily_measure,"CpGs",33:35),
                                 correlate_spearman(rmsk_TE_subfamily_measure,"Total_length",34:35),
                                 correlate_spearman(rmsk_TE_subfamily_measure,"CpGs_per_TE",34:35))
correlate_subfam_feature$Metric = c(rep("Count",49),rep("Median_length",42),rep("Mappability",35),rep("Age",28),rep("CpGs",21),rep("Total_length",14),rep("CpGs_per_TE",14))
...
correlate_subfam_feature = correlate_subfam_feature[which(correlate_subfam_feature$State != correlate_subfam_feature$Metric),]

# Comparison of feature overlap with other input variables
test = melt(as.matrix(apply(rmsk_TE_subfamily_measure[,12:31],2,function(x) apply(rmsk_TE_subfamily_measure[,c(4:5,7,9,32:35)],2,function(y) {table = aggregate(data=rmsk_TE_subfamily_measure,y~x,mean);(table$y[2]-table$y[1])/mean(y)}))))
colnames(test) = c("Metric","Feature","Difference")
test$class_update = rep("All",160)

compare_subfam_feature = ddply(rmsk_TE_subfamily_measure,~class_update,function(z) apply(z[,12:31],2,function(x) apply(z[,c(4:5,7,9,32:35)],2,function(y) {table = aggregate(data=z,y~x,mean);(table$y[2]-table$y[1])/mean(y)})))
compare_subfam_feature$Metric = rep(colnames(rmsk_TE_subfamily_measure)[c(4:5,7,9,32:35)],6)
compare_subfam_feature = melt(compare_subfam_feature,id.vars=c("class_update","Metric"))
colnames(compare_subfam_feature)[3:4] = c("Feature","Difference")
compare_subfam_feature = rbind(test,compare_subfam_feature)
rm(test)

subfam_pvalue_all_feature = apply(rmsk_TE_subfamily_measure[,12:31],2,function(x) apply(rmsk_TE_subfamily_measure[,c(4:5,7,9,32:35)],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
subfam_pvalue_class_feature = ddply(rmsk_TE_subfamily_measure,~class_update,function(z) apply(z[,12:31],2,function(x) apply(z[,c(4:5,7,9,32:35)],2,function(y) unlist(wilcox.test(y~x))["p.value"])))

# Number of samples by class
apply(rmsk_TE_subfamily_measure[,c(36:112)],2,function(x) unlist(kruskal.test(x~rmsk_TE_subfamily_measure$class_update))["p.value"])
aggregate(data=rmsk_TE_subfamily_measure[,c(3,36:112)],.~class_update,mean)

# Correlate samples in state, max, mean, range with subfamily features
correlate_subfamily = rbind(correlate_spearman(rmsk_TE_subfamily_measure,"Count",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"Median_length",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"Mappability",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"Age",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"CpGs",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"Total_length",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"CpGs_per_TE",c(36:112)),
             correlate_spearman(rmsk_TE_subfamily_measure,"CpGs_per_kbp",c(36:112)))
correlate_subfamily$Metric = c(rep("Count",539),rep("Median_length",539),rep("Mappability",539),rep("Age",539),rep("CpGs",539),rep("Total_length",539),rep("CpGs_per_TE",539),rep("CpGs_per_kbp",539))
...
# Comparison of TEs in state by genic feature
test = melt(as.matrix(apply(rmsk_TE_subfamily_measure[,12:31],2,function(x) apply(rmsk_TE_subfamily_measure[,36:112],2,function(y) {table = aggregate(data=rmsk_TE_subfamily_measure,y~x,mean);(table$y[2]-table$y[1])/mean(y)}))))
colnames(test) = c("Metric","Feature","Difference")
test$class_update = rep("All",1540)

compare_subfamily = ddply(rmsk_TE_subfamily_measure,~class_update,function(z) apply(z[,12:31],2,function(x) apply(z[,36:112],2,function(y) {table = aggregate(data=rmsk_TE_subfamily_measure,y~x,mean);(table$y[2]-table$y[1])/mean(y)})))
compare_subfamily$Metric = rep(colnames(rmsk_TE_subfamily_measure)[36:112],6)
compare_subfamily = melt(compare_subfamily,id.vars=c("class_update","Metric"))
colnames(compare_subfamily)[3:4] = c("Feature","Difference")
compare_subfamily = rbind(test,compare_subfamily)
rm(test) 

subfam_pvalue_all_state = apply(rmsk_TE_subfamily_measure[,12:31],2,function(x) apply(rmsk_TE_subfamily_measure[,36:112],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
subfam_pvalue_class_state = ddply(rmsk_TE_subfamily_measure,~class_update,function(z) apply(z[,12:31],2,function(x) apply(z[,36:112],2,function(y) unlist(wilcox.test(y~x))["p.value"])))

# Compare states to each other
subfam_state_correlation = ldply(colnames(rmsk_TE_subfamily_measure)[36:112],function(x) correlate_spearman(rmsk_TE_subfamily_measure,x,c(36:112)))
subfam_state_correlation$State2 = rep(colnames(rmsk_TE_subfamily_measure)[36:112],each=539)
subfam_state_correlation = subfam_state_correlation[which(subfam_state_correlation$State != subfam_state_correlation$State2),]
subfam_state_correlation$p.value = as.numeric(as.character(subfam_state_correlation$p.value))
subfam_state_correlation$estimate.rho = as.numeric(as.character(subfam_state_correlation$estimate.rho))
...
subfamily_CpG_meth_filter = subfamily_CpG_meth[which(subfamily_CpG_meth$CpGs >= 25 & subfamily_CpG_meth$Hypomethylated >= 6),]

# Clustering 
enrichment_lor = list(enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "1_TssA"),],"Enrichment",1.5), 
                       enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "2_TssAFlnk"),],"Enrichment",1.5), 
                       enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "3_TxFlnk"),],"Enrichment",1.5), 
                       enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "4_Tx"),],"Enrichment",1.5),
                       enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "5_TxWk"),],"Enrichment",1.5),
                       enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "6_EnhG"),],"Enrichment",1.5),
                       enrichment_proportion(subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == "7_Enh"),],"Enrichment",1.5),
                       enrichment_proportion(subfamily_DNase_sample_filter,"Enrichment",1.5),
                       enrichment_proportion(subfamily_H3K27ac_sample_filter,"Enrichment",1.5),
                       enrichment_proportion(subfamily_CpG_meth_filter,"Enrichment",1.5))
lapply(enrichment_lor,function(x) dim(x[which(x$Proportion > 0.5 & x$Enriched > 1),]))

enrichment_pc = list(enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "1_TssA"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "2_TssAFlnk"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "3_TxFlnk"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "4_Tx"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "5_TxWk"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "6_EnhG"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_state_sample[which(subfamily_state_sample$Length_ijk >= 600 & subfamily_state_sample$State == "7_Enh"),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_DNase_sample[which(subfamily_DNase_sample$Length_ijk >= 600),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_H3K27ac_sample[which(subfamily_H3K27ac_sample$Length_ijk >= 600),],"Length_percent_jk",0.01),
                      enrichment_proportion(subfamily_CpG_meth[which(subfamily_CpG_meth$CpGs >= 25),],"Hypo_percent",0.01))
lapply(enrichment_pc,function(x) dim(x[which(x$Proportion > 0.5 & x$Enriched > 1),]))
```
