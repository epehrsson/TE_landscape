...
sample_counts = rbind(apply(metadata[,c(1,10:13)],2,function(x) dim(metadata[which(!is.na(x)),])[1]),
                      apply(metadata[,c(1,10:13)],2,function(x) dim(metadata[which(!is.na(x) & metadata$Exclude == "Include"),])[1]),
                      apply(metadata[,c(1,10:13)],2,function(x) dim(metadata[which(!is.na(x) & metadata$chrY == "Yes"),])[1]),
                      apply(metadata[,c(1,10:13)],2,function(x) dim(metadata[which(!is.na(x) & metadata$chrY == "Yes" & metadata$Exclude == "Include"),])[1]))
...
# rmsk_TE_measure vectors
chromHMM_states_X = unlist(lapply(chromHMM_states,function(x) paste("X",x,sep="")))
chromHMM_states_X[8] = "X8_ZNF.Rpts"
measure_states = c(chromHMM_states_X,meth_states,"DNase","H3K27ac","Expressed_samples")
measure_states_extra = c("States","Max_states_intra","Max_expression")
measure_metrics = c("Length","mappability","JC_distance","CpGs","CpGs_per_length")
measure_features = c("cpgIsland","promoters","promoters_pc","promoters_nc","5UTR","5UTR_pc","5UTR_nc","coding_exon","coding_exon_pc","3UTR","3UTR_pc","3UTR_nc",
...
feature_overlap = split_coding(feature_overlap,cohort_column = 1)
...
test = rmsk_TE_measure[,c(1,35:49,52:58)]
...
c = ggplot(chromosome_potential_long,aes(x=State,y=Mean_samples,color=chromosome,label=chromosome)) + geom_point() + geom_text(angle=45,hjust=0,vjust=0,size=3) + scale_color_discrete(guide=FALSE) + scale_x_discrete(limits=rev(names(all_state_labels)),labels=all_state_labels) + theme(axis.title.y = element_blank(),panel.grid.major.y=element_line(color="grey")) + ylab("Mean % of samples in state") + coord_flip()
...
rmsk_TE_measure_SINE = na.omit(rmsk_TE_measure[which(rmsk_TE_measure$class_update == "SINE"),c(1:10,12:14,52:55,61)])
...
rmsk_TE_measure_Alu = na.omit(rmsk_TE_measure[which(rmsk_TE_measure$family == "Alu"),c(1:10,12:14,52:55,61)])
...
apply(rmsk_TE_measure[,c(measure_states,measure_states_extra)],2,function(x) unlist(kruskal.test(x~rmsk_TE_measure$class_update))["p.value"])
...
apply(rmsk_TE_measure[,measure_states],2,function(x) table(rmsk_TE_measure[which(x > 0),]$chromosome)/dim(rmsk_TE_measure[which(x > 0),])[1])
...
TE_pvalue_all_state = apply(rmsk_TE_measure[,measure_features],2,function(x) apply(rmsk_TE_measure[,c(measure_states,measure_states_extra)],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
TE_pvalue_class_state = ddply(rmsk_TE_measure,~class_update,function(z) apply(z[,measure_features],2,function(x) apply(z[,c(measure_states,measure_states_extra)],2,function(y) unlist(wilcox.test(y~x))["p.value"])))
```
...
rmsk_TE_class_feature = melt(rmsk_TE_class[,c("class_update",measure_features[1:19])],id.vars="class_update")
...
feature_proportion = apply(rmsk_TE[,measure_features[1:19]],2,function(x) length(na.omit(x)))
...
table(apply(rmsk_TE[,measure_features[2:19]],1,function(x) sum(!is.na(x))))
...
rmsk_TE_class[,c("class_update",measure_features[1:20])]

# Proportion of TEs ever in state overlapping each feature
apply(rmsk_TE_measure[,measure_states],2,function(x) apply(rmsk_TE_measure[,measure_features],2,function(y) dim(rmsk_TE_measure[which(x > 0 & y > 0),])[1]/dim(rmsk_TE_measure[which(x > 0),])[1]))
```
...
# Subfamily
rmsk_TE_subfamily_feature_long = melt(rmsk_TE_subfamily[,c("subfamily",measure_features[1:19])],id.var="subfamily")
...
sort(apply(rmsk_TE_measure[,measure_features],2,function(y) median(rmsk_TE_measure[which(y > 0),]$Max_expression,na.rm=TRUE)))

# Max expression by feature
sort(apply(rmsk_TE_measure[,measure_features],2,function(y) max(rmsk_TE_measure[which(y > 0),]$Max_expression,na.rm=TRUE)))
```
...
TE_pvalue_all_feature = apply(rmsk_TE_measure[,measure_features],2,function(x) apply(rmsk_TE_measure[,measure_metrics],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
TE_pvalue_class_feature = ddply(rmsk_TE_measure,~class_update,function(z) apply(z[,measure_features],2,function(x) apply(z[,measure_metrics],2,function(y) unlist(wilcox.test(y~x))["p.value"])))
```
...
test = rmsk_TE_measure[,c(4,35:49,52:57)]
...
#source("R_scripts/TE_correlation.R")
rmsk_TE_subfamily_ever = aggregate(data=rmsk_TE_measure[,c(4,35:49,52:58)],.~subfamily,function(x) sum(x > 0,na.rm=TRUE),na.action=na.pass)
...
apply(rmsk_TE_subfamily[,c("Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp")],2,median)
aggregate(data=rmsk_TE_subfamily[,c("class_update","Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp")],.~class_update,median)

colMeans(rmsk_TE_subfamily[,c("Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp")])
aggregate(data=rmsk_TE_subfamily[,c("class_update","Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp")],.~class_update,mean)
sort(apply(aggregate(data=rmsk_TE_subfamily[,c("class_update","Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp")],.~class_update,mean)[,2:10],2,function(x) sd(x)/mean(x)))

apply(rmsk_TE_subfamily[,c("Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp")],2,function(x) unlist(kruskal.test(x~rmsk_TE_subfamily$class_update))["p.value"])

# Subfamily metric outliers
lapply(c("Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp"),function(x) rmsk_TE_subfamily[which(rmsk_TE_subfamily[,x] > mean(rmsk_TE_subfamily[,x])+2*sd(rmsk_TE_subfamily[,x])),c(1:4,x)])
lapply(c("Count","Median_length","Mappability","Age","CpGs","Count_CpGs","Total_length","CpGs_per_TE","CpGs_per_kbp"),function(x) rmsk_TE_subfamily[which(rmsk_TE_subfamily[,x] < mean(rmsk_TE_subfamily[,x])-2*sd(rmsk_TE_subfamily[,x])),c(1:4,x)])
