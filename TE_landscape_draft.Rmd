...
Loads a data frame of metadata for each Roadmap epigenome, including: epigenome name; Group, Anatomy, and Type assignments from the REMC; Germ layer, Age, and Cancer assignments from this study; whether the epigenome has data for each techique, includes chrY, and is excluded with cancer cell lines and IMR90.
...
Creates a matrix of sample counts for each technique (all samples or excluding cancer cell lines/IMR90), for all samples and only those with chrY. Also creates labels for each technique.

```{r load sample stats}
# Samples with data for each technique: all, not excluded, no Y, both
...
Overlap of individual TEs with H3K27ac peaks. Intersected individual TEs with H3K27ac peaks by sample, then filtered to only intersections where the TE overlaps the center of the H3K27ac peak. Counted the number of intersections per TE and combined into one file. 
...
For DHS and H3K27ac, the dataframes provide the number of peaks each TE overlaps per sample. They also include the total number of samples in which the TE overlaps a peak center, across all samples and excluding cancer cell lines and IMR90. Each dataframe includes only TEs overlapping a peak in at least one sample.
...
Overlap of individual promoters with DHS peaks. Intersected individual promoters with DHS peaks by sample, then filtered to only intersections where the promoter overlaps the center of the DHS peak. Counted the number of intersections per promoter and combined into one file. 
...
Overlap of individual promoters with H3K27ac peaks. Intersected individual promoters with H3K27ac peaks by sample, then filtered to only intersections where the promoter overlaps the center of the H3K27ac peak. Counted the number of intersections per promoter and combined into one file. 
...
Creates dataframes of individual promoter assignment to epigenetic states. 
...
Overlap of shuffled TEs with DHS peaks. Intersected individual shuffled TEs with DHS peaks by sample, then filtered to only intersections where the TE overlaps the center of the DHS peak. Counted the number of intersections per TE and combined into one file per iteration. 
...
Overlap of shuffled TEs with H3K27ac peaks. Intersected individual shuffled TEs with H3K27ac peaks by sample, then filtered to only intersections where the TE overlaps the center of the H3K27ac peak. Counted the number of intersections per TE and combined into one file per iteration. 
...
For DHS and H3K27ac, the dataframes provide the number of peaks each TE overlaps per sample and the total number of samples in which each TE overlaps a peak center. Each dataframe includes only TEs overlapping a peak in at least one sample.
...
The dataframes of TEs and TE epigenetic state assignments created above are loaded (as well as the exon expression dataframe). 
...
Loads constants and feature sizes, including a matrix of RefSeq feature lengths. 
...
Prints a matrix of samples with data for each epigenetic technique, including those with chrY and excluding IMR90/cancer cell lines. 
...
The number of and total width of DHS peaks per sample, plus the number of peaks overlapping TEs (regardless of whether the center overlaps the TE). 
...
The total width of DHS peaks overlapping TEs and the number of DHS peaks whose center overlaps a TE, by sample. Intersected DHS peaks with merged TE regions and summed the length of overlap. Using the intersection of DHS peaks with individual TEs that was filtered to only those that overlap the center of a peak, counted the number of unique DHS peaks whose center overlaps a TE. 
...
The number of and total width of H3K27ac peaks per sample, plus the number of peaks overlapping TEs (regardless of whether the center overlaps the TE). 
...
The total width of H3K27ac peaks overlapping TEs and the number of H3K27ac peaks whose center overlaps a TE, by sample. Intersected H3K27ac peaks with merged TE regions and summed the length of overlap. Using the intersection of H3K27ac peaks with individual TEs that was filtered to only those that overlap the center of a peak, counted the number of unique H3K27ac peaks whose center overlaps a TE. 
...
# Number of samples each TE is in each epigenetic state
...
# Proportion of TEs ever in state categories
...
# Number and proportion of TEs ever in each state
...
For TEs in each state in 100% of samples, identifies the closest RefSeq gene. Reports all ties. Also counts TEs overlapping a DHS peak in all samples that are >5kb from the nearest RefSeq gene. 
...
awk -v FS='\t' '{if(($8 == "DNase") && ($21 > 5000)) print $0}' always_TEs_true_genes.txt | wc -l
```
...
```{bash, eval=FALSE}
# Intersect 200bp windows with chromHMM
...
/sample_lists/chromHMM_50state.txt
...
Using Roadmap 2015 Supplementary Fig. 4, for all 7 samples, I determined which of 18-state model states is most enriched for each of the 50 states.
...
# Plot proportion of TEs in each of the 50 states, by sample
# Colored by the corresponding 18-state model state
ggplot(state_sample_50state,aes(x=State50,y=Proportion,fill=State18)) + geom_bar(stat="identity",color="black") + scale_fill_manual(values=chromHMM_states_18,guide=FALSE) + coord_flip() + facet_wrap(~Sample) + labs(x="State with 50-state model",y="Proportion of TEs in state") + scale_x_discrete(limits=rev(levels(state_sample_50state$State50)))
```

The number of TEs in an active regulatory state in each sample (based on the corresponding 18-state model state, states 1-4 and 7-11). 

```{bash 50-state active reg, eval=FALSE}
awk '{if(($8 == "E1") || ($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E14") ||($8 == "E15") ||($8 == "E16") ||($8 == "E17") ||($8 == "E18") ||($8 == "E19") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28") ||($8 == "E29") ||($8 == "E30") ||($8 == "E31")) print $0}' rmsk_TEother_50state_E003_sorted.txt | cut -f1-7 | sort | uniq | wc -l 

awk '{if(($8 == "E1") ||($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E9") ||($8 == "E10") ||($8 == "E11") ||($8 == "E12") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28") ||($8 == "E29") ||($8 == "E30") ||($8 == "E31") ||($8 == "E32") ||($8 == "E33") ||($8 == "E34") ||($8 == "E35") ||($8 == "E36") ||($8 == "E37")) print $0}' rmsk_TEother_50state_E004_sorted.txt | cut -f1-7 | sort | uniq | wc -l

awk '{if(($8 == "E1") ||($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E9") ||($8 == "E10") ||($8 == "E11") ||($8 == "E18") ||($8 == "E19") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28") ||($8 == "E29") ||($8 == "E30") ||($8 == "E31") ||($8 == "E32") ||($8 == "E33") ||($8 == "E34") ||($8 == "E35") ||($8 == "E36") ||($8 == "E37") ||($8 == "E38") ||($8 == "E39")) print $0}' rmsk_TEother_50state_E005_sorted.txt | cut -f1-7 | sort | uniq | wc -l

awk '{if(($8 == "E1") ||($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E18") ||($8 == "E19") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28") ||($8 == "E29") ||($8 == "E30") ||($8 == "E31") ||($8 == "E32") ||($8 == "E33") ||($8 == "E34") ||($8 == "E35")) print $0}' rmsk_TEother_50state_E006_sorted.txt | cut -f1-7 | sort | uniq | wc -l

awk '{if(($8 == "E1") ||($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E9") ||($8 == "E10") ||($8 == "E14") ||($8 == "E15") ||($8 == "E16") ||($8 == "E17") ||($8 == "E18") ||($8 == "E19") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28")) print $0}' rmsk_TEother_50state_E007_sorted.txt | cut -f1-7 | sort | uniq | wc -l

awk '{if(($8 == "E1") ||($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E9") ||($8 == "E10") ||($8 == "E14") ||($8 == "E15") ||($8 == "E16") ||($8 == "E17") ||($8 == "E18") ||($8 == "E19") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28") ||($8 == "E29") ||($8 == "E30") ||($8 == "E31") ||($8 == "E32") ||($8 == "E33") ||($8 == "E34") ||($8 == "E35")) print $0}'  rmsk_TEother_50state_E008_sorted.txt | cut -f1-7 | sort | uniq | wc -l

awk '{if(($8 == "E1") ||($8 == "E2") ||($8 == "E3") ||($8 == "E4") ||($8 == "E5") ||($8 == "E6") ||($8 == "E7") ||($8 == "E8") ||($8 == "E17") ||($8 == "E18") ||($8 == "E19") ||($8 == "E20") ||($8 == "E21") ||($8 == "E22") ||($8 == "E23") ||($8 == "E24") ||($8 == "E25") ||($8 == "E26") ||($8 == "E27") ||($8 == "E28") ||($8 == "E29") ||($8 == "E30") ||($8 == "E31") ||($8 == "E32") ||($8 == "E33") ||($8 == "E34") ||($8 == "E35") ||($8 == "E36") ||($8 == "E37")) print $0}' rmsk_TEother_50state_E017_sorted.txt | cut -f1-7 | sort | uniq | wc -l
```
...
# Number of samples each promoter is in each state
source("R_scripts/potential_promoter.R")

# Number of samples each exon is transcribed RPKM > 1
...
Identified similar pairs of samples ("pseudo-replicates") using Table S1 from the Roadmap publication. 

Created files listing all TEs annotated with a chromHMM state in a sample by state and subfamily, for each active regulatory state. 
...
For each pseudo-replicate sample pair, I found the fraction of TEs that are in both samples over the fraction in either sample (or both). 
...
Above analysis, updated to include only TEs/windows that are not in an active regulatory state outside the pair. 

Generates a matrix of TEs that are in an active state in each sample pair, but are not in an active state outside the pair. 

```{r replicates restricted, cache=TRUE, cache.lazy=FALSE}
## TEs in an active regulatory state in each sample
chromHMM_active_unique = unique(chromHMM_active_matrix[,c(TE_coordinates,"Sample")])

## Total number of samples the TE is in an active regulatory state
...
## Number of samples the TE in in an active regulatory state, for each pair
...
## Restrict to TEs exclusive to that sample pair
...
## Windows in an active regulatory state in each sample
windows_active_unique = unique(windows_active[,c("chr","start","stop","Sample")])

## Total number of samples the window is in an active regulatory state
...
## Number of samples the window in in an active regulatory state, for each pair
...
## Restrict to windows exclusive to that sample pair
...
Filters the original matrices, which require the TE to be in the '''same''' active state in each pair member, to TEs that are not in any active regulatory state outside the sample pair.

```{r replicates restricted count}
# Merge with counts from original analysis
...
Epigenetic correlation between samples. 
...
```{r Figure R1, echo=FALSE,fig.height=4}
a = ggplot(replicates_count,aes(x=Feature,y=Fraction,fill=Set)) + geom_boxplot() + theme(axis.title.x=element_blank()) + scale_x_discrete(labels=c("200bp windows","TEs")) + scale_fill_manual(labels=setNames(c("Replicates","Different tissue"),c("Replicate","Tissue")),values=c("maroon4","lightseagreen")) + ylab("Fraction of TEs in both samples / either sample")

b = ggplot(replicates_restricted_count,aes(x=Feature,y=Fraction,fill=Set)) + geom_boxplot() + theme(axis.title.x=element_blank()) + scale_x_discrete(labels=c("200bp windows","TEs")) + scale_fill_manual(labels=setNames(c("Replicates","Different tissue"),c("Replicate","Tissue")),values=c("maroon4","lightseagreen")) + ylab("Fraction of TEs in both samples / either sample")

c = ggplot(cor_marks_replicates[which(cor_marks_replicates$Mark %in% c("H3K4me1_7","H3K4me3_1")),],aes(x=Sample1,y=Correlation,color=Set)) + geom_jitter(width = 0.2) + facet_wrap(~Mark) + scale_color_manual(values=c("red","blue","grey"),name="Pair type") + xlab("Sample 1")

grid.arrange(a,b,c,layout_matrix=rbind(c(1,2),c(3)))
```

### Supplementary Figure 7 

Creates sets for analysis of TEs with restricted active regulatory activity versus constitutively active, non-TE bases. 

+ TEs that are in each of the active regulatory states in 1-5 samples total
+ Shuffled TEs in each active regulatory state in >50 samples
+ TEs in each active regulatory state in >50 samples
+ TEs not in the active regulatory states in any sample
...
## Combining
shuffled_pos_combine = lapply(states[c(1:3,6:7)],function(x) lapply(seq(1,10,1),function(y) shuffled_pos[[y]][[x]]))
shuffled_pos_combine = lapply(shuffled_pos_combine,function(y) y <- ldply(y,.id="Iteration"))
names(shuffled_pos_combine) = states[c(1:3,6:7)]
...
Combined the files of constitutively active shuffled TEs together by state. 

To determine how much redundancy is present within the combined shuffled TE set, I found the total number of shuffled TEs and the number of unique TEs (collapsing those with the same coordinates) across iterations, per state. There are almost no redundancies (one in the 2_TssAFlnk state), because it is very unlikely for shuffled TE coordinates to be assigned the same location in different iterations. 

However, I also determined the degree of overlap within each TE set by comparing the total TE length to the unique TE length. Redundancy is high in the shuffled sets, particularly for the 1_TssA state, but is much smaller for the non-shuffled sets. 

```{bash combined shuffle, eval=FALSE}
# Combine shuffled iterations
...
## Overlapping bp (all sets)
for file in orthogonal/*.txt; do echo $file; cut -f1-3 $file | sort -k1,1 -k2,2n | bedtools merge -i - | awk '{sum+=$3-$2}END{print sum}' -; done
for file in orthogonal/*.txt; do echo $file; awk '{sum+=$3-$2}END{print sum}' $file; done
```

Characteristics of each of the TE sets. 

```{r orthogonal characteristics}
orthogonal_profile = rbind(profile_set(shuffled_pos_combine),profile_set(restricted),profile_set(TE_pos))
...
orthogonal_profile$Set = factor(c(rep("shuffle_constitutive",5),rep("restricted",5),rep("constitutive",5),"not_active"),levels=c("restricted","shuffle_constitutive","constitutive","not_active"))

ggplot(orthogonal_profile,aes(x=State,y=Length,shape=Set)) + geom_point() + ylim(0,350) + ylab("Median length (bp)") + scale_shape_discrete(labels=orthogonal_labels)

ggplot(melt(orthogonal_profile[,c("Set","State",cohorts[c(1:2,5,8,10,13,16,19)])],id.vars=c("Set","State")),aes(x=State,y=value,shape=Set)) + geom_point() + ylim(0,1) + scale_shape_discrete(labels=orthogonal_labels) + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + ylab("Proportion overlapping feature") + facet_wrap(~variable) 

ggplot(melt(orthogonal_profile[,c("Set","State","DNA","LINE","LTR","SINE","SVA","Other")],id.vars=c("Set","State")),aes(x=State,y=value,shape=Set)) + geom_point() + ylim(0,1) +  scale_shape_discrete(labels=orthogonal_labels) + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + ylab("Proportion in class") + facet_wrap(~variable) 
```

'''VISTA'''

Intersected the human VISTA enhancers with each set of TEs. The true TE 1_TssA and 3_TxFlnk constitutive sets overlap no enhancer.  
...
The total number of VISTA enhancers overlapping TEs in each set and the fraction that positively validated. 
...
lapply(states[c(2,6:7)],function(x) fisher.test(vista_val_fraction[which(vista_val_fraction$State == x & vista_val_fraction$Set %in% c("constitutive","restricted")),c("Pos","Neg")]))
...
VISTA with pooled states (response). Identifies the proportion of unique VISTA enhancers that are positively validated that overlap a TE within each set, regardless of state.

```{r vista pooled, eval=FALSE}
source("R_scripts/vista_pooled.R")
...
# Plot
ggplot(vista_val_fraction,aes(x=1,y=Fraction,shape=Set)) + geom_point() + ylim(0,1) + ylab("Proportion of VISTA enhancers\npositively validated") + theme(axis.text.x = element_blank(),axis.ticks.x = element_blank(),axis.title.x = element_blank()) + scale_shape_discrete(labels=orthogonal_labels)
```

'''HOMER'''

Ran HOMER on each of the TE sets to identify transcription factor binding motifs (known motifs only). HOMER provides more detail about the motifs [http://homer.ucsd.edu/homer/motif/motifDatabase.html| here].
...
# Stats
a = ddply(homer_known[which(homer_known$State %in% states & homer_known$`Motif Name` %in% grep("Promoter",unique(homer_known$`Motif Name`),value=TRUE)),],.(`Motif Name`,State),function(x) {y <- x[which(x$Set %in% c("restricted","shuffle_constitutive")),c("Target Seqs with Motif","Number")]; y$`Target without` = y$Number - y$`Target Seqs with Motif`; return(unlist(chisq.test(y[,c("Target Seqs with Motif","Target without")]))["p.value"])})
a$Comparison = rep("restricted_shuffle",dim(a)[1])

b = ddply(homer_known[which(homer_known$State %in% states & homer_known$`Motif Name` %in% grep("Promoter",unique(homer_known$`Motif Name`),value=TRUE)),],.(`Motif Name`,State),function(x) {y <- x[which(x$Set %in% c("constitutive","shuffle_constitutive")),c("Target Seqs with Motif","Number")]; y$`Target without` = y$Number - y$`Target Seqs with Motif`; return(unlist(chisq.test(y[,c("Target Seqs with Motif","Target without")]))["p.value"])})
b$Comparison = rep("constitutive_shuffle",dim(b)[1])

c = ddply(homer_known[which(homer_known$State %in% states & homer_known$`Motif Name` %in% grep("Promoter",unique(homer_known$`Motif Name`),value=TRUE)),],.(`Motif Name`,State),function(x) {y <- x[which(x$Set %in% c("restricted","constitutive")),c("Target Seqs with Motif","Number")]; y$`Target without` = y$Number - y$`Target Seqs with Motif`; return(unlist(chisq.test(y[,c("Target Seqs with Motif","Target without")]))["p.value"])})
c$Comparison = rep("restricted_constitutive",dim(c)[1])

d = ddply(homer_known[which(homer_known$State %in% states & homer_known$`Motif Name` %in% grep("Promoter",unique(homer_known$`Motif Name`),value=TRUE)),],.(`Motif Name`,State),function(x) {y <- x[which(x$Set %in% c("restricted","not_active")),c("Target Seqs with Motif","Number")]; y$`Target without` = y$Number - y$`Target Seqs with Motif`; return(unlist(chisq.test(y[,c("Target Seqs with Motif","Target without")]))["p.value"])})
d$Comparison = rep("restricted_inactive",dim(d)[1])

homer_stats = rbind(a,b,c,d)

# Promoter motifs
table(homer_stats[which(homer_stats$p.value < 0.05),c("Comparison","State")])
```

'''Population'''

Downloaded the hg19 conserved elements generated by phastCons, based on the primate alignment only, from UCSC. Removed the bin field (column 1) to convert it into a bed file. 
...
Downloaded the hg19 GERP elements and combined the chromosomes into one file, reformatting the chromosome column. There are 1,044,996 elements. 
...
Intersected the phastCons and GERP elements with each TE set, then counted the length of overlap with each TE. 

```{bash phastCons intersection, eval=FALSE}
# Intersect with phastCons
...
ddply(cons,.(Set,State,Database),summarise,Overlapping=sum(Overlap > 10)/length(Overlap))
...
lapply(states[c(1:3,6:7)],function(x) chisq.test(cons_10bp[which(cons_10bp$State == x & cons_10bp$Set %in% c("restricted","constitutive") & cons_10bp$Database == "PhastCons"),c("Overlapping","Not")]))
lapply(states[c(1:3,6:7)],function(x) chisq.test(cons_10bp[which(cons_10bp$State == x & cons_10bp$Set %in% c("restricted","inactive") & cons_10bp$Database == "PhastCons"),c("Overlapping","Not")]))
...
lapply(states[c(1:3,6:7)],function(x) chisq.test(cons_10bp[which(cons_10bp$State == x & cons_10bp$Set %in% c("restricted","constitutive") & cons_10bp$Database == "GERP"),c("Overlapping","Not")]))
lapply(states[c(1:3,6:7)],function(x) chisq.test(cons_10bp[which(cons_10bp$State == x & cons_10bp$Set %in% c("restricted","inactive") & cons_10bp$Database == "GERP"),c("Overlapping","Not")]))
```
...
ln -s /bar/epehrsson/TE_landscape/RNAseq/rmsk_TE_rpkm.txt .
...
# Python script outputs two files (hardcoded): combine_marks_counts.txt and combine_marks_counts_unique.txt
...
Overlap of TE subfamilies with DHS peaks. From the intersection of individual TEs with DHS peaks, filtered to intersections where the TE overlaps the center of the DHS peak, counts the number of unique peaks overlapping each subfamily. 
...
Overlap of TE subfamilies with H3K27ac peaks. From the intersection of individual TEs with H3K27ac peaks, filtered to intersections where the TE overlaps the center of the H3K27ac peak, counts the number of unique peaks overlapping each subfamily. 
...
For DHS and H3K27ac, intersected the merged shuffled subfamily files with all peaks, then filtered to intersections where the peak center overlaps the subfamily. Counted the number of peaks overlapping the subfamily per sample. 
...
# Enrichment of TE subfamilies in epigenetic states
source("R_scripts/subfamily_enrichment.R") #Requires proportion.R

# Number of samples each subfamily is in each state
source("R_scripts/subfamily_potential.R")

# PCA on subfamily enrichment
source("R_scripts/subfamily_PCA.R")

# Enrichment of shuffled TE subfamilies in epigenetic states
...
Prints bedfiles for TEs in the state when the subfamily is enriched and those never in the state. ''enrichment_candidates.txt'' is a list of subfamilies of interst, generated from above analysis. 
...
Overlap of candidates with RefSeq features.
...
Samples are ordered by group. Lines split ESC/iPSC/ES-derived; Blood & T-cell/HSC & B-cell/Thymus; Brain/Neurosph; Digestive; Epithelial; Adipose/Mesench/Myosat/Muscle/Sm. Muscle/Heart/Other; IMR90/ENCODE2012
...
#### 50-state model enrichment

Enrichment of TE subfamilies in epigenetic states using the separate 50-state models trained on 7 samples. Uses the same filters as for the 15-state model.

```{bash 50-state enrichment, eval=FALSE}
# Intersected bedfiles with merged subfamily files
...
# Number of TEs in each state in each sample
...
```{r 50-state subfam enrichment}
source("R_scripts/subfamily_enrichment_50state.R")

# Plot number of enrichments
ggplot(subfamily_50state_sample_filter,aes(x=State,fill=State18)) + geom_bar(color="black") + facet_wrap(~Sample) + scale_fill_manual(values=chromHMM_states_18,guide=FALSE) + ylab("Enrichments LOR > 1.5") + theme(axis.text.x=element_blank(),axis.ticks.x = element_blank())
```
...
A) Pattern is stronger for protein-coding than non-coding (not shown).

B) For each state and TE class, the proportion of TEs that are genic (including promoters) at each frequency of annotation. The dashed lines give the proportion of all members of the class that are genic. I split the analysis between all genes and just protein-coding genes, because TEs have a huge contribution to lncRNA.  
...
To answer the question of whether CpG density or class has a stronger impact on repression, I created generalized linear models with a quasi-Poisson distribution for the response, which matches the distributions. They include only class or class and CpG density as variables. I used raw counts for samples in state and performed anova using the chi-squared test for significance. For a log link function, the base mean is multiplied by exp(coefficient). 
...
Counting the number of hg19 TEs that liftover to mm10.
