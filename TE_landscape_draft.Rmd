...
knitr::opts_chunk$set(fig.width=7, fig.height=9, fig.path='TE_landscape_draft_figures/', cache.path="TE_landscape_draft_cache/")
#, error=TRUE
```
...
measure_metrics_subfam = c("Count","Total_length","Median_length","Mappability","Age","CpGs","Count_CpGs","CpGs_per_TE","CpGs_per_kbp")
...
legend = get_legend(ggplot(combined_proportion[which(combined_proportion$Coding == "All"),],aes(x=Feature,y=Proportion,fill=State)) + geom_bar(stat="identity") + scale_fill_manual(values=all_state_colors,labels=all_state_labels) + theme(axis.text.x = element_text(angle=90,hjust=1),axis.title.x=element_blank(),legend.title = element_blank(),legend.position="bottom",legend.direction = "horizontal") + facet_wrap(~Mark,nrow=4))

grid.arrange(a,b,c,d,legend, nrow = 4,layout_matrix = rbind(c(1,1),c(2,2),c(3,4),c(5,5)),heights=c(0.25,0.25,0.4,0.1),widths=c(0.55,0.45))
```
...
kable(combined_proportion[which(combined_proportion$Coding == "All"),],caption="Proportion, all features")
...
kable(ddply(combined_proportion,.(State),function(x) x[which(x$Proportion < sum(x[which(x$Feature == "TE"),]$Proportion)),]),caption="Features with <% in state than TEs")

# By-class proportion
kable(dcast(combine_class_proportion,State~class,value.var="Proportion"),caption="Proportion by class")

# Proportion of all genomic bases in TEs
CHROMHMM_TE_WIDTH/CHROMHMM_TOTAL_WIDTH

# Contribution
kable(contribution,caption="Contribution")

# Contribution, composite states
kable(contribution_composite,caption="Contribution, composite")
```
...
```{r Figure S1, echo=FALSE, cache=TRUE, dependson=c("setup","load libraries","set plot parameters","define vectors")}
...
a = ggplot(all_lengths_blocks,aes(x=log10(Blocks),y=log10(Median),color=State)) + geom_point() + xlab("Number of blocks/peaks per sample (log10)") + ylab("Median length of blocks/peaks per sample (log10(bp))") + scale_color_manual(values=all_state_colors[c(1:15,20:21)],guide=FALSE) + facet_wrap(~State,scales="free") 

# Bases/CpGs in state by sample
b = ggplot(all_state_proportion[which(all_state_proportion$Cohort %in% c("Genome","TE")),],aes(x=Sample,y=Proportion,fill=State)) + geom_bar(stat="identity") + ylab("Proportion of bases/CpGs in state") + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.text.x = element_text(angle=90,hjust=1),axis.ticks.x=element_blank(),axis.title.x=element_blank()) + facet_grid(Cohort~Mark,scales="free",labeller=labeller(Mark = mark_labels))

## Alternative - Average proportion by sample
ggplot(ddply(all_state_proportion[which(all_state_proportion$Coding == "All"),],.(Mark,State,Feature),summarise,Proportion=mean(Proportion)),aes(x=Feature,y=Proportion,fill=State)) + geom_bar(stat="identity") + ylab("Average proportion of feature in state") + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.text.x = element_text(angle=90,hjust=1),axis.title.x=element_blank()) + scale_x_discrete(labels=c("TE","Genome",genic_labels)) + facet_wrap(~Mark,nrow=1,labeller=labeller(Mark = mark_labels))
...
ddply(all_lengths_blocks,.(State),summarise,Range_number=max(Blocks)-min(Blocks),CV_number=sd(Blocks)/mean(Blocks),Range_length=max(Median)-min(Median),CV_length=sd(Median)/mean(Median))

# Comparing number of blocks to length of states
ddply(all_lengths_blocks,.(State),summarise,Cor=unlist(cor.test(Blocks,Median,method="spearman"))["estimate.rho"],Pvalue=unlist(cor.test(Blocks,Median,method="spearman"))["p.value"])
```

### By-sample analysis

```{r by-sample bases}
kable(ddply(all_state_proportion[which(all_state_proportion$Cohort %in% c("Genome","TE")),],.(State,Cohort),summarise,Median=median(Bases),Mean=mean(Bases),SD=sd(Bases),Min=min(Bases),Max=max(Bases),CV=sd(Bases)/mean(Bases)),caption="Cohort in state by sample")

kable(dcast(all_state_proportion[which(all_state_proportion$Mark == "WGBS" & all_state_proportion$Cohort == "Genome"),c("Sample","State","Proportion")],Sample~State,value.var="Proportion"),caption="CpGs in meth state by sample")
```
...
```{r analysis Fig S2}
# Feature %
kable(feature_overlap_long,caption="Feature overlap")

# Proportion of each feature in each state, across all samples
kable(combined_proportion,caption="Proportion, coding/non-coding")
```
...
c = ggplot(combine_stats,aes(x=State,y=Samples_avg_all,fill=State)) + geom_bar(stat="identity",show.legend=FALSE) + geom_errorbar(aes(ymin=Samples_avg_all-Samples_SE_all,ymax=Samples_avg_all+Samples_SE_all),width=0.2) + scale_fill_manual(values=all_state_colors) + coord_flip() + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y=element_blank()) + ylab("Average proportion of samples in state") + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels)) + ylim(0,1)
...
e = ggplot(ddply(combine_potential[which(combine_potential$Sample.Proportion >= 0.5),],.(State),summarise,Count=sum(Count)),aes(x=State,y=log10(Count+1),fill=State)) + geom_bar(stat="identity") + geom_text(aes(label=Count),position=position_stack(vjust=1.1)) + coord_flip() + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels)) + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + ylab("TEs in state in 50% of samples (log10)") 

f = ggplot(combine_potential_class[which(combine_potential_class$Sample.Proportion == 1),],aes(x=State,y=TEs.Proportion,fill=Class)) + geom_bar(stat="identity") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + labs(y="Proportion of TEs in state in 100% of samples in each class") + coord_flip() + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels))

legend = get_legend(ggplot(combine_class_ever,aes(x=State,y=Proportion,fill=Class)) + geom_bar(stat="identity") + scale_fill_manual(values=class_colors) + theme(legend.direction = "horizontal",legend.title = element_blank(),legend.position="bottom"))

grid.arrange(a,b,c,d,e,f,legend, nrow = 3, layout_matrix=rbind(c(1,2,3),c(4,5,6),c(7,7,7)),heights=c(0.45,0.45,0.1),widths=c(0.5,0.25,0.25))
```
...
kable(dcast(combine_class_ever,Class~State,value.var="Proportion"))
kable(combine_stats)
kable(combine_potential_class[which(combine_potential_class$Sample.Proportion == 1),])
```
...
```{r analysis Fig S6c}
# WGBS
z = apply(TE_meth_average[,46:49],2,function(x) dim(TE_meth_average[which(x == sample_counts["All","WGBS"]),])[1])
z
z/NUM_TE_WGBS
apply(TE_meth_average[,46:49],2,function(x) table(TE_meth_average[which(x == sample_counts["All","WGBS"]),]$class_update))

write.table(TE_meth_average[which(TE_meth_average$Hypomethylated == sample_counts["All","WGBS"]),c(1:7,45:49,54:55)],file="WGBS/always_Hypomethylated.txt",sep='\t',row.names=FALSE,quote=FALSE)
write.table(TE_meth_average[which(TE_meth_average$Intermediate == sample_counts["All","WGBS"]),c(1:7,45:49,54:55)],file="WGBS/always_Intermediate.txt",sep='\t',row.names=FALSE,quote=FALSE)

# DNase
x = dim(TE_DNase_peaks[which(TE_DNase_peaks$Samples == sample_counts["All","DNase"]),])[1] + 
  dim(TE_DNase_peaks[which(TE_DNase_peaks$chromosome == "chrY" & TE_DNase_peaks$Samples == sample_counts["chrY","DNase"]),])[1] 
x
x/NUM_TE

kable(TE_DNase_peaks[which(TE_DNase_peaks$Samples == sample_counts["All","DNase"] | (TE_DNase_peaks$chromosome == "chrY" & TE_DNase_peaks$Samples == sample_counts["chrY","DNase"])),c(1:7,61:63)])

# H3K27ac
x = dim(TE_H3K27ac_peaks[which(TE_H3K27ac_peaks$Samples == sample_counts["All","H3K27ac"]),])[1] + 
  dim(TE_H3K27ac_peaks[which(TE_H3K27ac_peaks$chromosome == "chrY" & TE_H3K27ac_peaks$Samples == sample_counts["chrY","H3K27ac"]),])[1]
x
x/NUM_TE

TE_H3K27ac_peaks[which(TE_H3K27ac_peaks$Samples == sample_counts["All","H3K27ac"] | (TE_H3K27ac_peaks$chromosome == "chrY" & TE_H3K27ac_peaks$Samples == sample_counts["chrY","H3K27ac"])),c(1:7,106:108)]

# chromHMM states
z = apply(chromHMM_TE_state[,8:22],2,function(x) dim(chromHMM_TE_state[which(x == sample_counts["All","chromHMM"]),])[1] + dim(chromHMM_TE_state[which(chromHMM_TE_state$chromosome == "chrY" & x == sample_counts["chrY","chromHMM"]),])[1])
z
sum(z/NUM_TE)

## 1_TssA
chromHMM_TE_state[which(chromHMM_TE_state$`1_TssA` == sample_counts["All","chromHMM"] | (chromHMM_TE_state$chromosome == "chrY" & chromHMM_TE_state$`1_TssA` == sample_counts["chrY","chromHMM"])),c(1:7,26)]

## 5_TxWk
always_5TxWk = chromHMM_TE_state[which(chromHMM_TE_state$`5_TxWk` == sample_counts["All","chromHMM"] | (chromHMM_TE_state$chromosome == "chrY" & chromHMM_TE_state$`5_TxWk` == sample_counts["chrY","chromHMM"])),c(1:7,26)]
kable(always_5TxWk)

# Genomic locations
always_5TxWk = merge(always_5TxWk,rmsk_TE[,c(TE_coordinates,cohorts)],by=TE_coordinates)
sort(apply(always_5TxWk[,cohorts],2,function(x) sum(!is.na(x))))
kable(always_5TxWk[which(!is.na(always_5TxWk$intergenic)),])

## 15_Quies (particular subfamilies or families)
test = merge(rmsk_TE_subfamily[,c(1:4,11)],aggregate(data=chromHMM_TE_state[which(chromHMM_TE_state$`15_Quies` == chromHMM_TE_state$Tissues),c(1:7,26)],start~subfamily,length),by="subfamily",all.x=TRUE)
test[is.na(test)] = 0
...
kable(tail(test[order(test$quiescent),],n=40))
rm(test)

test = merge(aggregate(data=rmsk_TE_subfamily[,c(2:3,4)],Count~family+class_update,sum),aggregate(data=chromHMM_TE_state[which(chromHMM_TE_state$`15_Quies` == chromHMM_TE_state$Tissues),c(1:7,26)],start~family,length),by="family",all.x=TRUE)
test[is.na(test)] = 0
test$quiescent = log2((test$start/sum(test$start))/(test$Count/NUM_TE))
dim(test[which(test$start > 0),][1]) # Number of families with a member always quiescent
kable(tail(test[order(test$quiescent),],n=44))
rm(test)
```
...
c = ggplot(combine_potential_class[which(combine_potential_class$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width",draw_quantiles = c(0.25, 0.5, 0.75)) + coord_flip() + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank()) + ylab("Proportion of samples in state") + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels)) + facet_wrap(~Class,nrow=1)

legend = get_legend(ggplot(combine_stats_class,aes(x=State,y=Samples_avg_ever,fill=Class)) + geom_bar(stat="identity",position="dodge") + scale_fill_manual(values=class_colors) + theme(legend.position="bottom",legend.direction="horizontal"))

grid.arrange(a,b,c,legend,nrow=3,layout_matrix=rbind(c(1,2),c(3,3),c(4,4)),heights=c(0.45,0.45,0.1))
```

### Class potential analysis

```{r class potential analysis}
kable(combine_stats_class)
kable(state_order)
```

### New Figure 3

```{r new Figure 3, fig.width=7, fig.height=5, echo=FALSE}
source("R_scripts/state_switching.R")

a = ggplot(combine_potential[which(combine_potential$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width",draw_quantiles = c(0.25, 0.5, 0.75)) + coord_flip() + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank()) + ylab("Proportion of samples in state") + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels))

# chromHMM states
b = ggplot(chromHMM_TE_state,aes(x=States)) + geom_histogram(binwidth=1,fill="blue",color="grey",alpha=0.5) + geom_histogram(data = chromHMM_TE_state,aes(x=Max_states_intra),binwidth=1,fill="darkred",color="grey",alpha=0.5) + ylab("Number of TEs") + xlab("Number of chromHMM states")

# chromHMM states by class 
c = ggplot(chromHMM_TE_state,aes(x=States)) + geom_histogram(binwidth=1,fill="blue",color="grey",alpha=0.5) + scale_x_continuous(limits=c(0,15)) + geom_histogram(data = chromHMM_TE_state,aes(x=Max_states_intra),binwidth=1,fill="darkred",color="grey",alpha=0.5) + ylab("Number of TEs") + xlab("Number of chromHMM states") + facet_wrap(~class_update,scales="free")

# State switching inter, chromHMM
d = ggplot(melt(as.matrix(state_switching_inter)),aes(x=Var2,y=Var1,fill=value/sample_counts["All","chromHMM"])) + geom_tile() + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1),legend.title = element_blank()) + scale_x_discrete(labels=chromHMM_states) + scale_y_discrete(limits=rev(chromHMM_states)) + scale_fill_gradient(low="white",high="darkred",limits=c(0,1))

# State switching inter, WGBS
e = ggplot(melt(as.matrix(ss_inter_meth)),aes(x=Var2,y=Var1,fill=value/sample_counts["All","WGBS"])) + geom_tile() + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1),legend.title = element_blank()) + scale_y_discrete(limits=rev(meth_states)) + scale_fill_gradient2(low="white",high="darkred",limits=c(0,1))

grid.arrange(a,b,c,d,e, nrow = 3, layout_matrix=rbind(c(1,1),c(2,3),c(4,5)),heights=c(0.4,0.2,0.4))
```
...
grid.arrange(a,b,c,d,e, nrow=4, layout_matrix=rbind(c(1,2),c(3,3),c(4,4),c(5,5)),heights=c(0.2,0.2,0.3,0.3),widths=c(0.3,0.7))
```

### State switching analysis

```{r state switching analysis}
kable(state_switching_intra)
kable(state_switching_inter)
```
...
False: grey, True: gold; chromHMM/WGBS: own state colors
...
a = ggplot(by_chromHMM,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  geom_text(aes(label = round(Proportion,2)), position = position_stack(vjust=0.5)) + facet_grid(Mark~chromHMM) + 
...
b = ggplot(by_WGBS,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  geom_text(aes(label = round(Proportion,2)), position = position_stack(vjust=0.5)) + facet_grid(Mark~WGBS) + 
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + ylab("Proportion of TE x sample") + 
  scale_fill_manual(values=c(chromHMM_colors,meth_colors,setNames(c("gold","darkgrey"),c("True","False"))),guide=FALSE)

c = ggplot(by_DNase,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  geom_text(aes(label = round(Proportion,2)), position = position_stack(vjust=0.5)) + facet_grid(Mark~DNase) + 
...
d = ggplot(by_H3K27ac,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  geom_text(aes(label = round(Proportion,2)), position = position_stack(vjust=0.5)) + facet_grid(Mark~H3K27ac) + 
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),axis.title.y = element_blank()) + 
  scale_fill_manual(values=c(chromHMM_colors,meth_colors,setNames(c("gold","darkgrey"),c("True","False"))),guide=FALSE)

e = ggplot(by_RNA,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  geom_text(aes(label = round(Proportion,2)), position = position_stack(vjust=0.5)) + facet_grid(Mark~RNA) + 
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),axis.title.y = element_blank()) + 
  scale_fill_manual(values=c(chromHMM_colors,meth_colors,setNames(c("gold","darkgrey"),c("True","False"))),guide=FALSE)

grid.arrange(a,b,c,d,e, nrow=2, layout_matrix=rbind(c(1,1,1,1),c(2,3,4,5)),widths=c(0.4,0.2,0.2,0.2))

mplot(mosaic(xtabs(TE_sample ~ chromHMM+WGBS+DNase+H3K27ac+RNA, data=compare_marks_all),shade=TRUE,return_grob=TRUE,legend=FALSE),pin=c(7,9)) # No longer normalized
```
...
b = ggplot(combine_potential_prom[which(combine_potential_prom$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width",draw_quantiles = c(0.25, 0.5, 0.75)) + coord_flip() + scale_fill_manual(values=all_state_colors[1:21],guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + ylab("Proportion of samples in state") + scale_x_discrete(limits = rev(states[1:21]),labels=all_state_labels[1:21])
...
d = ggplot(RNA_potential_exon_long[which(RNA_potential_exon_long$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width",draw_quantiles = c(0.25, 0.5, 0.75)) + coord_flip() + scale_fill_manual(values="black",guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + ylab("Proportion of samples RPKM > 1") + scale_x_discrete(labels=all_state_labels[22])
...
f = ggplot(combine_potential_noCancer[which(combine_potential_noCancer$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width",draw_quantiles = c(0.25, 0.5, 0.75)) + coord_flip() + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + ylab("Proportion of samples in state") + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels))

grid.arrange(a,b,c,d,e,f,nrow = 3,layout_matrix=rbind(c(1,2),c(3,4),c(5,6)),heights=c(0.4,0.2,0.4),widths=c(0.6,0.4))
```
...
combine_stats_prom
mean(combine_boxplot_prom$Proportion)
...
kable(tail(RNA_TE[order(RNA_TE$Max_expression),],n=20))
...
kable(tail(RNA_refseq_exon[order(RNA_refseq_exon$Max_expression),],n=20))
...
```{r analysis Fig S5a}
kable(combine_stats_noCancer_IMR90)
...
```{r Figure S7, echo=FALSE, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 2","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
source("R_scripts/potential_shuffled.R")

# Shuffled TEs
combine_boxplot$Iteration = rep("True",dim(combine_boxplot)[1])
combine_shuffled_boxplot = rbind(state_sample_count_shuffled[,c(1:2,4:5)],shuffled_WGBS_average_count,shuffled_DNase_potential_count[,2:5],shuffled_H3K27ac_potential_count[,2:5],combine_boxplot[which(combine_boxplot$State != "Expressed_samples"),])
...
b = ggplot(combine_shuffled_stats,aes(x=State,y=Value)) + geom_boxplot() + theme(axis.title.y = element_blank(),axis.title.x = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_x_discrete(limits=states[1:21]) + geom_point(data=combine_stats_long[which(combine_stats_long$State != "Expressed_samples"),],aes(x=State,y=Value),color="red",size=1) + facet_wrap(~Metric,nrow=3,labeller=labeller(Metric=metric_labels))

legend = get_legend(ggplot(combine_boxplot,aes(x=State,y=Proportion*100,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors,labels=all_state_labels) + theme(legend.direction = "horizontal",legend.title = element_blank(),legend.position="bottom"))

grid.arrange(a,b,legend, nrow = 3, ncol=2, layout_matrix=cbind(c(1,3,3,4,4),c(2,5,6,6,7)),heights=c(0.5,0.1,0.15,0.1,0.15))
```

### Shuffled TE statistics

```{r shuffled number of states}
...
# Number of states
# Shuffled chromHMM
shuffled_chromHMM_states = ldply(shuffled_chromHMM_potential,function(x) x[,23:24]) #Total states, max states intra
...
shuffled_WGBS_states = ldply(shuffled_WGBS_average,function(x) x[,50:51])
shuffled_WGBS_states$Iteration = as.factor(shuffled_WGBS_states$Iteration)

mean(shuffled_chromHMM_states$Max_states_intra)
mean(shuffled_chromHMM_states$States)
mean(shuffled_WGBS_states$States)
...
# 5_TxWk control
lapply(shuffled_chromHMM_potential,function(x) dim(x[which(x$X5_TxWk >= sample_counts["chrY","chromHMM"] & x$States == 1),]))
lapply(shuffled_chromHMM_potential,function(x) x[which(x$X5_TxWk >= sample_counts["chrY","chromHMM"] & x$States == 1),])
```

### Remove shuffled matrices

```{r}
rm(list=c("shuffled_chromHMM_potential","shuffled_WGBS_average","shuffled_DNase_potential","shuffled_H3K27ac_potential", "rmsk_TE_shuffled"))
```
...
```{r Figure 3, fig.width=7, fig.height=7, echo=FALSE, cache=TRUE, dependson=c("Figure 1","Figure 2","Figure S6","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
#source("R_scripts/potential.R")
...
# By-chromosome (normalized by total number of samples)
test = rmsk_TE_measure[,c("chromosome",states)]
test[,2:16] = apply(test[,2:16],2,function(x) ifelse(test$chromosome != "chrY",x/sample_counts["All","chromHMM"],x/sample_counts["chrY","chromHMM"]))
test[,17:20] = apply(test[,17:20],2,function(x) x/sample_counts["All","WGBS"])
test$DNase = ifelse(test$chromosome != "chrY",test$DNase/sample_counts["All","DNase"],test$DNase/sample_counts["chrY","DNase"])
test$H3K27ac = ifelse(test$chromosome != "chrY",test$H3K27ac/sample_counts["All","H3K27ac"],test$H3K27ac/sample_counts["chrY","H3K27ac"])
test$Expressed_samples = ifelse(test$chromosome != "chrY",test$Expressed_samples/sample_counts["All","RNA"],test$Expressed_samples/sample_counts["chrY","RNA"])

chromosome_potential = aggregate(data=test,.~chromosome,function(x) mean(na.omit(x)),na.action=na.pass)
colnames(chromosome_potential) = c("chromosome",states)
chromosome_potential$chromosome = factor(chromosome_potential$chromosome,levels=standard_chromosomes)
chromosome_potential_long = melt(chromosome_potential,id.var="chromosome")
colnames(chromosome_potential_long)[2:3] = c("State","Mean_samples")

c = ggplot(chromosome_potential_long,aes(x=State,y=Mean_samples,color=chromosome,label=chromosome)) + geom_point() + geom_text(angle=45,hjust=0,vjust=0,size=3) + scale_color_discrete(guide=FALSE) + scale_x_discrete(limits=rev(states),labels=all_state_labels) + theme(axis.title.y = element_blank(),panel.grid.major.y=element_line(color="grey")) + ylab("Mean % of samples in state") + coord_flip()
...
# Proportion of each class on each chromosome
z = apply(table(rmsk_TE$chromosome,rmsk_TE$class_update),1,function(x) x/table(rmsk_TE$class_update))
z
sort(apply(z,2,function(x) sd(x)/mean(x)))

# Proportion of TEs in each state on each chromosome
apply(rmsk_TE_measure[,states],2,function(x) table(rmsk_TE_measure[which(x > 0),]$chromosome)/dim(rmsk_TE_measure[which(x > 0),])[1])
...
ggplot(chrom_state_total,aes(x=chromosome,y=Bases,fill=State)) + geom_bar(stat="identity",position="fill") + scale_fill_manual(values = chromHMM_colors) + theme(axis.title.x = element_blank()) + ylab("% of total bp")

# Fig 3D

# Significant correlations between number of samples in state and metric (not MHC corrected)
correlate_TE_state[which(correlate_TE_state$p.value < 0.001 & abs(correlate_TE_state$estimate.rho) > 0.3 & correlate_TE_state$State != "Max_states_intra"),]
...
# Not MHC corrected
TE_pvalue_all_state = apply(rmsk_TE_measure[,cohorts],2,function(x) apply(rmsk_TE_measure[,c(states,measure_states_extra)],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
TE_pvalue_class_state = ddply(rmsk_TE_measure,~class_update,function(z) apply(z[,cohorts],2,function(x) apply(z[,c(states,measure_states_extra)],2,function(y) unlist(wilcox.test(y~x))["p.value"])))
```

### Supplementary Figure 8

```{r Figure S8, echo=FALSE, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 3","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
...
# Subfamily
rmsk_TE_subfamily_feature_long = melt(rmsk_TE_subfamily[,c("subfamily",cohorts[1:19])],id.var="subfamily")
...
sort(apply(rmsk_TE_measure[,cohorts],2,function(y) median(rmsk_TE_measure[which(y > 0),]$Max_expression,na.rm=TRUE)))

# Max expression by feature
sort(apply(rmsk_TE_measure[,cohorts],2,function(y) max(rmsk_TE_measure[which(y > 0),]$Max_expression,na.rm=TRUE)))
```

### Supplementary Figure 9

```{r Figure S9, echo=FALSE, cache=TRUE, dependson=c("Figure 3","Figure S8","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
#source("R_scripts/TE_class_stats.R")
#source("R_scripts/TE_correlation.R")
...
rmsk_TE_measure_metric = melt(rmsk_TE_measure[,c("class_update",measure_metrics)],id.vars="class_update")
...
```{r}
ddply(rmsk_TE_shuffled_map,.(Iteration),summarise,Mean=mean(mappability),SD=sd(mappability))
```
...
p.adjust(apply(rmsk_TE_measure[,c(states,measure_states_extra)],2,function(x) unlist(kruskal.test(x~rmsk_TE_measure$class_update))["p.value"]),method="bonf")
...
colMeans(rmsk_TE_measure[,measure_metrics])
aggregate(data=rmsk_TE_measure[,c("class_update",measure_metrics)],.~class_update,function(x) mean(na.omit(x)),na.action=na.pass)
sort(apply(aggregate(data=rmsk_TE_measure[,c("class_update",measure_metrics)],.~class_update,function(x) mean(na.omit(x)),na.action=na.pass)[,2:6],2,function(x) sd(x)/mean(x)))
...
wilcox.test(rmsk_TE_measure[which(rmsk_TE_measure$cpgIsland > 0),]$JC_distance,rmsk_TE_measure[which(rmsk_TE_measure$cpgIsland == 0),]$JC_distance)
aggregate(data=rmsk_TE_measure,JC_distance~cpgIsland_binary,mean)
...
# Not MHC corrected
TE_pvalue_all_feature = apply(rmsk_TE_measure[,cohorts],2,function(x) apply(rmsk_TE_measure[,measure_metrics],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
TE_pvalue_class_feature = ddply(rmsk_TE_measure,~class_update,function(z) apply(z[,cohorts],2,function(x) apply(z[,measure_metrics],2,function(y) unlist(wilcox.test(y~x))["p.value"])))
```
...
wilcox.test(rmsk_TE_measure[which(rmsk_TE_measure$States > 1 | (rmsk_TE_measure$States == 1 & rmsk_TE_measure$X15_Quies == 0)),]$mappability,rmsk_TE_measure[which(rmsk_TE_measure$X15_Quies > 0 & rmsk_TE_measure$States == 1),]$mappability)
mean(rmsk_TE_measure[which(rmsk_TE_measure$X15_Quies > 0 & rmsk_TE_measure$States == 1),]$mappability)
mean(rmsk_TE_measure[which(rmsk_TE_measure$States > 1 | (rmsk_TE_measure$States == 1 & rmsk_TE_measure$X15_Quies == 0)),]$mappability)
```

### By chromosome

```{r potential by chromosome}
...
sort(table(rmsk_TE$chromosome)/dim(rmsk_TE)[1])

# % of classes on each chromosome
class_chromosome = table(rmsk_TE$chromosome,rmsk_TE$class_update)
class_chromosome = apply(class_chromosome,2,function(x) x/sum(x))
class_chromosome
```
...
```{r Figure 4, fig.width=7, fig.height=5, echo=FALSE, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 1","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
#source("R_scripts/proportion.R")
...
# MHC adjusted for every group (not state or category)
...
lapply(states[1:21],function(x) ggplot(all_state_proportion[which(all_state_proportion$State == x),],aes(x=reorder(Sample,Bases,max),y=log10(Bases),color=Group,group=Cohort,shape=Cohort)) + geom_point(size=2) + geom_line() + theme(axis.text.x = element_text(angle=90),axis.title.x = element_blank()) + scale_color_manual(values=group_colors))

#lapply(states[1:21],function(x) ggplot(all_state_proportion_matrix[which(all_state_proportion_matrix$State == x),],aes(x=reorder(Sample,TE/Genome,max),y=(TE/(Genome-TE)),color=Group)) + geom_point(shape=16) + scale_color_manual(values=group_colors) + theme(axis.text.x = element_text(angle=90),axis.title.x = element_blank()))
```
...
## By sample
mean(DNase_stats$Summit_in_TE/DNase_stats$Peaks)
sd(DNase_stats$Summit_in_TE/DNase_stats$Peaks)
min(DNase_stats$Summit_in_TE/DNase_stats$Peaks)
DNase_stats[which.min(DNase_stats$Summit_in_TE/DNase_stats$Peaks),]
max(DNase_stats$Summit_in_TE/DNase_stats$Peaks)
...
## By sample
mean(H3K27ac_stats$Summit_in_TE/H3K27ac_stats$Peaks)
sd(H3K27ac_stats$Summit_in_TE/H3K27ac_stats$Peaks)
min(H3K27ac_stats$Summit_in_TE/H3K27ac_stats$Peaks)
H3K27ac_stats[which.min(H3K27ac_stats$Summit_in_TE/H3K27ac_stats$Peaks),]
max(H3K27ac_stats$Summit_in_TE/H3K27ac_stats$Peaks)
...
```{r analysis Fig S11a}
by_sample_stats_class = ddply(by_sample_class,.(State,class),summarise, Range = max(Proportion_state)-min(Proportion_state), Median = median(Proportion_state), Mean = mean(Proportion_state),CV = sd(Proportion_state)/mean(Proportion_state))
by_sample_stats_class[order(by_sample_stats_class$Range),]
by_sample_stats_class[order(by_sample_stats_class$CV),]

# MHC corrected?
by_sample_KW_class = ddply(by_sample_class,.(State,class),summarise,Group = unlist(kruskal.test(Proportion_state,Group))["p.value"],method="bonf")

# MHC corrected for group (not class, State)
by_sample_Wilcox_class = merge(ddply(by_sample_class,.(State,class),summarise,Group_pvalue=wilcox_to_all(Proportion_state,droplevels(Group)),Group=levels(droplevels(Group))),aggregate(data=by_sample_class,Proportion_state~State+Group+class,mean),by=c("State","Group","class"),all.x=TRUE)
...
```{r analysis Fig S11b}
table(by_sample_class[which(by_sample_class$Enrichment > 1.5),c("State","class")]) 
```
...
# MHC corrected for group, not cohort
...
b = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","7_Enh",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
...
rmsk_TE_subfamily[which(rmsk_TE_subfamily$Count_noY <= THRESHOLD_IK_MEMBER),c("subfamily","class_update","Count","Count_noY","Total_length","Total_length_noY","CpGs","Count_CpGs")]

# WGBS
dim(rmsk_TE_subfamily[which(rmsk_TE_subfamily$Count_CpGs <= THRESHOLD_IK_MEMBER),])[1]

rmsk_TE_subfamily[which(rmsk_TE_subfamily$Count_CpG <= THRESHOLD_IK_MEMBER),c("subfamily","class_update","Count","Count_noY","Total_length","Total_length_noY","CpGs","Count_CpGs")]
...
rmsk_TE_subfamily[which(rmsk_TE_subfamily$Count <= THRESHOLD_IJK_MEMBER | rmsk_TE_subfamily$Count_noY <= THRESHOLD_IJK_MEMBER),c("subfamily","Count","Count_noY","Total_length","Total_length_noY","CpGs")]
```
...
a = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","1_TssA",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
b = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","2_TssAFlnk",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
c = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","3_TxFlnk",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
d = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","6_EnhG",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
e = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","4_Tx",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
f = plot_binary_heatmap(subfamily_state_sample_filter,"chromHMM","5_TxWk",2,sample_counts["All","chromHMM"],THRESHOLD_LOR)
g = plot_binary_heatmap(subfamily_state_sample_filter,"WGBS","Hypomethylated",2,sample_counts["All","WGBS"],THRESHOLD_LOR)
h = plot_binary_heatmap(subfamily_state_sample_filter,"WGBS","Intermediate",2,sample_counts["All","WGBS"],THRESHOLD_LOR)
i = plot_binary_heatmap(subfamily_state_sample_filter,"DNase","DNase",2,sample_counts["All","DNase"],THRESHOLD_LOR)
j = plot_binary_heatmap(subfamily_state_sample_filter,"H3K27ac","H3K27ac",2,sample_counts["All","H3K27ac"],THRESHOLD_LOR)
...
# All instances
ddply(subfamily_state_sample_combined,~State,summarize,Percent_mean=mean(na.omit(Percent)),Members_mean=mean(na.omit(Members)),Percent_median=median(na.omit(Percent)), Members_median=median(na.omit(Members)),Percent_min=min(na.omit(Percent)), Members_min=min(na.omit(Members)), Percent_max=max(na.omit(Percent)), Members_max=max(na.omit(Members)))   

# Enriched
ddply(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Enrichment > THRESHOLD_LOR),],~State,summarise,Percent_mean=mean(Percent), Members_mean=mean(Members), Percent_median=median(Percent), Members_median=median(Members),Percent_min=min(Percent), Members_min=min(Members), Percent_max=max(Percent), Members_max=max(Members))

# >1%
ddply(subfamily_state_sample_combined[which(subfamily_state_sample_combined$Length_percent_jk > THRESHOLD_PC),],~State,summarise,Percent_mean=mean(Percent), Members_mean=mean(Members), Percent_median=median(Percent),Members_median=median(Members),Percent_min=min(Percent), Members_min=min(Members), Percent_max=max(Percent), Members_max=max(Members))
...
test = ddply(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Enrichment > THRESHOLD_LOR),c("subfamily","family","class_update","State","Sample",sample_categories,"Members","Percent")],.(State),function(x) x[which(x$Percent > mean(x$Percent)+2*sd(x$Percent)),])
...
test = rmsk_TE_measure[,c("subfamily",states[1:21])]
...
legend = get_legend(ggplot(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Enrichment > THRESHOLD_LOR),],aes(x=State,y=Percent,fill=State)) + geom_boxplot() + scale_fill_manual(values=all_state_colors,labels=all_state_labels))

grid.arrange(a,b,c,d,legend,nrow=2,ncol=3,layout_matrix=cbind(c(1,2),c(3,4),c(7,7)),widths=c(0.4,0.4,0.2))
```
...
TE_meth_subfamily_stats = ddply(TE_meth_subfamily_percent,.(subfamily,State),summarise,Mean=mean(Percent),Range=max(Percent)-min(Percent),Max=max(Percent),All_members=sum(Percent == 1))
...
ddply(TE_meth_subfamily_stats,.(State),summarise,SD=sd(Mean),Mean=mean(Mean),Range=mean(Range),Max=mean(Max))
```
...
# Subfamily features
write.table(rmsk_TE_subfamily_measure[,c("subfamily","family","class_update",measure_metrics_subfam,features[c(1:3,5:8)],"CDS","blacklist","Vista_enhancers","CGate")],file="enrichment/subfamily_features.txt",sep='\t',row.names=FALSE,quote=FALSE)
...
names(enrichment_lor) = states[1:21]
...
names(enrichment_pc) = states[1:21]
...
test = melt(rmsk_TE_subfamily_measure[,c("subfamily",all_pc_states[22:42])],id.vars=c("subfamily"))
test$variable = gsub("_PC","",test$variable)
test = merge(melt(rmsk_TE_subfamily_measure[,c("subfamily",states[1:21])],id.vars=c("subfamily")),test,by=c("subfamily","variable"))
...
rmsk_TE_subfamily_ever = aggregate(data=rmsk_TE_measure[,c("subfamily",states)],.~subfamily,function(x) sum(x > 0,na.rm=TRUE),na.action=na.pass)

lapply(states[1:21],function(x) write_subfamily_candidates(subfamily_state_sample_counts[which(subfamily_state_sample_counts$State == x & subfamily_state_sample_counts$V1 > 1),]$subfamily,x,FALSE))
```
...
subfamily_state_individual_V1$State = factor(subfamily_state_individual_V1$State,levels=states[1:21])
...
aggregate(data=test[,2:5],.~State,function(x) mean(x),na.action=na.pass)
```
...
correlate_subfamily = ldply(c(cohorts,measure_metrics_subfam),function(x) correlate_spearman(rmsk_TE_subfamily_measure,x,all_pc_states))
correlate_subfamily$Metric = rep(c(cohorts,measure_metrics_subfam),each=7*length(all_pc_states))
correlate_subfamily$Metric = factor(correlate_subfamily$Metric,levels=c(cohorts,measure_metrics_subfam))
...
# Not MHC corrected
correlate_subfamily[which(correlate_subfamily$p.value < 0.001 & abs(correlate_subfamily$estimate.rho) > 0.3 & correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% measure_metrics_subfam & correlate_subfamily$class_update == "All"),]

correlate_subfamily[which(correlate_subfamily$p.value < 0.001 & abs(correlate_subfamily$estimate.rho) > 0.3 & correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% measure_metrics_subfam & correlate_subfamily$class_update != "All"),]

correlate_subfamily[which(correlate_subfamily$p.value < 0.001 & abs(correlate_subfamily$estimate.rho) > 0.3 & correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% c(features[1:3],"coding_exon",features[5:8]) & correlate_subfamily$class_update == "All"),]

correlate_subfamily[which(correlate_subfamily$p.value < 0.001 & abs(correlate_subfamily$estimate.rho) > 0.3 & correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% c(features[1:3],"coding_exon",features[5:8]) & correlate_subfamily$class_update != "All"),]

# Correlation between input variables (including feature ratio)
correlate_subfam_feature = ldply(c(cohorts,measure_metrics_subfam),function(x) correlate_spearman(rmsk_TE_subfamily_measure,x,c(cohorts,measure_metrics_subfam)))
correlate_subfam_feature$Metric = rep(c(cohorts,measure_metrics_subfam),each=7*length(c(cohorts,measure_metrics_subfam)))
...
correlate_subfam_feature[which(correlate_subfam_feature$p.value < 0.001 & abs(correlate_subfam_feature$estimate.rho) > 0.3 & correlate_subfam_feature$State %in% measure_metrics_subfam & correlate_subfam_feature$Metric %in% measure_metrics_subfam & correlate_subfam_feature$class_update == "All"),] # Not MHC corrected
...
a = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% measure_metrics_subfam & correlate_subfamily$class_update == "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue",guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(states[1:21])) + scale_x_discrete(limits=measure_metrics_subfam)

b = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% measure_metrics_subfam & correlate_subfamily$class_update != "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue",guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(states[1:21])) + scale_x_discrete(limits=measure_metrics_subfam) + facet_wrap(~class_update)

c = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% c(features[1:3],"coding_exon",features[5:8],"Vista_enhancers") & correlate_subfamily$class_update == "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue",guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(states[1:21])) + scale_x_discrete(limits=c(features[1:3],"coding_exon",features[5:8],"Vista_enhancers"),labels=c(genic_labels,"Vista enhancers"))

d = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% states[1:21] & correlate_subfamily$Metric %in% c(features[1:3],"coding_exon",features[5:8],"Vista_enhancers") & correlate_subfamily$class_update != "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue",guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(states[1:21])) + scale_x_discrete(limits=c(features[1:3],"coding_exon",features[5:8],"Vista_enhancers"),labels=c(genic_labels,"Vista enhancers")) + facet_wrap(~class_update)
...
# State vs. state, PC
f = ggplot(correlate_subfam_state[which(correlate_subfam_state$State %in% all_pc_states[22:42] & correlate_subfam_state$State2 %in% all_pc_states[22:42] & correlate_subfam_state$class_update == "All"),],aes(x=State,y=State2,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))

# State vs. state
g = ggplot(correlate_subfam_state[which(correlate_subfam_state$State %in% states[1:21] & correlate_subfam_state$State2 %in% states[1:21] & correlate_subfam_state$class_update == "All"),],aes(x=State,y=State2,fill=estimate.rho)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue",guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))

h = ggplot(correlate_subfam_state[which(correlate_subfam_state$State %in% states[1:21] & correlate_subfam_state$State2 %in% states[1:21] & correlate_subfam_state$class_update != "All"),],aes(x=State,y=State2,fill=estimate.rho)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue",guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))
...
table(table(droplevels(rmsk_TE[which(!is.na(rmsk_TE$Vista_enhancers)),c(TE_coordinates,"Vista_enhancers")]$subfamily)))

correlate_subfamily[which(correlate_subfamily$Metric == "Vista_enhancers" & correlate_subfamily$State %in% states[1:21] & abs(correlate_subfamily$estimate.rho) > 0.3 & correlate_subfamily$p.value < 0.01),] # Not MHC corrected
```
...
test = rmsk_TE_subfamily_measure[which(rmsk_TE_subfamily_measure$subfamily %in% young_subfamilies),c("subfamily","family","class_update",all_pc_states)]
test[,which(apply(test,2,function(x) sum(as.numeric(x) > 0) > 0))]
```
