...
names(all_state_colors)[20:22] = c("DNase","H3K27ac","Expressed_samples")
...
all_state_colors[["Total"]] = NULL

d = ggplot(contribution_class,aes(x=State,y=Proportion,fill=class)) + geom_bar(stat="identity") + coord_flip() + ylab("Proportion in each TE class") + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.ticks.x=element_blank()) + scale_x_discrete(limits=as.vector(contribution[order(contribution$Proportion,decreasing = TRUE),]$State)) + scale_fill_manual(values=class_colors,guide=FALSE)

legend = get_legend(ggplot(combined_proportion[which(combined_proportion$Coding == "All"),],aes(x=Cohort,y=Proportion,fill=State)) + geom_bar(stat="identity") + scale_fill_manual(values=all_state_colors,labels=all_state_labels) + theme(axis.text.x = element_text(angle=90,hjust=1),axis.title.x=element_blank(),legend.title = element_blank(),legend.position="bottom",legend.direction = "horizontal") + facet_wrap(~Group,nrow=4))

grid.arrange(a,b,c,d,legend, nrow = 4,layout_matrix = rbind(c(1,1),c(2,2),c(3,4),c(5,5)),heights=c(0.25,0.25,0.4,0.1))
```
...
ggplot(all_lengths_blocks,aes(x=State,y=Blocks,fill=State)) + geom_boxplot() + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_fill_manual(values=all_state_colors[c(1:15,20:21)],guide=FALSE) + ylab("Number of blocks") 

# Comparing number of blocks to length of states
ggplot(all_lengths_blocks,aes(x=Blocks,y=Median)) + geom_point() + facet_wrap(~State,scales="free") + xlab("Number of blocks/peaks per sample") + ylab("Median length of blocks/peaks per sample")
