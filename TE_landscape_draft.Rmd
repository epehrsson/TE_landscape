...
CpGs per TE loaded from ''TE_CpG_count.txt'', generated in "TE CpG count" chunk below.
...
There are 15,478,399 200bp windows. All of the intersections with chromHMM are 200bp. Only 24 windows do not have chromHMM annotations (one per chromosome).

The total width of the windows (3,095,677,412) is slightly larger than what is covered by chromHMM (3,095,675,000; no chrM). The missing 2,412 bp is at the end of the chromosomes and does not overlap any TEs (both windows and chromHMM start at base 0 for all chromosomes).  
...
TEs ever in state, split into subfamily x state.
...
All shuffled files have 4,430,788 TEs, same number of class and subfamily as true TEs, but not same number on each chromosome. 
...
Note: chromHMM overlap bp is not the same for shuffled summit TEs as for real summit TEs.
...
chromHMM annotations are 2,583bp shorter than the width of all chromosomes and 2,417bp when chrY is excluded. All RefSeq feature bases are annotated, indicating all chromHMM-unannoted bases are intergenic. 
...
From the 200bp window-chromHMM intersections, pulled out those in the 5 active regulatory states. Then, found the number of unique windows that are ever in one of those states.   
...
The number of shuffled TEs (times 10 iterations) that are ever in an active regulatory state.  
...
TEs that overlap the center of chromHMM blocks.
...
As a QC, found the number of TEs per state that are identified using block summit overlap, but not the standard analysis, and compared it to the total number identified using block summit overlap. The proportion is small, usually <1%. There are no subfamily x state combinations that are present in block overlap but not normally.
...
Repeated the analysis with just TEs that overlap 200bp bin summits. 
...
Files of all TE x sample in the state by state and subfamily, for each active regulatory state. 
...
How many of the shuffled constitutive TEs are redundant? I found the total number of shuffled TEs per state, then compared it to the unique number. There are almost no redundancies, just one in the 2_TssAFlnk state. This is because it is very unlikely for the shuffled TE to land at the same location in different iterations. 

Overlapping bp within the sets. This is a large fraction of the total bp in each shuffled set, particularly for the 1_TssA state. 
...
Intersected the VISTA enhancers with each set of TEs. The real TE 1_TssA and 3_TxFlnk constitutive sets overlap no enhancer.  
...
Ran HOMER (known motifs only) on each of the orthogonal sets. HOMER gives more detail about the motifs [http://homer.ucsd.edu/homer/motif/motifDatabase.html| here].
...
Intersection with all sets, found overlapping elements. 
