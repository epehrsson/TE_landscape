...
```{r Figure S1b, echo=FALSE, cache=TRUE, dependson=c("setup","load libraries","set plot parameters","define vectors")}
...
```{r Figure S1c-e, echo=FALSE}
CpG_state = melt(as.matrix(all_CpG_meth))
CpG_state$Var2 = factor(CpG_state$Var2,levels=rev(meth_states))

c = ggplot(CpG_state,aes(x=Var1,y=value,fill=Var2)) + geom_bar(stat="identity") + theme(axis.text.x=element_text(angle=90),axis.ticks.y=element_blank(),axis.ticks.x=element_blank(),axis.title.x=element_blank()) + scale_fill_manual(limits=meth_states,values=meth_colors,name="Methylation state") + ylab("CpGs in methylation state")
...
rm(list=c("DNase_pca","H3K27ac_pca","enhancer_matrix_pca","enhancer_pca"))
```
