...
# TE classification
class_colors = setNames(c("#4A72E8","#FF6600","#006600","#cc0000","#488E8E","#5C5C5C"),c("DNA","LINE","LTR","SINE","SVA","Other"))
...
Black bar is now contribution across samples, not median. Dashed line is bp contribution; dot-dashed line is CpG contribution. For D/E, black bar is median for all samples.

```{r Figure 4, echo=FALSE, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 1","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
#source("R_scripts/proportion.R")
#source("R_scripts/proportion_class.R")
...
d = ggplot(RNA_proportion,aes(x=1,y=Ratio,color=Group)) + geom_point(position=position_jitterdodge(dodge.width = 0.6)) + theme(panel.grid=element_blank(),axis.title.y=element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + scale_color_manual(values=group_colors,guide=FALSE) + coord_flip() + ylab("TE:Genome average expression") + scale_y_reverse(limits=c(1,0)) + stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median, geom= "crossbar",color="black")

e = ggplot(class_RNA,aes(x=1,y=Ratio,color=Group)) + geom_point(position=position_jitterdodge(dodge.width = 0.6)) + theme(panel.grid=element_blank(),axis.title.y=element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + scale_color_manual(values=group_colors,guide=FALSE) + coord_flip() + ylab("Class:Genome average expression") + ylim(0,1) + facet_wrap(~class_update,nrow=1) + stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median, geom= "crossbar",color="black")
...
grid.arrange(a,b,c,d,e,legend,nrow=3,ncol=3,layout_matrix=rbind(c(1,2,2),c(3,4,5),c(3,6,6)),heights=c(0.6,0.2,0.2),widths=c(0.35,0.25,0.4))
```
...
permute_up[which(permute_up$Padjust < 0.05),]
...
permute_down[which(permute_down$Padjust < 0.05),]
...
ddply(class_RNA,.(Group,class_update),summarise,Median=median(Ratio))
```
...
Histogram - blue is max in one sample, red is across all samples.

Clustering - Alu are triangle, L1 are diamonds; all other families are circles.

Enrichment - peak summits vs. all peak summits for DNase/H3K27ac; bp/CpG in state for chromHMM/WGBS; subfamily length vs. genome length.
...
b = ggplot(sample_chromHMM_tsne,aes(x=V1,y=V2,color=Group,shape=Age)) + geom_point(size=4) + scale_color_manual(values=group_colors) + scale_shape_manual(values=setNames(c(16,17,18,16),c("Non_fetal","Fetal","Cell_line","Unknown")),labels=c("Cell line","Fetal","Non-fetal","Unknown")) + xlab("Axis 1") + ylab("Axis 2")
c = ggplot(subfamily_chromHMM_tsne,aes(x=V1,y=V2,color=class_update,shape=family)) + geom_point(size=4) + scale_color_manual(values=class_colors) + scale_shape_manual(values=family_shapes,guide=FALSE) + xlab("Axis 1") + ylab("Axis 2")

# Enrichment
d = ggplot(subfamily_state_sample_counts,aes(x=State,y=V1,color=class_update)) + geom_point(position=position_jitterdodge(jitter.width=0.5)) + coord_flip() + theme(axis.title.y=element_blank()) + ylab("Number of samples enriched LOR > 1.5") + scale_color_manual(values=class_colors,name="Class") + scale_y_continuous(breaks=c(0,seq(10,120,10),sample_counts["All","chromHMM"]),expand = c(0.01,0.01)) + geom_rect(xmin=0.5,xmax=1.5,ymin=sample_counts["All","H3K27ac"]+1,ymax=sample_counts["All","chromHMM"],fill="grey",color=NA) + geom_rect(xmin=1.5,xmax=2.5,ymin=sample_counts["All","DNase"]+1,ymax=sample_counts["All","chromHMM"],fill="grey",color=NA) + geom_rect(xmin=2.5,xmax=6.5,ymin=sample_counts["All","WGBS"]+1,ymax=sample_counts["All","chromHMM"],fill="grey",color=NA) + geom_vline(xintercept=seq(1.5,20.5,1), colour='grey') + scale_x_discrete(limits = rev(levels(subfamily_state_sample_counts$State)),labels=as.vector(rev(apply(subfamily_state_sample_counts_combine,1,function(x) paste(x[1],"\n(n=",x[2],")",sep="")))))
...
grid.arrange(a,b,c,d,e,nrow=2,ncol=3,layout_matrix=rbind(c(1,2,3),c(4,4,5)))
```

### Subfamily potential

The vast majority (759/968, 78%) are in all 21 states (chromHMM, methylation, DNase, and H3K27ac) across samples. The outliers are very short subfamilies. 
...
# Outliers
merge(subfam_states[which(subfam_states$value < 10 & subfam_states$variable == "States"),],rmsk_TE_subfamily[,c("subfamily","Total_length","Count")],by="subfamily")
merge(subfam_states[which(subfam_states$value < 10 & subfam_states$variable == "Intra"),],rmsk_TE_subfamily[,c("subfamily","Total_length","Count")],by="subfamily")
```
...
```{r Figure SXX, echo=FALSE}
## Samples
a = ggplot(sample_WGBS_tsne,aes(x=V1,y=V2,color=Group)) + geom_point(size=4) + scale_color_manual(values=group_colors,guide=FALSE) + xlab("Axis 1") + ylab("Axis 2")
b = ggplot(sample_DNase_tsne,aes(x=V1,y=V2,color=Group)) + geom_point(size=4) + scale_color_manual(values=group_colors,guide=FALSE) + xlab("Axis 1") + ylab("Axis 2")
c = ggplot(sample_H3K27ac_tsne,aes(x=V1,y=V2,color=Group)) + geom_point(size=4) + scale_color_manual(values=group_colors,guide=FALSE) + xlab("Axis 1") + ylab("Axis 2")

## Subfamilies
d = ggplot(subfamily_WGBS_tsne,aes(x=V1,y=V2,color=class_update,shape=family)) + geom_point(size=4) + scale_color_manual(values=class_colors,guide=FALSE) + scale_shape_manual(values=family_shapes,guide=FALSE)+ xlab("Axis 1") + ylab("Axis 2")
e = ggplot(subfamily_DNase_tsne,aes(x=V1,y=V2,color=class_update,shape=family)) + geom_point(size=4) + scale_color_manual(values=class_colors,guide=FALSE) + scale_shape_manual(values=family_shapes,guide=FALSE)+ xlab("Axis 1") + ylab("Axis 2")
f = ggplot(subfamily_H3K27ac_tsne,aes(x=V1,y=V2,color=class_update,shape=family)) + geom_point(size=4) + scale_color_manual(values=class_colors,guide=FALSE) + scale_shape_manual(values=family_shapes,guide=FALSE)+ xlab("Axis 1") + ylab("Axis 2")

## chromHMM other groupings
g = ggplot(sample_chromHMM_tsne,aes(x=V1,y=V2,color=Anatomy)) + geom_point(size=4) + scale_color_manual(values=anatomy_colors) + xlab("Axis 1") + ylab("Axis 2")
h = ggplot(sample_chromHMM_tsne,aes(x=V1,y=V2,color=Type)) + geom_point(size=4) + scale_color_manual(values=type_colors) + xlab("Axis 1") + ylab("Axis 2")
i = ggplot(sample_chromHMM_tsne,aes(x=V1,y=V2,color=Germline)) + geom_point(size=4) + scale_color_manual(values=germline_colors) + xlab("Axis 1") + ylab("Axis 2")
j = ggplot(sample_chromHMM_tsne,aes(x=V1,y=V2,color=Cancer)) + geom_point(size=4) + scale_color_manual(values=cancer_colors) + xlab("Axis 1") + ylab("Axis 2")

grid.arrange(a,b,c,d,e,f,g,h,i,j,nrow=4,layout_matrix=cbind(c(a,b,c),c(d,e,f),c(g,g,h),c(i,i,j)))
```
...
```{r Figure S12, echo=FALSE, eval=FALSE}
...
```{r core subfamily members, eval=FALSE}
...
```{r Figure 3, echo=FALSE, cache=TRUE, dependson=c("Figure 1","Figure 2","Figure S6","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
#source("R_scripts/potential.R")
source("R_scripts/TE_correlation.R")
