...
```{bash fetch_raw_data_Roadmap.sh, eval=FALSE}
...
```{bash get_mouse_chromHMM.sh, eval=FALSE}
...
```{bash filter_repeats.sh, eval=FALSE}
# Filter repeat file
...
```{bash calculate_length.sh, eval=FALSE}
...
```{bash process_features.sh, eval=FALSE}
...
```{bash merge_features.sh, eval=FALSE}
...
```{bash process_cgate.sh, eval=FALSE}
...
```{bash feature_overlap_length.sh, eval=FALSE}
```

```{bash feature_overlap_individual.sh, eval=FALSE}
...
```{bash mappability.sh, eval=FALSE}
# Mappability
...
```{bash process_WGBS.sh, eval=FALSE}
# Process WGBS
# 5/5/2016, 6/15/2017
...
```{bash intersect_Roadmap.sh, eval=FALSE}
...
```{bash intersect_sample_list_DNase_peak.sh, eval=FALSE}
```

```{bash RNA runfiles, eval=FALSE}
intersect_RNA_agnostic.sh
intersect_RNA_features.sh
intersect_RNA_chrY.sh
```

```{bash combine_RNA.sh, eval=FALSE}
...
```{bash sum_overlap_Roadmap.sh, eval=FALSE}
...
```{bash combine_states.sh, eval=FALSE}
```

```{bash mean_CpG_overlap.sh, eval=FALSE}
...
```{bash state_CpG_overlap.sh, eval=FALSE}
...
```{bash sum_individual_Roadmap.sh, eval=FALSE}
...
```{bash average_meth_individual.sh, eval=FALSE}
...
```{bash add_WGBS_state.sh, eval=FALSE}
...
```{bash potential_chromHMM.sh, eval=FALSE}
# chromHMM potential 
...
```{bash peak_widths.sh, eval=FALSE}
...
```{bash liftover_human_mouse.sh, eval=FALSE}
...
```{bash ortholog_hypo_chromHMM.sh, eval=FALSE}
...
```{bash investigate_ortholog.sh, eval=FALSE}

# Mouse-human candidates
# 5/30/2017, 7/26/2017, 7/31/2017, 10/3/2017, 10/24/2018, 10/25/2018, 1/15/2019

## Overlap with GENCODE promoters 
bedtools intersect -wo -a Mouse/hg19_orthologs_hypo_1TssA.bed -b ~/genic_features/Gencode/Gencodev19_promoter.txt > Mouse/hg19_orthologs_hypo_1TssA_Gencode.txt
...
```{bash make_Browser.sh, eval=FALSE}
...
```{bash enhancer_matrix.sh, eval=FALSE}
...
```{bash run_shuffled_potential.sh, eval=FALSE}
...
```{bash shuffled_mappability.sh, eval=FALSE}
...
```{bash process_vista.sh, eval=FALSE}
...
```{bash profile_chip.sh, eval=FALSE}
# ChIP-seq line plots
...
```{bash investigate_always.sh, eval=FALSE}
...
```{bash state_switching.sh, eval=FALSE}
# State switching
...
```{bash combine_marks.sh, eval=FALSE}
...
Removed at the suggestion of Reviewer 1:

```{r former Fig S16C-F, eval=FALSE}
