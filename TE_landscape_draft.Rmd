...
apply(chromHMM_TE_state[,8:22],2,function(x) dim(chromHMM_TE_state[which(x == chromHMM_TE_state$Tissues),])[1])/NUM_TE
...
# 15_Quies (particular subfamilies?)
...
# Average proportion of a subfamily in each state across all samples (even those not enriched)
subfamily_state_members = merge(aggregate(data=subfamily_state_sample_combined,Percent~subfamily+State,FUN=sum),aggregate(data=subfamily_state_sample_combined,Percent~subfamily+State,FUN=max),by=c("subfamily","State"))
...
# Mean for those where max_percent != zero (ever in state)
ddply(subfamily_state_members,.(State),summarise,Mean=mean(na.omit(Total_Percent/Max_Percent)),SD=sd(na.omit(Total_Percent/Max_Percent)))
