...
```{r vista pooled, fig.height=3, fig.width=4}
...
# Write out enrichment bedfiles for updated subfamily x state combinations
...
ggplot(subfamily_50state_sample_filter,aes(x=State,fill=State18)) + geom_bar(color="black") + facet_wrap(~Sample) + scale_fill_manual(values=chromHMM_states_18,name="State (18-state model)") + ylab("Enrichments LOR > 1.5") + theme(axis.text.x=element_blank(),axis.ticks.x = element_blank(),legend.position="bottom",legend.key.size = unit(2,'mm'))
```

Compare enrichments with the 15-state and 50-state models.

Merge the 15-state and 50-state model enrichments, LOR > 1.5 and active states only.

```{r compare 50 15 state}
subfamily_state_sample_compare = merge(subfamily_50state_sample_filter[which(subfamily_50state_sample_filter$Enrichment > 1.5 & subfamily_50state_sample_filter$State18 %in% names(chromHMM_states_18)[1:11]),c("subfamily","Sample","State","Members","State18")],subfamily_state_sample_filter[which(subfamily_state_sample_filter$State %in% chromHMM_states[1:7] & subfamily_state_sample_filter$Enrichment > 1.5),c("subfamily","family","class_update","Sample",sample_categories,"State","Members")],by=c("subfamily","Sample"))
...
# MER121 E017 in 7_Enh, 15-state
awk '{if($8 == "E017") print $0}' chromHMM/subfamily/by_state/MER121_7_Enh.txt
