...
apply(rmsk_TE[,13:32],2,function(x) length(na.omit(x)))
feature_proportion = apply(rmsk_TE[,13:32],2,function(x) length(na.omit(x))/NUM_TE)
feature_proportion
...
### Shuffled control - change to comparison with shuffled distribution 

```{r shuffled feature distribution}
# Distribution of shuffled TE overlaps
shuffled_features = ldply(rmsk_TE_shuffled,function(x) apply(x[,10:31],2,function(x) length(na.omit(x))))
