...
all_state_labels = setNames(c(chromHMM_states,meth_states,"DNase","H3K27ac","RPKM > 1"),c(chromHMM_states,meth_states,"DNase","H3K27ac","Expression"))
...
```{r Figure S1a, echo=FALSE}
a = ggplot(rmsk_TE,aes(x=reorder(class_update,Length,median),y=log10(Length),fill=class_update)) + geom_boxplot() + ylab("log10(bp)") + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_fill_manual(values=class_colors,guide=FALSE)
```

```{r Figure S1b, echo=FALSE, cache=TRUE}
source("R_scripts/state_lengths.R")

b = ggplot(all_lengths,aes(x=State,y=log10(Length),fill=State)) + geom_boxplot() + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_fill_manual(values=all_state_colors[c(1:15,20:21)],guide=FALSE) + ylab("log10(bp)") 
```

```{r Figure S1c, echo=FALSE, cache=TRUE}
...
# This is a placeholder - load separately!
c = ggplot(hg19_genome,aes(x=chrom,y=size)) + geom_point()
```

```{r Figure S1d-e, echo=FALSE}
CpG_state = melt(as.matrix(all_CpG_meth))
CpG_state$Var2 = factor(CpG_state$Var2,levels=rev(meth_states))

d = ggplot(CpG_state,aes(x=Var1,y=value,fill=Var2)) + geom_bar(stat="identity") + theme(axis.text.x=element_text(angle=90),axis.ticks.y=element_blank(),axis.ticks.x=element_blank(),axis.title.x=element_blank()) + scale_fill_manual(limits=meth_states,values=meth_colors,name="Methylation state") + ylab("CpGs in methylation state")
...
```{r Figure 3, fig.width=7, fig.height=7, echo=FALSE, cache=TRUE}
#source("R_scripts/proportion.R")
...
a = ggplot(combine_class_proportion,aes(x=class,y=Proportion,fill=State)) + geom_bar(stat="identity") + ylab("Proportion of class in state") + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.text.x = element_text(angle=90,hjust=1),axis.title.x = element_blank()) + facet_wrap(~Cohort,nrow=2,scales="free")
...
# Feature table
c = ggplot(feature_state_mean[which(feature_state_mean$Coding == "All"),],aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1)) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-1,50)) + scale_y_discrete(limits=rev(levels(feature_state_mean$Feature)),labels=genic_labels) + scale_x_discrete(labels=all_state_labels)
...
feature_state_mean[which(feature_state_mean$Coding == "All"),]
...
```{r by-chromosome analysis}
# Number of samples in state by chromosome
chromosome_potential = aggregate(data=rmsk_TE_measure[,c(1,35:49,52:55,60:62)],.~chromosome,mean)
lapply(colnames(chromosome_potential[,2:22]),function(x) {test = chromosome_potential[,c("chromosome",x)]; test$Zscore = (test[[x]]-mean(test[[x]]))/sd(test[[x]]); test[order(test$Zscore),]})
```
...
```{r Figure S8, echo=FALSE, cache=TRUE}
...
d = ggplot(feature_state_mean,aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1),panel.background = element_rect(fill="lightgrey")) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-1,50)) + scale_y_discrete(limits=rev(levels(feature_state_mean$Feature)),labels=genic_labels) + scale_x_discrete(labels=all_state_labels) + facet_wrap(~Coding,labeller=labeller(Coding=coding_labels))

e = ggplot(feature_state_mean_class[which(feature_state_mean_class$Coding == "All"),],aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1),panel.background = element_rect(fill="lightgrey")) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-1,175)) + scale_y_discrete(limits=rev(levels(feature_state_mean_class$Feature)),labels=genic_labels) + scale_x_discrete(labels=all_state_labels) + facet_wrap(~Class)
feature_state_mean_class[which(feature_state_mean_class$Coding == "All"),]

grid.arrange(a,b,c,d,e,nrow=5,heights=c(0.15,0.15,0.20,0.20,0.30))
```

### TE feature stats

```{r analysis Fig S8}
rmsk_TE_class[,c(1,12:30)]
...
```{r outliers by feature, eval=FALSE}
...
```{r Figure S9, echo=FALSE, cache=TRUE}
#source("R_scripts/TE_class_stats.R")
#source("R_scripts/TE_correlation.R")
...
grid.arrange(b,c,d,e,f,g,nrow=6,layout_matrix=rbind(c(1:5,6),c(1:5,6)),heights=c(rep(0.14,5),0.3))
```
...
e = ggplot(feature_metric_mean[which(feature_metric_mean$Coding == "All"),],aes(x=Metric,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1),panel.background = element_rect(fill="lightgrey")) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-1,7)) + scale_y_discrete(limits=rev(levels(feature_metric_mean$Feature)),labels=genic_labels) + scale_x_discrete(labels=c("Length (bp)","Mappability","Age","CpGs","CpGs/bp"))
feature_metric_mean[which(feature_metric_mean$Coding == "All"),]

f = ggplot(feature_metric_mean_class[which(feature_metric_mean_class$Coding == "All"),],aes(x=Metric,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1),panel.background = element_rect(fill="lightgrey")) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-1,15)) + scale_y_discrete(limits=rev(levels(feature_metric_mean_class$Feature)),labels=genic_labels) + scale_x_discrete(labels=c("Length (bp)","Mappability","Age","CpGs","CpGs/bp")) + facet_wrap(~Class)
...
```{r Figure 4, fig.width=7, fig.height=5, echo=FALSE, cache=TRUE}
#source("R_scripts/proportion.R")
...
```{r Figure S11, echo=FALSE, cache=TRUE}
...
dim(rmsk_TE_subfamily[which(rmsk_TE_subfamily$Total_length <= THRESHOLD_IK_BASE),c("subfamily","Total_length","Total_length_noY")])[1]
# chromHMM, no Y
dim(rmsk_TE_subfamily[which(rmsk_TE_subfamily$Total_length > THRESHOLD_IK_BASE & rmsk_TE_subfamily$Total_length_noY <= THRESHOLD_IK_BASE),c("subfamily","Total_length","Total_length_noY")])[1]
# CpGs
dim(rmsk_TE_subfamily[which(rmsk_TE_subfamily$CpGs <= THRESHOLD_IK_CPG),])[1]

rmsk_TE_subfamily[which(rmsk_TE_subfamily$Total_length_noY <= THRESHOLD_IK_BASE | rmsk_TE_subfamily$CpGs <= THRESHOLD_IK_CPG),c("subfamily","Count","Total_length","Total_length_noY","CpGs")]

# Subfamilies with no CpGs
rmsk_TE_subfamily[which(rmsk_TE_subfamily$CpGs == 0),c("subfamily","Count","Total_length","Total_length_noY","CpGs")]
```
...
### Average bp of state in subfamily in sample

```{r analysis Fig S13a}
# chromHMM, DNase, H3K27ac
test = rbind(subfamily_state_sample[,c(1,4:5,12:13,16:17)],subfamily_DNase_sample[,c(1,4:5,12:13,16:17)],subfamily_H3K27ac_sample[,c(1,4:5,12:13,16:17)])

merge(aggregate(data=test,Length_ijk~State,median),merge(aggregate(data=test[which(test$Length_ijk >= THRESHOLD_IJK_BASE & test$Length_ik > THRESHOLD_IK_BASE & test$Enrichment > THRESHOLD_LOR),],Length_ijk~State,median),aggregate(data=test[which(test$Length_ijk >= THRESHOLD_IJK_BASE & test$Length_ik > THRESHOLD_IK_BASE & test$Enrichment > THRESHOLD_PC),],Length_ijk~State,median),by="State",all=TRUE),by="State",all=TRUE)

# WGBS
merge(aggregate(data=subfamily_CpG_meth,CpG_ijk~State,median),merge(aggregate(data=subfamily_CpG_meth[which(subfamily_CpG_meth$CpG_ijk >= THRESHOLD_IJK_BASE & subfamily_CpG_meth$CpG_ik > THRESHOLD_IK_BASE & subfamily_CpG_meth$Enrichment > THRESHOLD_LOR),],CpG_ijk~State,median),aggregate(data=subfamily_CpG_meth[which(subfamily_CpG_meth$CpG_ijk >= THRESHOLD_IJK_BASE & subfamily_CpG_meth$CpG_ik > THRESHOLD_IK_BASE & subfamily_CpG_meth$Enrichment > THRESHOLD_PC),],CpG_ijk~State,median),by="State",all=TRUE),by="State",all=TRUE)
```

### Average proportion of members in a state for subfamilies in a state

```{r analysis Fig S13b, cache=TRUE}
# Combined, unfiltered enrichment matrix
test = subfamily_CpG_meth[,c(1:5,15:18)]
colnames(test)[7] = "Length_percent_jk"
subfamily_state_sample_combined = rbind(subfamily_state_sample[,c(1:5,16:19)],test,subfamily_DNase_sample[,c(1:5,16:19)],subfamily_H3K27ac_sample[,c(1:5,16:19)])

# All instances
ddply(subfamily_state_sample_combined,~State,summarize,Percent_mean=mean(na.omit(Percent)), Members_mean=mean(na.omit(Members)),Percent_median=median(na.omit(Percent)), Members_median=median(na.omit(Members)))                               
...
# >1%
ddply(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Length_percent_jk > THRESHOLD_PC),],~State,summarise, Percent_mean=mean(Percent), Members_mean=mean(Members), Percent_median=median(Percent),Members_median=median(Members),Percent_min=min(Percent), Members_min=min(Members), Percent_max=max(Percent), Members_max=max(Members))
...
ddply(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Enrichment > THRESHOLD_LOR),],.(State),function(x) tail(x[order(x$Percent),c(1:11,14:15)],n=10))

test = ddply(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Enrichment > THRESHOLD_LOR),c(1:11,14:15)],.(State),function(x) x[which(x$Percent > mean(x$Percent)+2*sd(x$Percent)),])
test
ddply(test,.(State),function(x) length(unique(x$subfamily)))

# For each subfamily x state (including not enriched), members ever in state vs. max members in state in a single sample
test = rmsk_TE_measure[,c(4,35:49,52:55,60:61)]
colnames(test)[2:16] = chromHMM_states
subfamily_state_members = merge(aggregate(data=melt(test,id.var="subfamily"),value~variable+subfamily,function(x) length(x[which(x > 0)])),aggregate(data=subfamily_state_sample_combined,Members~subfamily+State,FUN=max),by.x=c("subfamily","variable"),by.y=c("subfamily","State"))
mean(subfamily_state_members[which(subfamily_state_members$Members > 0),]$value/subfamily_state_members[which(subfamily_state_members$Members > 0),]$Members)
sd(subfamily_state_members[which(subfamily_state_members$Members > 0),]$value/subfamily_state_members[which(subfamily_state_members$Members > 0),]$Members)
ddply(subfamily_state_members,.(variable),summarise,Mean=mean(na.omit(value/Members)),SD=sd(na.omit(value/Members)),Max=max(na.omit(value/Members)))
```

### WGBS subfamily state stats

```{r analysis Fig S13c, cache=TRUE}
...
ddply(subfamily_state_sample_combined[which(subfamily_state_sample_combined$Enrichment <= 1.5),],~State,function(x) x[which.max(x$Length_percent_jk),])
...
pc_table = merge(pc_table,as.data.frame(table(subfamily_state_sample_filter[which(subfamily_state_sample_filter$Length_percent_jk > THRESHOLD_PC & subfamily_state_sample_filter$Enrichment > THRESHOLD_LOR),]$State)),by.x="State",by.y="Var1",all=TRUE)
...
lapply(c(chromHMM_states,"DNase","H3K27ac"),function(x) if(pc_table[which(pc_table$State == x),]$Enriched_Instances > 0) {join(test[which(test[,x] > 0),c("Order","subfamily","family","class_update","Count","Total_length","CpGs",x)],aggregate(data=subfamily_state_sample_filter[which(subfamily_state_sample_filter$State == x & subfamily_state_sample_filter$Length_percent_jk > THRESHOLD_PC),],Enrichment~subfamily,function(y) sum(y > THRESHOLD_LOR)),by="subfamily",type="left")} else{test[which(test[,x] > 0),c("Order","subfamily","family","class_update","Count","Total_length","CpGs",x)]})
...
```{r analysis Fig S14b, eval=FALSE}
#source("R_scripts/subfamily_enrichment.R")
...
**Figure 6. MER121 is enriched for the 7_Enh enhancer state in mesodermal samples.** (A) GREAT (B) HOMER (C) enrichment

```{r Figure 6, echo=FALSE, eval=FALSE}
...
**Supplementary Figure 15. Subfamily metrics.** (C) Density plot of mean subfamily mappability, by class. (E) Density plot of mean subfamily age, by class.
...
a = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% unique(correlate_subfamily$State)[1:21] & correlate_subfamily$Metric %in% unique(correlate_subfamily$Metric)[c(1:4,26:30)] & correlate_subfamily$class_update == "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac"))) + scale_x_discrete(limits=levels(correlate_subfamily$Metric)[c(1,3,6,4:5,2,7:9)])

b = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% unique(correlate_subfamily$State)[1:21] & correlate_subfamily$Metric %in% unique(correlate_subfamily$Metric)[c(1:4,26:30)] & correlate_subfamily$class_update != "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac"))) + scale_x_discrete(limits=levels(correlate_subfamily$Metric)[c(1,3,6,4:5,2,7:9)]) + facet_wrap(~class_update)

c = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% unique(correlate_subfamily$State)[1:21] & correlate_subfamily$Metric %in% names(genic_labels) & correlate_subfamily$class_update == "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac"))) + scale_x_discrete(limits=names(genic_labels),labels=genic_labels)

d = ggplot(correlate_subfamily[which(correlate_subfamily$State %in% unique(correlate_subfamily$State)[1:21] & correlate_subfamily$Metric %in% names(genic_labels) & correlate_subfamily$class_update != "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac"))) + scale_x_discrete(limits=names(genic_labels),labels=genic_labels) + facet_wrap(~class_update)

# Metric vs. metric
e = ggplot(correlate_subfam_feature[which(correlate_subfam_feature$State %in% unique(correlate_subfamily$Metric)[c(1:4,26:30)] & correlate_subfam_feature$Metric %in% unique(correlate_subfamily$Metric)[c(1:4,26:30)] & correlate_subfam_feature$class_update == "All"),],aes(x=Metric,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_x_discrete(limits=levels(correlate_subfamily$Metric)[c(1,3,6,4:5,2,7:9)]) + scale_y_discrete(limits=levels(correlate_subfamily$Metric)[rev(c(1,3,6,4:5,2,7:9))])

# State vs. state
f = ggplot(correlate_subfam_state[which(correlate_subfam_state$State %in% unique(correlate_subfamily$State)[1:21] & correlate_subfam_state$State2 %in% unique(correlate_subfamily$State)[1:21]),],aes(x=State,y=State2,fill=estimate.rho)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))
...
grid.arrange(a,b,c,d,e,f,g,nrows=4,grid.arrange=rbind(c(1,3,5,6),c(2,4,5,7)))
```
...
ggplot(correlate_subfamily[which(correlate_subfamily$Metric == "Vista_enhancers" & correlate_subfamily$State %in% unique(correlate_subfamily$State)[1:21]),],aes(x=class_update,y=State,fill=estimate.rho)) + geom_tile() + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1)) + scale_y_discrete(limits=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac"))) 
```
...
**Supplementary Figure 17. Mouse TE statistics.** (A) Proportion of human TEs and TE subfamilies with orthologs in mouse, by class. (B) chromHMM states in the 7-state model, defining histone modifications, and the 15-state model state with a similar definition.
...
mean(unique(human_mouse_orthologs_mm10[,1:4])$human_stop_mm10-unique(human_mouse_orthologs_mm10[,1:4])$human_start_mm10)
mean(unique(human_mouse_orthologs_mm10[,5:11])$human_stop_hg19-unique(human_mouse_orthologs_mm10[,5:11])$human_start_hg19)
mean(unique(human_mouse_orthologs_mm10[,12:18])$mouse_stop_mm10-unique(human_mouse_orthologs_mm10[,12:18])$mouse_start_mm10)
...
test = merge(ddply(hg19_mm10_TE_WGBS_subfamily_hypo_paired,.(subfamily),summarise,unlist(cor.test(Mouse,Human,method="spearman"))["estimate.rho"]),rmsk_TE_subfamily[c("subfamily","Age")],by="subfamily")
colnames(test)[2] = "Spearman_correlation"
plot(test$Age,test$Spearman_correlation)
cor.test(test$Age,as.numeric(test$Spearman_correlation),method="spearman")
```
...
**Supplementary Figure 18. ChromHMM states of human-mouse orthologs.**

```{r Fig S18, echo=FALSE}
# Mouse-human chromHMM states
human_mouse_chromHMM_states = matrix(c(chromHMM_states[c(1:2,7,6,4,15,13)],"1","2","3","4","5","6","7"),nrow=7)

mosaic(table(human_mouse_orthologs_mm10_hypo[,19:20])[c(1,8:15,2:7),],shade=TRUE,return_grob=TRUE)
```
...
apply(human_mouse_chromHMM_states,1,function(x) { c(
  pairs = dim(human_mouse_orthologs_mm10_hypo[which(human_mouse_orthologs_mm10_hypo$Human_state == x[1] & human_mouse_orthologs_mm10_hypo$Mouse_State == x[2]),])[1],
  human = dim(unique(human_mouse_orthologs_mm10_hypo[which(human_mouse_orthologs_mm10_hypo$Human_state == x[1] & human_mouse_orthologs_mm10_hypo$Mouse_State == x[2]),9:15]))[1],
  mouse = dim(unique(human_mouse_orthologs_mm10_hypo[which(human_mouse_orthologs_mm10_hypo$Human_state == x[1] & human_mouse_orthologs_mm10_hypo$Mouse_State == x[2]),1:8]))[1]
)})

# Summarise hg19 TE class by state
apply(human_mouse_chromHMM_states,1,function(x) table(unique(human_mouse_orthologs_mm10_hypo[which(human_mouse_orthologs_mm10_hypo$Human_state == x[1] & human_mouse_orthologs_mm10_hypo$Mouse_State == x[2]),9:15])$human_class))

# Summarise hg19 TE subfamily by state
apply(human_mouse_chromHMM_states,1,function(x) sort(table(droplevels(unique(human_mouse_orthologs_mm10_hypo[which(human_mouse_orthologs_mm10_hypo$Human_state == x[1] & human_mouse_orthologs_mm10_hypo$Mouse_State == x[2]),9:15])$human_subfamily))))
```
