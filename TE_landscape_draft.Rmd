...
grid.arrange(a,b,c,d,legend_class, nrow = 4,layout_matrix = rbind(c(1),c(2),c(3,4),c(NA,5)),heights=c(0.2,0.3,0.45,0.05),widths=c(0.55,0.45))
```
...
a = ggplot(all_state_proportion_matrix,aes(x=log10(Genome),y=log10(TE))) + geom_point(aes(color=Group)) + scale_color_manual(values=group_colors) + facet_wrap(~State,scales="free",ncol=6) + xlab("Bases/CpGs in genome (log10)") + ylab("Bases/CpGs in TEs (log10)") + geom_smooth(method="lm",se=FALSE,color="black") + geom_text(data=all_state_proportion_corr,aes(x = -Inf, y = Inf,label=round(Cor,2)),hjust=-0.1,vjust=1.5,size=3) + scale_y_continuous(breaks=pretty_breaks(n=3)) + scale_x_continuous(breaks=pretty_breaks(n=3)) + theme(legend.position="bottom",legend.margin=margin(0,0,0,0),legend.key.size = unit(1,'mm'))

b = ggplot(histones,aes(x=Bin,y=Level,color=Mark,group=Mark)) + geom_line(size=1.5,alpha=0.7) + theme(axis.text.x = element_blank(),axis.ticks.x=element_blank(),panel.grid=element_blank(),legend.position = "bottom",legend.margin=margin(0,0,0,0),legend.key.size = unit(1,'mm')) + ylab("Fold enrichment over input") + scale_color_manual(labels=c(levels(histones$Mark)[1:8],"DNA methylation"),values=c(gg_color_hue(8),"black")) + xlab("Genomic position (+/-5kb around TE)")+ylim(0,12) + facet_wrap(~ State,nrow=3) + scale_y_continuous(sec.axis = sec_axis(~./12.5, name = "DNA methylation level"))
...
a = ggplot(ddply(all_state_proportion[which(all_state_proportion$Coding == "All"),],.(Mark,State,Feature),summarise,Proportion=mean(Proportion)),aes(x=Feature,y=Proportion,fill=State)) + geom_bar(stat="identity") + xlab("Feature") + ylab("Proportion in state") + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + scale_x_discrete(labels=c("TE","Genome",genic_labels)) + facet_wrap(~Mark,nrow=1,labeller=labeller(Mark = mark_labels)) 
...
# Feature overlap
c = ggplot(feature_overlap_long,aes(x=Feature,y=Percent,fill=Coding)) + geom_bar(position="dodge",stat="identity") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + scale_x_discrete(labels=genic_labels) + scale_fill_discrete(labels=coding_labels,name="Feature") + facet_wrap(~Measure,nrow=1,labeller=labeller(Measure=overlap_labels)) + xlab("Feature") + ylab("Proportion")
...
a = ggplot(rmsk_TE_class,aes(x=factor(1),y=Total_length,fill=class_update,label=paste(round(Total_length/1000000,1)," (",round(Total_length/sum(Total_length)*100,0),"%) ",sep=""))) + geom_bar(stat="identity",position="fill",width=0.8) + coord_polar(theta="y") + scale_fill_manual(name="Class",values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + geom_text(position = position_fill(vjust = 0.5),size=3) + ggtitle("Total length (Mb)")

b = ggplot(rmsk_TE_class,aes(x=factor(1),y=Count,fill=class_update,label=paste(format(Count, big.mark=",", scientific=FALSE)," (",round(Count/sum(Count)*100,0),"%) ",sep=""))) + geom_bar(stat="identity",position="fill",width=0.8) + coord_polar(theta="y") + scale_fill_manual(name="Class",values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + geom_text(position = position_fill(vjust = 0.5),size=3) + ggtitle("TEs")

c = ggplot(rmsk_TE_class,aes(x=factor(1),y=Subfamilies,fill=class_update,label=paste(format(Subfamilies, big.mark=",", scientific=FALSE)," (",round(Subfamilies/sum(Subfamilies)*100,0),"%) ",sep=""))) + geom_bar(stat="identity",position="fill",width=0.8) + coord_polar(theta="y") + scale_fill_manual(name="Class",values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + geom_text(position = position_fill(vjust = 0.5),size=3) + ggtitle("Subfamilies")
...
a = ggplot(combine_boxplot,aes(x=State,y=Proportion,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.ticks.y=element_blank(),axis.title.y = element_text(margin = margin(r = -10))) + labs(y="Proportion of TEs in state") + coord_flip() + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels)) + geom_point(data=combine_stats,aes(x=State,y=Proportion_ever),color="red",size=2)

b = ggplot(combine_class_ever,aes(x=State,y=Proportion,fill=forcats::fct_rev(Class))) + geom_bar(stat="identity") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + labs(y="TEs ever in state by class") + coord_flip() + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels))
...
f = ggplot(ddply(combine_potential_class[which(combine_potential_class$Sample.Proportion >= 0.9),],.(State,Class),summarise,TEs.Proportion=sum(Count)/sum(Total)),aes(x=State,y=TEs.Proportion,fill=forcats::fct_rev(Class))) + geom_bar(stat="identity") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + labs(y="Proportion in\neach class (90%)") + coord_flip() + scale_x_discrete(limits = rev(states),labels=rev(all_state_labels))

grid.arrange(a,b,c,d,e,f,legend_class, nrow = 3, layout_matrix=rbind(c(1,2,3),c(4,5,6),c(NA,NA,7)),widths=c(0.4,0.3,0.3),heights=c(0.475,0.475,0.05))
```
...
c = ggplot(combine_potential_class[which(combine_potential_class$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width") + coord_flip() + scale_fill_manual(values=all_state_colors,guide=FALSE) + xlab("State") + ylab("Proportion of samples in state") + scale_x_discrete(limits = rev(states)) + facet_wrap(~Class,nrow=1) + geom_boxplot(width=0.2,outlier.size = 0.1)  + theme(axis.title.y = element_text(margin = margin(r = -10))) + scale_y_continuous(breaks=pretty_breaks(n=3))

grid.arrange(a,b,c,nrow=2,layout_matrix=rbind(c(1,2),c(3)),heights=c(0.55,0.45))
```
...
```{r Figure S5 scripts, echo=FALSE, cache=TRUE, cache.lazy=FALSE}
...
# Promoters
a = ggplot(combine_boxplot_prom,aes(x=State,y=Proportion,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors[1:21],guide=FALSE) + theme(axis.ticks.y=element_blank(),axis.title.y = element_text(margin = margin(r = -10))) + labs(y="Proportion of promoters in state") + coord_flip() + scale_x_discrete(limits=rev(states[1:21]),labels=all_state_labels[1:21]) + geom_point(data=combine_stats_prom,aes(x=State,y=Proportion_ever),color="red",size=2)
...
# Exons
c = ggplot(RNA_exon_sample,aes(x=State,y=Proportion,fill=State)) + geom_boxplot() +scale_fill_manual(values="black",guide=FALSE) + theme(axis.ticks.y=element_blank()) + labs(y="Proportion of exons in state") + coord_flip() + scale_x_discrete(labels="RPKM > 1") + geom_point(data=RNA_potential_exon_stats,aes(x=State,y=Proportion_ever),color="red",size=3) + scale_y_continuous(limits=c(0,1))
...
# No cancer/IMR90
e = ggplot(combine_boxplot_noCancer_IMR90,aes(x=State,y=Proportion,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.ticks.y=element_blank(),axis.title.y = element_text(margin = margin(r = -10))) + labs(y="Proportion of TEs in state") + coord_flip() + scale_x_discrete(limits = rev(levels(combine_boxplot_noCancer_IMR90$State)),labels=rev(all_state_labels)) + geom_point(data=combine_stats_noCancer_IMR90,aes(x=State,y=Proportion_ever),color="red",size=2)
...
### Supplementary Figure NEW (shuffled control)
...
### Supplementary Figure NEW (replicates)
...
source("R_scripts/potential_replicates.R")
```
...
ggplot(chromHMM_replicates_count,aes(x=Pair,y=Fraction,fill=State,shape=Threshold)) + geom_point(color="black",size=2,stroke=1) + 
  ...
  theme(axis.text.x=element_text(angle=90,vjust=0.5)) + scale_shape_manual(values=c(21,22,24,25)) + facet_grid(State~Set,scales="free_x",space="free_x",labeller=labeller(Set=setNames(c("Replicates","Not replicates"),c("Replicate","Tissue"))))
```
...
dcast(ddply(chromHMM_replicates_count,.(Threshold,State,Set),summarise,Median=median(Fraction)),Threshold+State~Set,value.var="Median")
```

Shuffled replicate analysis, currently with iteration 1 only. 

```{bash shuffled replicate, eval=FALSE}
awk '{if(($8 == "1_TssA") || ($8 == "2_TssAFlnk") || ($8 == "3_TxFlnk") || ($8 == "6_EnhG") || ($8 == "7_Enh")) print $0}' chromHMM/shuffled_TEs/rmsk_TE_shuffle_1_chromHMM_sorted.txt > chromHMM/shuffled_TEs/rmsk_TE_shuffle_1_active_reg.txt
```

```{r shuffled replicate analysis, cache=TRUE, cache.lazy=FALSE}
source("R_scripts/replicate_matrices_shuffled.R")
```

```{r plot shuffled replicate analysis}
# Plot
...
### Supplementary Figure NEW (orthogonal)
...
orthogonal_labels = setNames(c("Restricted activity","Shuffled constitutively active","Constitutively active","Inactive"),c("restricted","shuffle_constitutive","constitutive","not_active"))
```
...
# Plot
ggplot(vista_val_fraction,aes(x=State,y=Fraction,shape=Set)) + geom_point() + ylim(0,1) + ylab("Proportion of enhancers positively validated") + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + scale_shape_discrete(labels=orthogonal_labels)
```
...
# Load results
homer_files = list.files(path="orthogonal/homer/",pattern="knownResults.txt",recursive=TRUE,full.names=TRUE)

homer = lapply(homer_files,function(x) read.table(x,sep='\t',header=TRUE,comment.char=""))
names(homer) = homer_files
homer = lapply(homer,setNames,nm=c("Motif Name","Consensus","P-value","Log P-value","q-value (Benjamini)",
                                               "Target Seqs with Motif","% of Target Sequences with Motif",
                                               "Background Seqs with Motif","% of Background Sequences with Motif"))

# Members with motif (no background)
homer_indv = homer[grep("/individual/",names(homer),value=TRUE)]

## Use inactive set for each state
homer_indv = homer_indv[c(1:5,7:11,rep(6,5))]

homer_indv = ldply(homer_indv,.id = "File")
homer_indv$File = gsub("/knownResults.txt","",gsub("orthogonal/homer//individual/","",homer_indv$File))
homer_indv$State = rep(rep(states[c(1:3,6:7)],each=364),3)
homer_indv$Set = c(rep("constitutive",1820),rep("restricted",1820),rep("not_active",1820))
homer_indv$`% of Target Sequences with Motif` = as.numeric(gsub("%","",homer_indv$`% of Target Sequences with Motif`))
homer_indv$`% of Background Sequences with Motif` = as.numeric(gsub("%","",homer_indv$`% of Background Sequences with Motif`))
homer_indv$Set = factor(homer_indv$Set,levels=names(orthogonal_labels))

# Comparisons
homer_known = homer[grep("/individual/",names(homer),invert = TRUE,value=TRUE)]

homer_known = ldply(homer_known,.id = "File")
homer_known$File = gsub("/knownResults.txt","",gsub("orthogonal/homer//","",homer_known$File))
homer_known$State = rep(rep(states[c(1:3,6:7)],each=364),2)
homer_known$Set1 = c(rep("constitutive",1820),rep("restricted",1820))
homer_known$Set2 = c(rep("shuffle_constitutive",3640))
...
# Promoter motifs
ggplot(homer_indv[which(homer_indv$`Motif Name` %in% grep("Promoter",unique(homer_indv$`Motif Name`),value=TRUE)),],aes(x=`Motif Name`,y=`% of Target Sequences with Motif`,shape=Set)) + geom_point() + ylab("% of seqs with motif") + scale_shape_discrete(labels=orthogonal_labels) + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + facet_wrap(~State,ncol=1) 

homer_known[which(homer_known$`Motif Name` %in% grep("Promoter",unique(homer_known$`Motif Name`),value=TRUE) & homer_known$`q-value (Benjamini)` < 0.05),]

# Number of significant comparisons
table(droplevels(homer_known[which(homer_known$`q-value (Benjamini)` < 0.05),c("Set1","Set2","State")]))

# Other significant comparisons
sort(table(droplevels(homer_known[which(homer_known$`q-value (Benjamini)` < 0.05 & homer_known$State %in% c("3_TxFlnk","6_EnhG","7_Enh")),]$"Motif Name")))

# Unique TFs
unique(sapply(homer_known$`Motif Name`,function(x) unlist(strsplit(as.character(x),split='[(]'))[[1]][1]))
```
...
cons = merge(cons,orthogonal_sets,by="File")
cons$Proportion = cons$Overlap/(cons$stop-cons$start)
cons$Set = factor(cons$Set,levels=names(orthogonal_labels))

# Plot
ggplot(cons,aes(x=Set,y=Proportion,fill=State)) + geom_violin() + ylab("Proportion of overlap with phastCons/GERP elements") + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + scale_x_discrete(labels=orthogonal_labels) + facet_grid(State~Database) + scale_fill_manual(values=all_state_colors,guide=FALSE)
...
### Supplementary Figure NEW (block)
...
```{r block summit control, echo=FALSE}
...
a = ggplot(test,aes(x=State,y=Ratio,fill=Category)) + geom_boxplot() + ylim(0,1) + 
  ...
  ylab("TEs in state in sample\n(proportion of paper TEs)") + scale_x_discrete(limits=chromHMM_states[c(2,10:12,3,6:8,1,13,9,4:5,14:15)]) + 
...
### Supplementary Figure NEW (imputed)

The Roadmap project has 98 samples annotated with the [https://egg2.wustl.edu/roadmap/web_portal/chr_state_learning.html#exp_18state](18-state model) (which includes H3K27ac), as well as all samples annotated with a 25-state model generated using imputed data. The 25-state model uses "imputed data for 12 marks, H3K4me1, H3K4me2, H3K4me3, H3K9ac, H3K27ac, H4K20me1, H3K79me2, H3K36me3, H3K9me3, H3K27me3, H2A.Z, and DNase". 

'''18-state model'''

For the 18-state model, 15,478,375 windows have annotations for most samples (the same as for the 15-state model), 15181508 for the samples without chrY. The annotations do not go to the end of the chromosomes.
...
# Summit TEs
while read line; do bedtools intersect -wo -sorted -a features/TEs/rmsk_TEother_summit.txt -b chromHMM/genome/windows/windows_18state_$line\.bed > /scratch/ecp/TE_landscape/state18/rmsk_TEother_summit_$line\.txt; done < <(head -n 51 sample_lists/H3K27ac_samples.txt)
while read line; do bedtools intersect -wo -sorted -a features/TEs/rmsk_TEother_summit.txt -b chromHMM/genome/windows/windows_18state_$line\.bed > /scratch/ecp/TE_landscape/state18/rmsk_TEother_summit_$line\.txt; done < <(tail -n +52 sample_lists/H3K27ac_samples.txt) 
...
The imputed chromHMM annotations do not include chrY. 15,181,531 200bp windows have annotations in each sample (out of 15,478,399; does not include the 296,868 windows on chrY). However, the annotations go to the end of the chromosomes.

```{bash imputed, eval=FALSE}
# Intersect 200bp windows with imputed chromHMM
while read line; do bedtools intersect -wo -a raw_data/chromHMM_imputed25/$line\_25_imputed12marks_mnemonics.bed -b features/hg19_standard.windows | awk -v OFS='\t' '{print $5, $6, $7, $4}' - | sort -k1,1 -k2,2n - > chromHMM/genome/windows/windows_imputed_$line\.bed; done < sample_lists/mnemonics.txt

# Summit
## Intersect summit TEs with imputed chromHMM windows
while read line; do bedtools intersect -wo -sorted -a features/TEs/rmsk_TEother_summit.txt -b chromHMM/genome/windows/windows_imputed_$line\.bed > /scratch/ecp/TE_landscape/imputed/rmsk_TEother_summit_$line\.txt; done < sample_lists/mnemonics.txt

## Filter to summit overlaps
for file in rmsk_TEother_summit_E*.txt; do awk -v OFS='\t' '{if(($9+99.5 >= $2) && ($9+99.5 < $3)) a[$1, $2, $3, $4, $5, $6, $7, $11]+=$12}END{for(i in a) {split(i, sep, SUBSEP); print sep[1], sep[2], sep[3], sep[4], sep[5], sep[6], sep[7], sep[8], a[i]}}' $file > $(basename $file .txt); done &
for file in rmsk_TEother_summit_E[0-9][0-9][0-9] ; do mv $file $file\.txt; done

# Majority
## Intersect majority TEs with imputed chromHMM blocks
while read line; do bedtools intersect -wo -a features/TEs/rmsk_TEother_majority.txt -b raw_data/chromHMM_imputed25/$line\_25_imputed12marks_mnemonics.bed > /scratch/ecp/TE_landscape/imputed/rmsk_TEother_majority_$line\.txt; done < sample_lists/mnemonics.txt

## Identify majority state
for file in rmsk_TEother_majority_*.txt; do python ~/bin/TE_landscape/pick_majority.py $file $(basename $file .txt) 10 11; done
for file in rmsk_TEother_majority_E[0-9][0-9][0-9]; do mv $file $file\.txt; done

# Combine and sort
while read line; do awk -v OFS='\t' -v sample=$line '{print $0, sample, "summit"}' rmsk_TEother_summit_$line\.txt >> rmsk_TEother_imputed.txt; awk -v OFS='\t' -v sample=$line '{print $0, sample, "majority"}' rmsk_TEother_majority_$line\.txt >> rmsk_TEother_imputed.txt; done < ~/TE_landscape/sample_lists/mnemonics.txt &
awk '{print>$1}' rmsk_TEother_imputed.txt
# Where chromosomes.txt is chr 1-22 and sex chromosomes
while read line; do sort -k1,1V -k2,2n -k3,3n -k4,4 -k10,10 $line >> rmsk_TEother_imputed_sorted.txt; rm $line; done < chromosomes.txt
```

```{bash imputed potential, eval=FALSE}
# List of states in 25-state model
sample_lists/chromHMM_imputed_states.txt

# Potential 
python ~/bin/TE_landscape/potential.py chromHMM/rmsk_TEother_imputed_sorted.txt features/TEs/rmsk_TEother.txt sample_lists/chromHMM_imputed_states.txt sample_lists/mnemonics.txt chromHMM/potential/rmsk_TEother_imputed_potential.txt 0 9 7 8 &

# Number of TEs in state per sample
awk -v OFS='\t' '{a[$8,$10]+=1}END{for(i in a){split (i, sep, SUBSEP); print sep[1], sep[2], a[i];}}' chromHMM/rmsk_TEother_imputed_sorted.txt > chromHMM/state_sample_counts_imputed.txt
```

```{r imputed potential load}
# Load matrix
chromHMM_imputed = read.table("chromHMM/potential/rmsk_TEother_imputed_potential.txt",sep='\t',header=TRUE,quote="")
colnames(chromHMM_imputed)[8:32] = names(imputed_states)

# Potential
## Distribution
chromHMM_imputed_dist = sample_distribution(chromHMM_imputed,c(8:32),sample_counts["All","chromHMM"])
chromHMM_imputed_potential = melt(chromHMM_imputed_dist,id.vars="Samples")
colnames(chromHMM_imputed_potential) = c("Samples","State","Count")
chromHMM_imputed_potential = ddply(chromHMM_imputed_potential,.(State),transform,Sample.Proportion = Samples/(length(Samples)-1))

## Stats
chromHMM_imputed_stats = potential_stats(chromHMM_imputed_dist,25,sample_counts["All","chromHMM"])
chromHMM_imputed_stats$State = factor(rownames(chromHMM_imputed_stats),levels=names(imputed_states))

## Number of TEs in each chromHMM state by sample
state_sample_imputed = read.table("chromHMM/state_sample_counts_imputed.txt",sep='\t',quote="")
colnames(state_sample_imputed) = c("State","Sample","Count")
state_sample_imputed$Count = as.numeric(state_sample_imputed$Count)
state_sample_imputed$Proportion = state_sample_imputed$Count/NUM_TE_noY
state_sample_imputed$State = factor(state_sample_imputed$State,levels=names(imputed_states))
```

The 25-state model splits the active regulatory states into much finer detail (15 vs. 5), splits the transcribed states into 4 (from 2), and removes the weakly Polycomb repressed state and the bivalent enhancer state. Using this model, the median number of TEs in the quiescent state per sample is actually greatly increased to 77%, although interestingly, on 89% of TEs are ever in the state. Only 30% of TEs can be in an active regulatory state (15 states), 33% in a transcribed state (4 states), 7% in a poised regulatory state (2 states), and 14% in a repressed state (2 states). 19% of TEs can be in the 17_EnhW2 state (Weak Enhancer 2), 12% in 18_EnhAc (Primary H3K27ac possible Enhancer), and 11% in 15_EnhAF (Active Enhancer Flank); they are less represented in the stronger enhancer states, although 9% are in 14_EnhA2 (Active Enhancer 2).

```{r imputed potential analysis}
ddply(state_sample_imputed,.(State),summarise,Median=median(Proportion))

## Active regulatory states (states 1-4, 9-19)
sum(apply(chromHMM_imputed[c(8:11,16:26)],1,function(x) sum(x > 0)) > 0)/NUM_TE
## Transcribed states (states 5-8)
sum(apply(chromHMM_imputed[12:15],1,function(x) sum(x > 0)) > 0)/NUM_TE
## Poised regulatory states (states 22-23)
sum(apply(chromHMM_imputed[29:30],1,function(x) sum(x > 0)) > 0)/NUM_TE
## Repressed states (states 21, 24)
sum(apply(chromHMM_imputed[c(28,31)],1,function(x) sum(x > 0)) > 0)/NUM_TE

ddply(chromHMM_imputed_potential,.(State),summarise,Count=sum(Count[which(Samples != 0)]))
chromHMM_imputed_stats
```

Potential analysis (Fig 2A/C) using other chromHMM models. 

```{r 18 imputed state potential figure, echo=FALSE}
a = ggplot(state_sample_18state,aes(x=State,y=Proportion,fill=State)) + geom_boxplot() +scale_fill_manual(values=chromHMM_states_18,guide=FALSE) + scale_x_discrete(limits=rev(names(chromHMM_states_18))) + theme(axis.ticks.y=element_blank(),axis.title.y = element_text(margin = margin(r = -5))) + labs(y="Proportion of TEs in state") + ylim(0,1) + coord_flip() + geom_point(data=chromHMM_18state_stats,aes(x=State,y=Proportion_ever),color="red",size=2)

b = ggplot(chromHMM_18state_potential[which(chromHMM_18state_potential$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width") + coord_flip() + scale_fill_manual(values=chromHMM_states_18,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + ylab("Proportion of samples in state") + scale_x_discrete(limits=rev(names(chromHMM_states_18))) + geom_boxplot(width=0.2,outlier.size = 0.1)

c = ggplot(state_sample_imputed,aes(x=State,y=Proportion,fill=State)) + geom_boxplot() +scale_fill_manual(values=imputed_states,guide=FALSE) + scale_x_discrete(limits=rev(names(imputed_states))) + theme(axis.ticks.y=element_blank(),axis.title.y = element_text(margin = margin(r = -5))) + labs(y="Proportion of TEs in state") + ylim(0,1) + coord_flip() + geom_point(data=chromHMM_imputed_stats,aes(x=State,y=Proportion_ever),color="red",size=2)

d = ggplot(chromHMM_imputed_potential[which(chromHMM_imputed_potential$Samples != 0),],aes(x=State,y=Sample.Proportion,weight=Count,fill=State)) + geom_violin(scale="width") + coord_flip() + scale_fill_manual(values=imputed_states,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank(),axis.text.y = element_blank()) + ylab("Proportion of samples in state") + scale_x_discrete(limits=rev(names(imputed_states))) + geom_boxplot(width=0.2,outlier.size = 0.1)

grid.arrange(a,b,c,d,nrow=2,heights=c(0.45,0.55))
```

'''Compare models'''

To compare the 15-state to the 25-state model, I split both files by sample. Then, intersected the TEs to get identical TEs. Confirmed that all had 4,399,208 TEs, as expected. There are two locations where two TEs overlap each other exactly, but they were removed from the files. Then, I found the number of TEs per state for each annotation.  

```{bash compare 15 25 chromHMM, eval=FALSE}
# Split by sample
awk '{print>$10}' rmsk_TEother_imputed_sorted.txt 
awk '{print>"chromHMM/"$8}' ~/TE_landscape/chromHMM/rmsk_TEother_chromHMM_summit_sorted.txt

# Intersect and filter to same TE
while read line; do bedtools intersect -f 1 -r -sorted -wo -a imputed/$line -b chromHMM/$line > $line; done < ~/TE_landscape/sample_lists/mnemonics.txt
while read line; do awk -v OFS='\t' '{if(($1 == $12) && ($2 == $13) && ($3 == $14) && ($4 == $15) && ($5 == $16) && ($6 == $17) && ($7 == $18)) print $1, $2, $3, $4, $5, $6, $7, $10, $21, $8; else print $0 >> "mismatches.txt"}' $line > $line\.bed; done < ~/TE_landscape/sample_lists/mnemonics.txt

# Number of TEs per state pair
for file in *.bed; do awk -v OFS='\t' '{a[$9, $10]+=1}END{for(i in a){split(i, sep, SUBSEP); print sep[1], sep[2], a[i]}}' $file >> counts.txt; done
awk -v OFS='\t' '{a[$1, $2]+=$3}END{for(i in a){split(i, sep, SUBSEP); print sep[1], sep[2], a[i]}}' counts.txt > counts_imputed.txt
```

I got the corresponding colors for the imputed states from the [https://egg2.wustl.edu/roadmap/web_portal/imputed.html#chr_imp| Roadmap website], but many states share the same color. 

```{r imputed analysis, echo=FALSE}
...
imputed$Imputed.State = factor(imputed$Imputed.State,levels=names(imputed_states))

ggplot(imputed,aes(x=State,y=Count,fill=Imputed.State)) + geom_bar(position="fill",stat="identity") + xlab("State (15-state model)") + ylab("Proportion of instances in state") + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + scale_fill_manual(values=imputed_states,name="State (25-state model)")
```
...
# State dynamics
a = ggplot(chromHMM_dynamics,aes(x=States,y=..scaled..,fill=forcats::fct_rev(variable))) + geom_density(adjust=10,alpha=0.5) + scale_fill_manual(values=chromHMM_colors,name="State",limits=chromHMM_states) + xlim(1,15) + ylab("Density") + theme(legend.position="bottom",legend.margin=margin(0,0,0,0),legend.key.size = unit(3,'mm')) + guides(fill=guide_legend(nrow=4,title.position = "top"))

b = ggplot(meth_dynamics,aes(x=States,y=..scaled..,fill=forcats::fct_rev(variable))) + geom_density(adjust=10,alpha=0.8) + scale_fill_manual(values=meth_colors,name="State",limits=meth_states) + ylab("Density") + theme(legend.position="bottom",legend.margin=margin(0,0,0,0),legend.key.size = unit(3,'mm')) + guides(fill=guide_legend(nrow=4,title.position = "top"))
...
# State switching intra, chromHMM
d = ggplot(melt(as.matrix(state_switching_intra)),aes(x=Var2,y=Var1,fill=value)) + geom_tile() + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + scale_x_discrete(labels=chromHMM_states) + scale_y_discrete(labels=rev(chromHMM_states),limits=rev(levels(melt(as.matrix(state_switching_intra))$Var1))) + scale_fill_gradient(low="white",high="darkblue",limits=c(0,1),name="Proportion of\nTE instances") + coord_equal() + xlab("State 2") + ylab("State 1")
...
grid.arrange(a,b,c,d,e,scale_switching,nrow=4,layout_matrix=rbind(c(1,2),c(3,4),c(5),c(6)),heights=c(0.35,0.3,0.3,0.05))
```
...
### Supplementary Figure NEW (state switching)
...
```{r shuffled dynamics control, echo=FALSE}
a = ggplot(shuffled_chromHMM_inter,aes(x=State2,y=State1,fill=Mean_samples/sample_counts["All","chromHMM"])) + geom_tile() +
  theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5),legend.title = element_blank()) + scale_y_discrete(limits=rev(chromHMM_states)) + scale_x_discrete(labels=chromHMM_states) + scale_fill_gradient(low="white",high="darkmagenta",limits=c(0,1),guide=FALSE) + coord_equal() + facet_wrap(~Iteration,nrow=2)
...
b = ggplot(shuffled_meth_inter,aes(x=State2,y=State1,fill=Mean_samples/sample_counts["All","WGBS"])) + geom_tile() +
  ...
  scale_fill_gradient(low="white",high="darkmagenta",limits=c(0,1),guide=FALSE) + coord_equal() + facet_wrap(~Iteration,nrow=2)
...
a = ggplot(by_chromHMM,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  ...
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),strip.text.x = element_text(size = 5), plot.title=element_text(size=8)) + ylab("Proportion of TE x sample") + 
...
b = ggplot(by_WGBS,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  ...
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.title=element_text(size=8)) + ylab("Proportion of TE x sample") + 
...
legend_states = get_legend(ggplot(by_DNase,aes(x=1,y=Proportion,fill=State)) + geom_bar(stat="identity",position="stack") + 
  theme(legend.position = "bottom",legend.key.size = unit(3,'mm')) + scale_fill_manual(values=c(chromHMM_colors,meth_colors,setNames(c("gold","darkgrey"),c("True","False")))) + guides(fill=guide_legend(ncol=7,title.position = "top")))

grid.arrange(a,b,c,d,e,legend_states, nrow=3, layout_matrix=rbind(c(1),c(2,3,4,5),c(6)),widths=c(0.34,0.22,0.22,0.22),heights=c(0.45,0.45,0.1))
```
...
# Potential
a = ggplot(subfam_states[which(subfam_states$variable == "States"),],aes(x=value)) + geom_histogram(binwidth=1,fill="black") + ylab("Subfamilies") + xlab("States") + facet_wrap(~Metric,scales="free_x",nrow=1) + scale_y_continuous(limits=c(0,968)) + expand_limits(x=0)

# Clustering
b = ggplot(sample_chromHMM_pca$eigenvectors,aes(x=PC1,y=PC2,color=Group,shape=Age)) + geom_point() + scale_color_manual(values=group_colors,guide=FALSE) + scale_shape_manual(values=setNames(c(16,17,18,16),c("Non_fetal","Fetal","Cell_line","Unknown")),labels=c("Cell line","Fetal","Non-fetal","Unknown")) + labs(x=paste("PC1 (",round(sample_chromHMM_pca$eigenvalues[1],1),"%)",sep=""),y=paste("PC2 (",round(sample_chromHMM_pca$eigenvalues[2],1),"%)",sep="")) + theme(aspect.ratio = 1,legend.margin=margin(0,0,0,0),legend.key.size = unit(1,'mm'))
...
e = ggplot(homer_top5,aes(x=reorder(Motif.Name,`q.value..Benjamini.`,median),y=PC,fill=variable)) + geom_bar(stat="identity",position="dodge") + ylab("% TEs with motif") + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5)) + scale_fill_discrete(guide=FALSE) + scale_x_discrete(labels=c("Nkx2.1","Nkx2.2","Tcf3","Fli1","HNF4a")) + xlab("Transcription factor")
...
grid.arrange(a,b,c,d,e,legend_group,legend_class,nrow=6,ncol=2,layout_matrix=rbind(c(1),c(6),c(2,4),c(3,4),c(5,4),c(5,7)),heights=c(0.2,0.05,0.25,0.25,0.2,0.05),widths=c(0.5,0.5))
```
...
ggplot(test,aes(x=Sample.Proportion,y=..scaled..,fill=Iteration)) + geom_density(alpha=0.5) + facet_wrap(~State) + ylab("Subfamily density") + scale_fill_manual(values=c(rep("grey",10),"red")) + xlab("Proportion of samples in state") + theme(legend.position="bottom") + scale_x_continuous(breaks=pretty_breaks(seq(0,1,0.5)))
```
...
subfamily_potential_shuffle_ever = ddply(ddply(subfamily_state_potential_shuffle,.(Iteration,Metric,subfamily),summarise,States=sum(Samples > 0)),.(Iteration,Metric,States),summarise,Subfamilies=length(subfamily))
...
legend_shape_age = get_legend(ggplot(sample_H3K27ac_pca$eigenvectors,aes(x=PC1,y=PC2,color=Group,shape=Age)) + geom_point() + scale_color_manual(values=group_colors,guide=FALSE) + scale_shape_manual(values=setNames(c(16,17,18,16),c("Non_fetal","Fetal","Cell_line","Unknown")),labels=c("Cell line","Fetal","Non-fetal","Unknown")) + theme(aspect.ratio = 1,legend.position = "bottom",legend.margin=margin(0,0,0,0),legend.key.size = unit(1,'mm')))
...
c = ggplot(great[which(great$set == "AmnSINE1_7_Enh"),],aes(x=reorder(`Term Name`,`Binom Raw P-Value`,median),y=log10(`Binom Raw P-Value`))) + geom_bar(stat="identity") + coord_flip() + theme(axis.title.y = element_blank(),plot.title = element_text(hjust = 1),axis.text=element_text(size=6),plot.margin = margin(0,0,0,0)) + ylab("-Log10(p-value)") + ggtitle("AmnSINE1, 7_Enh")

d = ggplot(great[which(great$set == "AmnSINE2_DNase"),],aes(x=reorder(`Term Name`,`Binom Raw P-Value`,median),y=log10(`Binom Raw P-Value`))) + geom_bar(stat="identity") + coord_flip() + theme(axis.title.y = element_blank(),plot.title = element_text(hjust = 1),axis.text=element_text(size=6),plot.margin = margin(0,0,0,0)) + ylab("-Log10(p-value)") + ggtitle("AmnSINE2, DHS")

e = ggplot(great[which(great$set == "MER121_7_Enh"),],aes(x=reorder(`Term Name`,`Binom Raw P-Value`,median),y=log10(`Binom Raw P-Value`))) + geom_bar(stat="identity") + coord_flip() + theme(axis.title.y = element_blank(),plot.title = element_text(hjust = 1),axis.text=element_text(size=6),plot.margin = margin(0,0,0,0)) + ylab("-Log10(p-value)") + ggtitle("MER121, 7_Enh")

f = ggplot(great[which(great$set == "MER131_DNase"),],aes(x=reorder(`Term Name`,`Binom Raw P-Value`,median),y=log10(`Binom Raw P-Value`))) + geom_bar(stat="identity") + coord_flip() + theme(axis.title.y = element_blank(),plot.title = element_text(hjust = 1),axis.text=element_text(size=6),plot.margin = margin(0,0,0,0)) + ylab("-Log10(p-value)") + ggtitle("MER131, DHS")
...
# Plot
a = ggplot(subfamily_state_sample_combined[which(subfamily_state_sample_combined$subfamily %in% pc_subfamilies & subfamily_state_sample_combined$State %in% chromHMM_states[1:7]),],aes(x=reorder(Sample,match(Group,group_order)),y=reorder(subfamily,rmsk_TE_subfamily[match(subfamily,rmsk_TE_subfamily$subfamily),]$Total_length),fill=Length_percent_jk)) + geom_tile(linetype="blank") + theme(axis.text.x = element_text(angle=90,vjust=0.5,size=5),axis.ticks.x=element_blank(),axis.title.y = element_text(margin = margin(r = -10))) + scale_fill_gradient(low="white",high="darkblue",limits=c(0,0.12),guide=FALSE) +  facet_wrap(~State,nrow=2) + xlab("Sample") + ylab("Subfamily") + geom_vline(xintercept=22.5,color="black") + geom_vline(xintercept=47.5,color="black") + geom_vline(xintercept=59.5,color="black") + geom_vline(xintercept=71.5,color="black") + geom_vline(xintercept=79.5,color="black") +  geom_vline(xintercept=111.5,color="black")

b = ggplot(subfamily_state_sample_combined[which(subfamily_state_sample_combined$subfamily %in% pc_subfamilies & subfamily_state_sample_combined$State == "DNase"),],aes(x=reorder(Sample,match(Group,group_order)),y=reorder(subfamily,rmsk_TE_subfamily[match(subfamily,rmsk_TE_subfamily$subfamily),]$Total_length),fill=Length_percent_jk)) + geom_tile(linetype="blank") + theme(axis.text.x = element_text(angle=90,vjust=0.5,size=5),axis.ticks.x=element_blank(),axis.title.y = element_text(margin = margin(r = -10))) + scale_fill_gradient(low="white",high="darkblue",limits=c(0,0.12),guide=FALSE) +  facet_wrap(~State,ncol=3) + xlab("Sample") + ylab("Subfamily") + geom_vline(xintercept=8.5,color="black") + geom_vline(xintercept=16.5,color="black") + geom_vline(xintercept=18.5,color="black") + geom_vline(xintercept=23.5,color="black") + geom_vline(xintercept=28.5,color="black") +  geom_vline(xintercept=38.5,color="black")

c = ggplot(subfamily_state_sample_combined[which(subfamily_state_sample_combined$subfamily %in% pc_subfamilies & subfamily_state_sample_combined$State == "H3K27ac"),],aes(x=reorder(Sample,match(Group,group_order)),y=reorder(subfamily,rmsk_TE_subfamily[match(subfamily,rmsk_TE_subfamily$subfamily),]$Total_length),fill=Length_percent_jk)) + geom_tile(linetype="blank") + theme(axis.text.x = element_text(angle=90,vjust=0.5,size=5),axis.ticks.x=element_blank(),axis.text.y = element_blank(),axis.ticks.y=element_blank(),axis.title.y=element_blank()) + scale_fill_gradient(low="white",high="darkblue",limits=c(0,0.12),guide=FALSE) +  facet_wrap(~State,ncol=3) + xlab("Sample") + ylab("Subfamily") + geom_vline(xintercept=16.5,color="black") + geom_vline(xintercept=35.5,color="black") + geom_vline(xintercept=42.5,color="black") + geom_vline(xintercept=52.5,color="black") + geom_vline(xintercept=57.5,color="black") +  geom_vline(xintercept=81.5,color="black")
...
scale_length_jk = get_legend(ggplot(subfamily_state_sample_combined[which(subfamily_state_sample_combined$subfamily %in% pc_subfamilies & subfamily_state_sample_combined$State %in% chromHMM_states[1:7]),],aes(x=reorder(Sample,match(Group,group_order)),y=reorder(subfamily,rmsk_TE_subfamily[match(subfamily,rmsk_TE_subfamily$subfamily),]$Total_length),fill=Length_percent_jk)) + geom_tile(linetype="blank") + scale_fill_gradient(name="Proportion of\nin subfamily",low="white",high="darkblue",limits=c(0,0.12)) + theme(legend.position = "bottom"))

grid.arrange(a,b,c,d,scale_length_jk,layout_matrix=rbind(c(1,1,1),c(2,3,4),c(5)),heights=c(0.65,0.3,0.05),widths=c(0.35,0.35,0.3))
```
...
```{r Figure S14 scripts, echo=FALSE, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 6 scripts")}
...
b = ggplot(chr_state_WGBS_total,aes(x=chromosome,y=CpGs,fill=State)) + geom_bar(position="fill",stat="identity") + scale_fill_manual(values=meth_colors) + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5),legend.position="bottom",legend.box.margin = margin(t=-10),legend.margin=margin(0,0,0,0),legend.key.size = unit(3,'mm')) + ylab("Proportion in state") + xlab("Chromosome") + guides(fill = guide_legend(nrow=4,title.position = "top"))
...
# Feature vs. state
f = ggplot(na.omit(feature_state_mean[which(feature_state_mean$Coding == "All"),]),aes(x=Feature,y=State,fill=Enrichment*100)) + geom_tile() + theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5),panel.background = element_rect(fill="lightgrey"),axis.title.y = element_text(margin = margin(r = -10)),axis.title.x = element_text(margin = margin(t = -10))) + scale_fill_gradient2(name="% increase\nin sample\nproportion",low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-100,4000)) + scale_x_discrete(labels=genic_labels) + scale_y_discrete(limits=rev(states),labels=all_state_labels)

grid.arrange(a,b,c,d,e,f,layout_matrix=rbind(c(1,2),c(3,4),c(5,6)),heights=c(0.35,0.25,0.4))
```

### Supplementary Figure NEW (intergenic potential)
...
```{r intergenic potential, echo=FALSE}
source("R_scripts/intergenic_potential.R")

a = ggplot(state_genic[which(!(state_genic$class_update %in% c("SVA","Other")) & state_genic$State %in% states[c(1,4:5,7,20:21)]),],
...
b = ggplot(na.omit(feature_state_mean_class[which(feature_state_mean_class$Coding == "All"),]),aes(x=State,y=Feature,fill=Enrichment*100)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1,vjust=0.5),panel.background = element_rect(fill="lightgrey")) + scale_fill_gradient2(name="% increase",low="darkblue",high="darkred",mid="white",midpoint=0) + scale_y_discrete(limits=rev(levels(feature_state_mean_class$Feature)),labels=genic_labels) + scale_x_discrete(labels=all_state_labels) + facet_wrap(~Class)

grid.arrange(a,b,nrow=2)
```

### Supplementary Figure NEW (CpG density repression)
...
```{r repression modelling analysis}
...
```{r analysis Fig S15, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 6 scripts","Figure S15")}
# States
...
a = ggplot(human_mouse_orthologs_WGBS_paired,aes(x=Human_methylation,y=Mouse_methylation)) + stat_bin2d(bins=50) + scale_fill_gradient(low="white",high="darkblue",name="Number of TEs") + xlab("Methylation level (human)") + ylab("Methylation level\n(mouse)") + coord_equal() + theme(legend.position = "bottom",legend.direction="horizontal",legend.box.margin = margin(t=-10),legend.key.size = unit(3,'mm'))
...
d = ggplot(hg19_orthologs_specific,aes(x=factor(1),y=Count,fill=Subset)) + geom_bar(stat="identity",position="fill",width=0.8) + coord_polar(theta="y") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank(),strip.text.x = element_text(size = 8)) + facet_grid(State~Tissue,labeller=labeller(Tissue = setNames(c("Brain","Instestine","Stomach","Heart","Lung"),unique(hg19_orthologs_specific$Tissue)))) + scale_fill_discrete(labels=c("<2 samples","Tissue-specific","2-8 samples","> 8 samples"),name="Category") + guides(fill = guide_legend(nrow = 4))
...
grid.arrange(a,b,c,d,e,f,g,legend,nrow=4,layout_matrix=rbind(c(1,4,4),c(2,3,3),c(5,6,7),c(8,8,8)),heights=c(0.3,0.35,0.3,0.05))
```

### Supplementary Figure 16

```{r Figure S16, echo=FALSE,fig.height=3}
...
a = ggplot(ortholog_stats,aes(x=Category,y=TEs,fill=class_update,label=paste(format(TEs, big.mark=",", scientific=FALSE)," (",round(Percent,0),"%) ",sep=""))) + geom_bar(stat="identity",position="fill",width=0.8) + coord_polar(theta="y") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + ggtitle("TEs") + geom_text(position = position_fill(vjust = 0.5),size=3)
...
b = ggplot(ortholog_stats_subfam,aes(x=Category,y=Count,fill=class_update,label=paste(format(Count, big.mark=",", scientific=FALSE)," (",round(Percent,0),"%) ",sep=""))) + geom_bar(stat="identity",position="fill",width=0.8) + coord_polar(theta="y") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) + ggtitle("Subfamilies") + geom_text(position = position_fill(vjust = 0.5),size=3)

grid.arrange(a,b,legend_class,nrow=2,layout_matrix=rbind(c(1,2),c(3)),heights=c(0.95,0.05))
```
...
```{r Figure S18 scripts, echo=FALSE, cache=TRUE, cache.lazy=FALSE}
...
ggplot(all_lengths_blocks,aes(x=log10(Blocks),y=log10(Median))) + geom_point(aes(color=Group)) + xlab("Number of blocks/peaks (log10)") + ylab("Median length of blocks/peaks (log10(bp))") + scale_color_manual(values=group_colors) + facet_wrap(~State,scales="free") + geom_smooth(method="lm",se=FALSE,color="black") + geom_text(data=all_lengths_blocks_corr,aes(x = -Inf, y = Inf,label=round(Cor,2)),hjust=-0.1,vjust=1.5,size=3) + scale_y_continuous(breaks=pretty_breaks(n=3)) + scale_x_continuous(breaks=pretty_breaks(n=3))
```
