...
test = rmsk_TE_measure[,c("chromosome",measure_states)]
...
TE_pvalue_all_state = apply(rmsk_TE_measure[,cohorts],2,function(x) apply(rmsk_TE_measure[,c(measure_states,measure_states_extra)],2,function(y) unlist(wilcox.test(y~x))["p.value"]))
TE_pvalue_class_state = ddply(rmsk_TE_measure,~class_update,function(z) apply(z[,cohorts],2,function(x) apply(z[,c(measure_states,measure_states_extra)],2,function(y) unlist(wilcox.test(y~x))["p.value"])))
```
...
apply(rmsk_TE_measure[,measure_states],2,function(x) apply(rmsk_TE_measure[,cohorts],2,function(y) dim(rmsk_TE_measure[which(x > 0 & y > 0),])[1]/dim(rmsk_TE_measure[which(x > 0),])[1]))
```
