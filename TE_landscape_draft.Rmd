...
table(RNA_exon_unique$Expressed_samples)/NUM_EXON
...
chromosome_potential = aggregate(data=test,.~chromosome,function(x) mean(na.omit(x)))
...
apply(rmsk_TE_measure[,c(states,measure_states_extra)],2,function(x) unlist(kruskal.test(x~rmsk_TE_measure$class_update))["p.value"])
...
```{r analysis Fig S7}
merge(ddply(RNA_TE_agnostic,.(class_update),summarise,Proportion_expressed=sum(Expressed_samples > 0)/length(Expressed_samples),Max_median=median(Max_expression)),
      ddply(RNA_TE_agnostic[which(RNA_TE_agnostic$Expressed_samples > 0),],.(class_update),summarise,Max_median_expressed=median(Max_expression)),by="class_update")
```
...
aggregate(data=rmsk_TE_measure[,c("class_update",measure_metrics)],.~class_update,median)

colMeans(rmsk_TE_measure[,measure_metrics])
aggregate(data=rmsk_TE_measure[,c("class_update",measure_metrics)],.~class_update,mean)
sort(apply(aggregate(data=rmsk_TE_measure[,c("class_update",measure_metrics)],.~class_update,mean)[,2:6],2,function(x) sd(x)/mean(x)))

apply(rmsk_TE_measure[,measure_metrics],2,function(x) unlist(kruskal.test(x~rmsk_TE_measure$class_update))["p.value"])
...
aggregate(data=rmsk_TE_subfamily[,c("class_update",measure_metrics_subfam)],.~class_update,median)

colMeans(rmsk_TE_subfamily[,measure_metrics_subfam])
aggregate(data=rmsk_TE_subfamily[,c("class_update",measure_metrics_subfam)],.~class_update,mean)
sort(apply(aggregate(data=rmsk_TE_subfamily[,c("class_update",measure_metrics_subfam)],.~class_update,mean)[,2:10],2,function(x) sd(x)/mean(x)))

apply(rmsk_TE_subfamily[,measure_metrics_subfam],2,function(x) unlist(kruskal.test(x~rmsk_TE_subfamily$class_update))["p.value"])
...
aggregate(data=rmsk_TE_subfamily[,c("class_update",cohorts)],.~class_update,median)

colMeans(rmsk_TE_subfamily[,cohorts])
aggregate(data=rmsk_TE_subfamily_measure[,c("class_update",cohorts)],.~class_update,mean)
sort(apply(aggregate(data=rmsk_TE_subfamily_measure[,c("class_update",cohorts)],.~class_update,mean)[,2:22],2,function(x) sd(x)/mean(x)))

sort(apply(rmsk_TE_subfamily_measure[,cohorts],2,function(x) as.numeric(unlist(kruskal.test(x~rmsk_TE_subfamily_measure$class_update))["p.value"])))
...
sort(apply(rmsk_TE_subfamily_measure[,all_pc_states],2,function(x) as.numeric(unlist(kruskal.test(x~rmsk_TE_subfamily_measure$class_update))["p.value"])))
aggregate(data=rmsk_TE_subfamily_measure[,c("class_update",all_pc_states)],.~class_update,mean)
sort(apply(aggregate(data=rmsk_TE_subfamily_measure[,c("class_update",all_pc_states)],.~class_update,mean)[,2:43],2,function(x) sd(x)/mean(x)))
...
correlate_subfam_feature[which(correlate_subfam_feature$p.value < 0.001 & abs(correlate_subfam_feature$estimate.rho) > 0.3 & correlate_subfam_feature$State %in% measure_metrics_subfam & correlate_subfam_feature$Metric %in% measure_metrics_subfam & correlate_subfam_feature$class_update == "All"),]
...
correlate_subfamily[which(correlate_subfamily$Metric == "Vista_enhancers" & correlate_subfamily$State %in% states[1:21] & abs(correlate_subfamily$estimate.rho) > 0.3 & correlate_subfamily$p.value < 0.01),]
```
