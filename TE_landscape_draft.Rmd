...
# Correlations between number of samples in state and metric
...
# PCA on TEs in DNase, H3K27ac
 plot_pca(DNase_pca,c(1,2),metadata[which(!is.na(metadata$DNase)),],"Group","Roadmap Group",group_color,guide=T)
 plot_pca(DNase_pca,c(3,2),metadata[which(!is.na(metadata$DNase)),],"Group","Roadmap Group",group_color,guide=T)
 plot_pca(DNase_pca,c(3,4),metadata[which(!is.na(metadata$DNase)),],"Group","Roadmap Group",group_color,guide=T)
 plot_pca(H3K27ac_pca,c(1,2),metadata[which(!is.na(metadata$H3K27ac)),],"Group","Roadmap Group",group_color,guide=T)
 plot_pca(H3K27ac_pca,c(3,2),metadata[which(!is.na(metadata$H3K27ac)),],"Group","Roadmap Group",group_color,guide=T)
 plot_pca(H3K27ac_pca,c(3,4),metadata[which(!is.na(metadata$H3K27ac)),],"Group","Roadmap Group",group_color,guide=T)
```

# Test by class

```{r}
#9/19/2017 - For each state, I performed a Kruskal-Wallis test of the proportion in each state by class. Almost all are p < 2.2e-16, and 14_ReprPCWk (3.532e-05) and Hypermethylated (4.58e-15) are also significant. Most pairwise Wilcox tests are also significant.
 by(combine_class_sample,combine_class_sample$State,function(x) kruskal.test(x$Proportion~x$class))
 by(combine_class_sample,combine_class_sample$State,function(x) pairwise.wilcox.test(x$Proportion,x$class,method="bonf"))
 ggplot(combine_class_sample,aes(x=class,y=Proportion)) + geom_boxplot(position="dodge") + facet_wrap(~State,scales="free")
```
