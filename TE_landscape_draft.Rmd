...
combine_stats$Group = factor(c(rep("chromHMM",15),rep("WGBS",4),"DNase","H3K27ac","Expressed_samples"),levels=c("chromHMM","WGBS","DNase","H3K27ac","Expressed_samples"))
...
c = ggplot(chromHMM_TE_state_cum,aes(x=Sample_proportion,y=Proportion,color=State,group=State)) + geom_point(size=0.1) + geom_line(size=1.5) + labs(x=paste("Proportion of all samples (n=",sample_counts["All","chromHMM"],")",sep=""),y="Cumulative proportion of TEs in state") + scale_color_manual(values=chromHMM_colors,guide=FALSE) + geom_vline(xintercept = chromHMM_TE_state_dist_stats$Samples_avg_ever/100,color=chromHMM_colors)
...
a = xtabs(TE_sample ~ chromHMM+WGBS, data=compare_marks_all)
names(dimnames(test)) = c("chromHMM","WGBS")
a = t(apply(a,1,function(x) x*(WGBS_norm[match(WGBS_norm$WGBS,colnames(a)),]$TE_sample/colSums(a))))

b = xtabs(TE_sample ~ WGBS+chromHMM, data=compare_marks_all)
names(dimnames(test)) = c("WGBS","chromHMM")
b = t(apply(b,2,function(x) x*(WGBS_norm[match(WGBS_norm$WGBS,rownames(b)),]$TE_sample/rowSums(b))))

c = xtabs(TE_sample ~ DNase+chromHMM, data=compare_marks_all)
names(dimnames(test)) = c("DNase","chromHMM")
c = t(apply(c,2,function(x) x*(DNase_norm[match(DNase_norm$DNase,rownames(c)),]$TE_sample/rowSums(c))))

d = xtabs(TE_sample ~ H3K27ac+chromHMM, data=compare_marks_all)
names(dimnames(test)) = c("H3K27ac","chromHMM")
d = t(apply(d,2,function(x) x*(H3K27ac_norm[match(H3K27ac_norm$H3K27ac,rownames(d)),]$TE_sample/rowSums(d))))

e = xtabs(TE_sample ~ RNA+chromHMM, data=compare_marks_all)
names(dimnames(test)) = c("RNA","chromHMM")
e = t(apply(e,2,function(x) x*(RNA_norm[match(RNA_norm$RNA,rownames(e)),]$TE_sample/rowSums(e))))

f = xtabs(TE_sample ~ chromHMM+DNase, data=compare_marks_all)
names(dimnames(test)) = c("chromHMM","DNase")
f = t(apply(f,1,function(x) x*(DNase_norm[match(DNase_norm$DNase,colnames(f)),]$TE_sample/colSums(f))))
...
k = xtabs(TE_sample ~ chromHMM+H3K27ac, data=compare_marks_all)
names(dimnames(test)) = c("chromHMM","H3K27ac")
k = t(apply(k,1,function(x) x*(H3K27ac_norm[match(H3K27ac_norm$H3K27ac,colnames(k)),]$TE_sample/colSums(k))))
...
p = xtabs(TE_sample ~ chromHMM+RNA, data=compare_marks_all)
names(dimnames(test)) = c("chromHMM","RNA")
p = t(apply(p,1,function(x) x*(RNA_norm[match(RNA_norm$RNA,colnames(p)),]$TE_sample/colSums(p))))
...
u = xtabs(TE_sample ~ chromHMM+WGBS+DNase+H3K27ac+RNA, data=compare_marks_all) # No longer normalized
...
table_list = list(n,i,h,r,j,t,f,k,p,a)
...
test2 = merge(ddply(test1,.(chromHMM),summarise,TE_sample=sum(TE_sample)),ddply(test1[which(test1$DNase == "yes" & test1$H3K27ac == "yes"),],.(chromHMM),summarise,TE_sample=sum(TE_sample)),by="chromHMM")
...
test2 = merge(ddply(test1,.(WGBS),summarise,TE_sample=sum(TE_sample)),ddply(test1[which(test1$DNase == "yes" & test1$H3K27ac == "yes"),],.(WGBS),summarise,TE_sample=sum(TE_sample)),by="WGBS")
test2$TE_sample_proportion = test2$TE_sample.y/test2$TE_sample.x
test2

# QC
## Total TE x sample = 562520596 - matches total unique TEs in combine_marks_counts_RNA.txt and compare_marks_unique$TE_sample
...
## Total TE x sample x chromHMM states = 582516697 - matches total TEs in combine_marks_table_counts_RNA.txt and compare_marks_all$TE_sample
```

### Supplementary Figure 4

```{r Figure S4, echo=FALSE, cache=TRUE, cache.lazy=FALSE, dependson=c("Figure 2","setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
...
d = ggplot(combine_cum_prom,aes(x=Sample_proportion,y=Proportion,color=State,group=State)) + geom_point(size=0.1) + geom_line(size=1.5) + labs(x="Proportion of samples",y="Cumulative proportion of promoters in state") + scale_color_manual(values=all_state_colors[1:21],guide=FALSE) + geom_vline(xintercept = combine_stats_prom$Samples_avg_ever/100,color=all_state_colors[as.vector(combine_stats_prom$State)])
...
f = ggplot(RNA_potential_exon_cum,aes(x=Sample_proportion,y=Proportion,color=State,group=State)) + geom_point(size=0.1) + geom_line(size=1.5) + labs(x=paste("Proportion of samples (n=",sample_counts["All","RNA"],")",sep=""),y="Cumulative proportion of exons RPKM > 1") + scale_color_manual(values="black",guide=FALSE) + geom_vline(xintercept = RNA_potential_exon_stats$Samples_avg_ever/100,color="black")
...
### Shuffled number of states

```{r table shuffled number of states}
# Shuffled chromHMM
...
mean(apply(RNA_refseq_exon[,5:60],1,function(x) sum(x > 0)))

table(RNA_refseq_exon$Expressed_samples)
table(RNA_refseq_exon$Expressed_samples)/NUM_EXON

# Maximum expression
#hist(RNA_refseq_exon$Max_expression)
...
combine_stats_noCancer_IMR90$Group = factor(c(rep("chromHMM",15),rep("WGBS",4),"DNase","H3K27ac","Expressed_samples"),levels=c("chromHMM","WGBS","DNase","H3K27ac","Expressed_samples"))
...
e = ggplot(melt(as.matrix(state_switching_intra)),aes(x=Var2,y=Var1,fill=value)) + geom_tile() + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1),legend.title = element_blank()) + scale_x_discrete(labels=chromHMM_states) + scale_y_discrete(labels=rev(chromHMM_states),limits=rev(levels(melt(as.matrix(state_switching_intra))$Var1))) + scale_fill_gradient(low="white",high="darkred",limits=c(0,1))

f = ggplot(melt(as.matrix(state_switching_inter)),aes(x=Var2,y=Var1,fill=value)) + geom_tile() + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1),legend.title = element_blank()) + scale_x_discrete(labels=chromHMM_states) + scale_y_discrete(labels=rev(chromHMM_states),limits=rev(levels(melt(as.matrix(state_switching_inter))$Var1))) + scale_fill_gradient(low="white",high="darkred")

# State switching, chromHMM by class 
g = ggplot(melt(ss_inter_class,id.vars=c("Class","State")),aes(x=variable,y=State,fill=value)) + geom_tile() + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1),legend.title = element_blank()) + scale_x_discrete(labels=chromHMM_states) + scale_y_discrete(limits=rev(chromHMM_states)) + scale_fill_gradient(low="white",high="darkred") + facet_wrap(~Class)
...
# By chromosome
ddply(chromHMM_TE_state,.(chromosome),summarise,Max_states_intra=mean(Max_states_intra),States=mean(States))
```
...
## No IMR90
z = apply(TE_meth_average[,50:53],2,function(x) dim(TE_meth_average[which(x == sample_counts["Include","WGBS"]),])[1])
z
z/NUM_TE_WGBS
...
TE_DNase_peaks[which(TE_DNase_peaks$Samples == sample_counts["All","DNase"] | (TE_DNase_peaks$chromosome == "chrY" & TE_DNase_peaks$Samples == sample_counts["chrY","DNase"])),c(1:7,61:63)]
...
always_5TxWk[which(!is.na(always_5TxWk$intergenic)),]
...
combine_boxplot_class = rbind(class_state_sample,TE_meth_average_state_class,TE_DNase_peaks_class,TE_H3K27ac_peaks_class,RNA_RPKM_class)
...
# Methods 
Strand-agnostic RNA-seq files (n=56)

Summary of RNA-seq notes from Misha and the Roadmap data portal: 

* Unnormalized wiggle files with RNA-seq signal coverage (raw read coverage - 1 added for entire read length) (http://egg2.wustl.edu/roadmap/data/byDataType/rna/signal/unnormalized_wig/)
  + Excluded multimapped reads (BWA quality threshold 5), kept duplicate reads
* Coverages are NOT normalized, but normalization coefficients are in the file all.EGID.N.readlength. Misha says I should multiply wig coverage then integral over coding part of gene
  + However, this is a "modified RPKM measure, total number of reads aligned into coding exons, excluded those in mitochondrial geome, ribosomal protein genes, 0.5% expressed exons". Can this be used for something other than coding genes?
