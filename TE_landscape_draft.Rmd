...
# Shuffled WGBS
shuffled_WGBS_states = ldply(shuffled_WGBS_average,function(x) x$States)
...
# chromHMM states
z = apply(chromHMM_TE_state[,8:22],2,function(x) dim(chromHMM_TE_state[which(x == chromHMM_TE_state$Tissues),])[1])
z
sum(z/NUM_TE)
...
rmsk_TE_subfamily[which(rmsk_TE_subfamily$Count_noY <= THRESHOLD_IK_MEMBER),c("subfamily","Count","Count_noY","Total_length","Total_length_noY","CpGs")]
...
```{r analysis Fig S13c, cache=TRUE, cache.lazy=FALSE, dependson=c("setup","load libraries","set plot parameters","load functions","create essential matrices","load metadata","load essential matrices","define vectors","define facet labels","load feature sizes","calculate TE stats")}
#source("R_scripts/WGBS_subfamily_enrichment_TE.R")

# Average, max, and range of proportion of subfamily in methylation state (change to Percent)
...
ldply(TE_meth_subfamily,summarise,SD=sd(Mean),Mean=mean(Mean),Range=mean(Range),Max=mean(Max))
```
