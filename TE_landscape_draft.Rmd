...
human_mouse_chromHMM_states = as.data.frame(matrix(c(chromHMM_states[c(1:2,7,6,4,15,13)],"1","2","3","4","5","6","7"),nrow=7))
colnames(human_mouse_chromHMM_states) = c("Human_State","Mouse_State")

mouse_chromHMM_colors = setNames(chromHMM_colors[as.vector(human_mouse_chromHMM_states$Human_State)],as.vector(human_mouse_chromHMM_states$Mouse_State))
```
...
source("R_scripts/human_mouse_subfamily_WGBS.R")
...
write.table(unique(human_mouse_orthologs_WGBS_hypo[,c(hg19_coordinates,"Human_sample")]),file="Mouse/hg19_ortholog_hypo.test",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
write.table(unique(human_mouse_orthologs_WGBS_hypo[,mm10_coordinates]),file="Mouse/mm10_ortholog_hypo.test",sep="\t",col.names=FALSE,row.names=FALSE,quote=FALSE)
```

```{r overlap with GENCODE,eval=FALSE}
write.table(unique(human_mouse_orthologs_WGBS_hypo_chromHMM[which(human_mouse_orthologs_WGBS_hypo_chromHMM$Human_State == "1_TssA" & human_mouse_orthologs_WGBS_hypo_chromHMM$Mouse_State == "1"),hg19_coordinates]),row.names=FALSE,col.names=FALSE,quote=FALSE,sep='\t',file="Mouse/hg19_orthologs_hypo_1TssA.bed")
```
...
state_freq = ddply(human_mouse_orthologs_WGBS_paired,.(Human_state),summarise,Entries=100*length(Human_state)/dim(human_mouse_orthologs_WGBS_paired)[1])

b = ggplot(human_mouse_orthologs_WGBS_paired,aes(x=Human_state,fill=Mouse_state)) + geom_bar(position="fill") + scale_fill_manual(values=meth_colors,guide=FALSE) + xlab("Human") + ylab("Mouse") + theme(axis.text.x = element_text(angle=90,hjust=1)) + scale_x_discrete(labels=as.vector(apply(state_freq,1,function(x) paste(x[1],"\n(",round(as.numeric(x[2]),0),"%)",sep=""))))

c = ggplot(hg19_mm10_TE_WGBS_subfamily_hypo_paired,aes(x=Human_proportion,y=Mouse_proportion,color=Anatomy)) + geom_point(alpha=0.5) + scale_color_manual(values=anatomy_colors) + xlab("Human") + ylab("Mouse") + geom_smooth(method='lm',se=FALSE,fullrange=TRUE) + facet_wrap(~Human) + theme(legend.position = "bottom",legend.direction = "horizontal")

state_freq_chromHMM = ddply(human_mouse_orthologs_WGBS_hypo_chromHMM,.(Human_State),summarise,Entries=100*length(Human_State)/dim(human_mouse_orthologs_WGBS_hypo_chromHMM)[1])

d = ggplot(human_mouse_orthologs_WGBS_hypo_chromHMM,aes(x=Human_State,fill=Mouse_State)) + geom_bar(position="fill") + xlab("Human") + ylab("Mouse") + theme(axis.text.x = element_text(angle=90,hjust=1)) + scale_fill_manual(values=mouse_chromHMM_colors,name="Mouse state") + scale_x_discrete(labels=as.vector(apply(state_freq_chromHMM,1,function(x) paste(x[1],"\n(",round(as.numeric(x[2]),1),"%)",sep=""))))

grid.arrange(a,b,c,d,nrow=3,layout_matrix=rbind(c(1,2),c(3,3),c(4,4)),heights=c(0.25,0.5,0.25))
```
...
a = ggplot(human_mouse_orthologs_WGBS_paired,aes(x=Human_methylation,y=Mouse_methylation)) + stat_bin2d(bins=50) + scale_fill_gradient(low="white",high="darkblue",name="TEs") + xlab("Human") + ylab("Mouse") + facet_wrap(~Human_sample)

b = ggplot(human_mouse_spearman,aes(x=Test,y=Correlation,fill=Tissue)) + geom_boxplot() + ylab("Spearman correlation across TEs") + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_x_discrete(labels=c("Human-human","Mouse-mouse","Human-mouse, same tissue","Human-mouse, random tissue")) + scale_y_continuous(limits=c(0,1)) + scale_fill_discrete(guide=FALSE)

c = ggplot(human_mouse_spearman_subfamily,aes(x=Test,y=Correlation,fill=Tissue)) + geom_boxplot() + ylab("Spearman correlation across subfamilies") + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_x_discrete(labels=c("Human-human","Mouse-mouse","Human-mouse, same tissue","Human-mouse, random tissue")) + scale_y_continuous(limits=c(0,1)) 

grid.arrange(a,b,c,nrow=2,layout_matrix=rbind(c(1,1),c(2,3)),widths=c(0.45,0.55))
```
...
# Number of unique orthologous TEs with CpGs in mm10, hg19, and hg19->mm10 (both have CpGs)
...
ddply(human_mouse_orthologs_WGBS_paired,.(Human_sample,Mouse_sample),summarise,Both=sum(!is.na(Human_methylation) & !is.na(Mouse_methylation)))

# Chi-sq test for human-mouse pairs (WGBS)
# Table of human/mouse states, overall
table(human_mouse_orthologs_WGBS_paired[,c("Mouse_state","Human_state")])

# All methylation states, overall and by sample
ddply(human_mouse_orthologs_WGBS_paired,.(),summarise,Pvalue=unlist(chisq.test(x=Human_state,y=Mouse_state))["p.value"])
ddply(human_mouse_orthologs_WGBS_paired,.(Human_sample),summarise,Pvalue=unlist(chisq.test(x=Human_state,y=Mouse_state))["p.value"])

# Hypomethylated vs. all other states
test = human_mouse_orthologs_WGBS_paired[,c("Human_sample","Human_state","Mouse_state")]
test$Human_state = mapvalues(test$Human_state,meth_states[2:3],rep("Missing",2))
test$Mouse_state = mapvalues(test$Mouse_state,meth_states[2:3],rep("Missing",2))

ddply(test,.(),summarise,Pvalue=unlist(chisq.test(x=Human_state,y=Mouse_state))["p.value"])
ddply(test,.(Human_sample),summarise,Pvalue=unlist(chisq.test(x=Human_state,y=Mouse_state))["p.value"])
...
by(human_mouse_spearman[which(human_mouse_spearman$Test != "Paired"),],human_mouse_spearman[which(human_mouse_spearman$Test != "Paired"),]$Test,function(x) unlist(wilcox.test(x[which(x$Tissue == "Same"),]$Correlation,x[which(x$Tissue == "Different"),]$Correlation))["p.value"])
by(human_mouse_spearman,human_mouse_spearman$Tissue,function(x) pairwise.wilcox.test(x$Correlation,x$Test))
...
by(human_mouse_spearman_subfamily[which(human_mouse_spearman_subfamily$Test != "Paired"),],human_mouse_spearman_subfamily[which(human_mouse_spearman_subfamily$Test != "Paired"),]$Test,function(x) unlist(wilcox.test(x[which(x$Tissue == "Same"),]$Correlation,x[which(x$Tissue == "Different"),]$Correlation))["p.value"])
by(human_mouse_spearman_subfamily,human_mouse_spearman_subfamily$Tissue,function(x) pairwise.wilcox.test(x$Correlation,x$Test))
...
dim(unique(human_mouse_orthologs_WGBS_hypo[,c(mm10_coordinates,"Mouse_sample")]))
```

### Age analysis

```{r}
test = ddply(human_mouse_orthologs_WGBS_paired,.(human_chr_hg19,human_start_hg19,human_subfamily,human_class,mouse_chr_mm10,mouse_start_mm10,JC_distance),summarise,Concordant=sum(Human_state == Mouse_state),Con_hypo=sum(Human_state == Mouse_state & Human_state == "Hypomethylated"))
...
chromHMM_mouse_samples = unique(human_mouse_orthologs_WGBS_hypo_chromHMM[,c("Human_sample","Mouse_sample_chromHMM")])
table(human_mouse_orthologs_WGBS_hypo_chromHMM$Human_sample,human_mouse_orthologs_WGBS_hypo_chromHMM$Mouse_sample_chromHMM)

# Number of pairs and unique TEs in each state
dim(human_mouse_orthologs_WGBS_hypo_chromHMM)
dim(unique(human_mouse_orthologs_WGBS_hypo_chromHMM[,hg19_coordinates]))
dim(unique(human_mouse_orthologs_WGBS_hypo_chromHMM[,c(hg19_coordinates,"Human_sample")]))
dim(unique(human_mouse_orthologs_WGBS_hypo_chromHMM[,mm10_coordinates]))
dim(unique(human_mouse_orthologs_WGBS_hypo_chromHMM[,c(mm10_coordinates,"Mouse_sample_chromHMM")]))

# Corresponding states
human_mouse_orthologs_chromHMM_match = merge(human_mouse_orthologs_WGBS_hypo_chromHMM,human_mouse_chromHMM_states,by=c("Mouse_State","Human_State"))

# Number of pairs and human/mouse TEs per state
ddply(human_mouse_orthologs_chromHMM_match,.(Human_State),function(x) dim(x)[1])
ddply(human_mouse_orthologs_chromHMM_match,.(Human_State),function(x) dim(unique(x[,hg19_coordinates]))[1])
ddply(human_mouse_orthologs_chromHMM_match,.(Human_State),function(x) dim(unique(x[,mm10_coordinates]))[1])

# hg19 TE class per state
ddply(human_mouse_orthologs_chromHMM_match,.(Human_State),function(x) table(unique(x[,hg19_coordinates])$human_class))

# hg19 TE subfamily per state
human_mouse_orthologs_chromHMM_subfamily = melt(ddply(human_mouse_orthologs_chromHMM_match,.(Human_State),function(x) table(unique(x[,hg19_coordinates])$human_subfamily)),id.var="Human_State")
...
ddply(human_mouse_orthologs_chromHMM_subfamily,.(Human_State),function(x) x[order(x$value/x$Count),])
```
