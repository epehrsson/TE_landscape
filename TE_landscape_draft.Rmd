...
chromHMM_colors_X = setNames(chromHMM_colors,lapply(names(chromHMM_colors),function(x) paste("X",x,sep="")))
...
names(metric_labels) = levels(combine_shuffled_stats$Metric)
...
a = ggplot(combine_boxplot,aes(x=State,y=Proportion*100,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank()) + labs(y="% of TEs in state") + coord_flip() + scale_x_discrete(limits = rev(levels(combine_boxplot$State)),labels=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac","RPKM > 1"))) + geom_point(data=combine_stats,aes(x=State,y=Proportion_ever),color="red",size=3)
...
test = xtabs(TE_sample_norm ~ chromHMM+WGBS+DNase+H3K27ac, data=compare_marks_all)
mosaic(test,shade=TRUE)
```
...
a = ggplot(combine_shuffled_stats,aes(x=State,y=Value,fill=State)) + geom_boxplot() + scale_fill_manual(values=all_state_colors[1:21],guide=FALSE) + theme(axis.title.y = element_blank(),axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + scale_x_discrete(limits=c(chromHMM_states,meth_states,"DNase","H3K27ac")) + geom_point(data=combine_stats_long[which(combine_stats_long$State != "Expression"),],aes(x=State,y=Value),color="red",size=3) + facet_wrap(~Metric,nrow=3,labeller=labeller(Metric=metric_labels))
...
b = ggplot(combine_boxplot_prom,aes(x=State,y=Proportion*100,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors[1:21],guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank()) + labs(y="% of promoters in state") + coord_flip() + scale_x_discrete(limits = rev(levels(combine_boxplot_prom$State))) + geom_point(data=combine_stats_prom,aes(x=State,y=Proportion_ever),color="red",size=3)

#b = ggplot(combine_stats_prom,aes(x=1,y=Samples_avg_all,fill=State)) + geom_bar(stat="identity",show.legend=FALSE) + scale_fill_manual(values=all_state_colors[1:21]) + theme(axis.title.x=element_blank(),axis.ticks.x=element_blank(),axis.text.x=element_blank()) + ylab("Average % of samples in state") + facet_wrap(~Group,nrow=1,labeller=labeller(Group = group_labels))

c = ggplot(combine_cum_prom,aes(x=Sample_proportion,y=Proportion,color=State,group=State)) + geom_point(size=0.1) + geom_line(size=1.5) + labs(x="Proportion of all samples (n=127)",y="Cumulative proportion of promoters in state") + scale_color_manual(values=c(chromHMM_colors_X,all_state_colors[16:21]),labels=c(chromHMM_states,meth_states,"DNase","H3K27ac"),guide=FALSE) + geom_vline(xintercept = combine_stats_prom$Samples_avg_ever/100,color=c(chromHMM_colors_X,all_state_colors[16:21]))

# Exons
source("R_scripts/potential_exon.R")

d = ggplot(RNA_exon_sample,aes(x=State,y=Proportion*100,fill=State)) + geom_boxplot() +scale_fill_manual(values="black",guide=FALSE) + theme(axis.title.y=element_blank(),axis.ticks.y=element_blank()) + labs(y="% of exons in state") + coord_flip() + scale_x_discrete(labels="RPKM > 1") + geom_point(data=RNA_potential_exon_stats,aes(x=State,y=Proportion_ever),color="red",size=3) + scale_y_continuous(limits=c(0,100))

#e = ggplot(RNA_potential_exon_stats,aes(x=1,y=Samples_avg_all,fill=State)) + geom_bar(stat="identity",show.legend=FALSE) + scale_fill_manual(values="black") + theme(axis.title.x=element_blank(),axis.ticks.x=element_blank(),axis.text.x=element_blank()) + ylab("Average % of samples in state")

e = ggplot(RNA_potential_exon_cum,aes(x=Sample_proportion,y=Proportion,color=State,group=State)) + geom_point(size=0.1) + geom_line(size=1.5) + labs(x="Proportion of all samples (n=127)",y="Cumulative proportion of exons RPKM > 1") + scale_color_manual(values="black",guide=FALSE) + geom_vline(xintercept = RNA_potential_exon_stats$Samples_avg_ever/100,color="black")

# Plot all

legend = get_legend(ggplot(combine_boxplot,aes(x=State,y=Proportion*100,fill=State)) + geom_boxplot() +scale_fill_manual(values=all_state_colors) + theme(legend.direction = "horizontal",legend.title = element_blank(),legend.position="bottom"))

grid.arrange(a,b,c,d,e,legend, nrow = 4, layout_matrix=rbind(c(1,1),c(2,3),c(4,5),c(6,6)),heights=c(0.3,0.3,0.3,0.1))
...
combine_boxplot_noCancer_IMR90 = droplevels(combine_boxplot[which(!(combine_boxplot$Sample %in% c("E017","E114","E115","E117","E118","E123"))),])
...
grid.arrange(a,b,c,d,e,f,g,h,i, nrow= 5, layout_matrix=rbind(c(1,2),c(3,4),c(5,6),c(7,7),c(8,9)))
```
...
# (B) Proportion of TEs in each class hypomethylated per sample, sorted by decreasing mean.
...
a = ggplot(combine_class_proportion,aes(x=class,y=Proportion,fill=State)) + geom_bar(stat="identity") + ylab("Total proportion of feature in state") + scale_fill_manual(values=all_state_colors,guide=FALSE) + theme(axis.text.x = element_text(size=15,angle=90,hjust=1),axis.title.x = element_blank()) + facet_wrap(~Cohort,nrow=2,scales="free")

#combine_class_sample = rbind(class_chromHMM[,c(1:3,6)],class_CpG_meth,TE_DNase_class[,c(1:2,4:5)],TE_H3K27ac_class[,c(1:2,4:5)])
...
b = ggplot(combine_stats_class,aes(x=State,y=Samples_avg_all)) + geom_point(aes(color=class_update),size=4) + scale_color_manual(values=class_colors,name="Class") + scale_x_discrete(limits=state_order[order(state_order$Range),]$State) + scale_y_continuous(limits=c(0,100)) + theme(axis.title.y = element_blank()) + ylab("Average % samples in state") + geom_text(data=state_order,aes(x=State,y=100,label=paste("(",round(CV,2),")",sep=""))) + coord_flip() 

# Feature table
feature_state_mean_reduced = droplevels(feature_state_mean[which(feature_state_mean$Feature %in% levels(feature_state_mean$Feature)[c(1,4,7,9,12,15,18,19)]),])

c = ggplot(feature_state_mean_reduced,aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1)) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-40,40)) + scale_y_discrete(limits=rev(levels(feature_state_mean_reduced$Feature)))

# Most significant correlations between state, metrics
d = ggplot(rmsk_TE_subfamily_measure,aes(x=Age,y=CpGs_per_kbp,color=class_update)) + geom_point(size=2,alpha=0.9) + scale_color_manual(name="Class",values=class_colors,guide=FALSE) + theme(text=element_text(size=15)) + xlab("Mean subfamily age") + ylab("Average CpGs per kbp")

e = ggplot(rmsk_TE_subfamily_measure,aes(x=Age,y=Range_noIMR90,color=class_update)) + geom_point(size=2,alpha=0.9) + scale_color_manual(name="Class",values=class_colors,guide=FALSE) + theme(text=element_text(size=15)) + xlab("Mean subfamily age") + ylab("Range % hypomethylated")
...
```{r}
cor(rmsk_TE_measure[,c(8,10,12:14)],method="spearman")
...
# TE metrics vs. overlap with genic features, p-values
...
Supplementary Figure 7. TE metrics by class. (A) Proportion of all TEs (left), TE subfamilies (middle), and length (bp; right) in each TE class. Blue slice represents the length of the non-TE genome. (B) Density plot of mappability per TE, by class. (D) Density plot of TE age (Jukes-Cantor evolutionary distance from consensus) per TE, by class. (F) Proportion of TEs in each class overlapping RefSeq features. TEs may overlap multiple features. (G) CpGs per TE and proportion of TEs with CpGs by class. 

# Put table with CpGs per class in this figure. Standardize b axis labels
...
rmsk_TE_class_feature = melt(rmsk_TE_class[,c(1,12:31)],id.vars="class_update")
colnames(rmsk_TE_class_feature) = c("Class","Cohort","Proportion")
rmsk_TE_class_feature$Cohort = gsub("coding_exon","CDS",rmsk_TE_class_feature$Cohort)
rmsk_TE_class_feature$Cohort = factor(rmsk_TE_class_feature$Cohort,levels=c("cpgIsland","promoters","promoters_pc","promoters_nc","5UTR","5UTR_pc","5UTR_nc","CDS","CDS_pc","3UTR","3UTR_pc","3UTR_nc","exons","exons_pc","exons_nc","introns","introns_pc","introns_nc","intergenic"))
rmsk_TE_class_feature$Feature = factor(apply(rmsk_TE_class_feature,1,function(x) unlist(strsplit(as.character(x[2]),"_"))[1]),levels=c("cpgIsland","promoters","5UTR","CDS","3UTR","exons","introns","intergenic"))
rmsk_TE_class_feature$Coding = apply(rmsk_TE_class_feature,1,function(x) unlist(strsplit(as.character(x[2]),"_"))[2])
rmsk_TE_class_feature[which(is.na(rmsk_TE_class_feature$Coding)),]$Coding = "All"
rmsk_TE_class_feature$Coding = factor(rmsk_TE_class_feature$Coding,levels=c("All","pc","nc"))

a = ggplot(rmsk_TE_class_feature,aes(x=Class,y=Proportion,fill=Feature)) + geom_bar(stat="identity") + theme(axis.title.x = element_blank()) +  ylab("Proportion of TEs overlapping feature") + scale_fill_manual(values=c(brewer.pal(8,"Spectral"))) + facet_wrap(~Coding,nrow=1,labeller=labeller(Coding=coding_labels))
...
b = ggplot(rmsk_TE_measure_metric,aes(x=value,fill=class_update)) + geom_density() + scale_fill_manual(values=class_colors,guide=FALSE) + theme(panel.grid=element_blank(),axis.title.x = element_blank()) + ylab("Density") + facet_wrap(variable~class_update,scales="free",labeller=labeller(variable=variable_labels))
...
a = ggplot(combine_boxplot_class,aes(x=State,y=Proportion*100,fill=Class)) + geom_boxplot() + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank()) + coord_flip() + labs(y="% of TEs in state in any sample") + scale_x_discrete(labels=rev(c(chromHMM_states,meth_states,"DNase","H3K27ac","RPKM > 1")),limits=rev(levels(combine_boxplot_class$State)))

b = ggplot(combine_stats_class,aes(x=State,y=Proportion_ever,fill=class_update)) + geom_bar(stat="identity",position="dodge") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + labs(y="% of TEs in state in any sample") + scale_x_discrete(labels=c(chromHMM_states,meth_states,"DNase","H3K27ac","RPKM > 1"))
...
d = ggplot(combine_stats_class,aes(x=State,y=Samples_avg_ever,fill=class_update)) + geom_bar(stat="identity",position="dodge") + scale_fill_manual(values=class_colors,guide=FALSE) + theme(axis.title.x = element_blank(),axis.text.x = element_text(angle=45,hjust=1)) + labs(y="Average % of samples in state") + scale_y_continuous(limits=c(0,100)) + scale_x_discrete(labels=c(chromHMM_states,meth_states,"DNase","H3K27ac","RPKM > 1"))

legend1 = get_legend(ggplot(combine_stats_class,aes(x=State,y=Samples_avg_ever,fill=class_update)) + geom_bar(stat="identity",position="dodge") + scale_fill_manual(values=class_colors,name="Class"))
...
grid.arrange(a,b,c,d,legend1,legend2, nrow=4, layout_matrix=rbind(c(1,4),c(2,4),c(3,4),c(5,5)),widths=c(0.9,0.1),heights=c(0.3,0.3,0.3,0.1))
```
...
Supplementary Figure 9. Epigenetic state by feature overlap. (A) Difference in proportion of samples a TE is in a state by overlap with RefSeq genic features, normalized by overall average, including features split by protein-coding/non-coding. (B) By class. 

```{r Figure S9}
source("R_scripts/feature_overlap_state.R")

a = ggplot(feature_state_mean,aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1)) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-40,40)) + scale_y_discrete(limits=rev(levels(feature_state_mean$Feature)))

b = ggplot(feature_state_mean_class,aes(x=State,y=Feature,fill=Enrichment)) + geom_tile() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle=90,hjust=1)) + scale_fill_gradient2(low="darkblue",high="darkred",mid="white",midpoint=0,limits=c(-40,40)) + scale_y_discrete(limits=rev(levels(feature_state_mean_class$Feature))) + facet_wrap(~Class)
...
# Individual TE analysis

```{r }
# Plots
ggplot(correlate_TE_feature,aes(x=State,y=Metric,fill=estimate.rho)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))

ggplot(correlate_TE_state,aes(x=State,y=Metric,fill=estimate.rho)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(name="Rho",limits=c(-1,1),high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))

ggplot(compare_TE_feature,aes(x=Metric,y=Feature,fill=Difference)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))

ggplot(compare_TE_state,aes(x=Metric,y=Feature,fill=Difference)) + geom_tile() + facet_wrap(~class_update) + scale_fill_gradient2(high="darkred",low="darkblue") + theme(axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x = element_text(angle=90,hjust=1))

# Linear modelling
library(MASS)
fit_1TssA = lm(data=rmsk_TE_measure[,c(8:10,12:34,35)],X1_TssA ~ .)
summary(fit_1TssA)

test = stepAIC(fit_1TssA, direction="both")
test$anova
```

## Other individual TE analysis
...
Supplementary Figure 10. Methylation level of RefSeq features across samples. (B) Average methylation level of CpGs overlapping RefSeq features by sample. 
...
by_sample_class = rbind(class_chromHMM[,c(1:3,8)],class_CpG_meth[,c(1:2,4,8)],TE_DNase_class[,c(1:2,4:5)],TE_H3K27ac_class[,c(1:2,4,8)])
...
a = ggplot(by_sample_class,aes(x=State,y=Proportion,color=Group)) + geom_point(position = position_jitter(width = 0.3)) + theme(panel.grid=element_blank(),axis.title.x=element_blank(),axis.text.x=element_text(angle=90,hjust=1)) + ylim(0,1) + ylab("Proportion of state in TEs") + scale_color_manual(values=group_color,guide=FALSE) + scale_x_discrete(limits=levels(by_sample_class$State)) + stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median, geom= "crossbar",color="black") + facet_wrap(~class)
...
```{r}
legend_group = get_legend(ggplot(metadata,aes(x=Sample,y=Exclude,fill=Group)) + geom_tile() + scale_fill_manual(values=group_color))
legend_anatomy = get_legend(ggplot(metadata,aes(x=Sample,y=Exclude,fill=Anatomy)) + geom_tile() + scale_fill_manual(values=anatomy_colors))

legend_type = get_legend(ggplot(metadata,aes(x=Sample,y=Exclude,fill=Type)) + geom_tile() + scale_fill_manual(values=brewer.pal(5,"Greens")))
legend_age = get_legend(ggplot(metadata,aes(x=Sample,y=Exclude,fill=Age)) + geom_tile() + scale_fill_manual(values=brewer.pal(4,"YlOrRd")))
legend_cancer = get_legend(ggplot(metadata,aes(x=Sample,y=Exclude,fill=Cancer)) + geom_tile() + scale_fill_manual(values=c("white","red")))
legend_germline = get_legend(ggplot(metadata,aes(x=Sample,y=Exclude,fill=Germline)) + geom_tile() + scale_fill_manual(values=brewer.pal(6,"Dark2")))
...
b = ggplot(human_mouse_samples,aes(x=Test,y=Spearman)) + geom_boxplot() + ylab("Pearson correlation across TEs") + theme(axis.title.x = element_blank()) + scale_x_discrete(labels=c("Human-human","Mouse-mouse","Human-mouse, same tissue","Human-mouse, random tissue"))

c = ggplot(human_mouse_samples,aes(x=Test,y=Subfamily_Spearman)) + geom_boxplot() + ylab("Pearson correlation across subfamilies") + theme(axis.title.x = element_blank()) + scale_x_discrete(labels=c("Human-human","Mouse-mouse","Human-mouse, same tissue","Human-mouse, random tissue"))
