...
# TEs always overlapping a DHS peak summit that are >5kb from the nearest RefSeq gene
...
The number of TEs in an active regulatory state per sample, using the 15-state model. 
...
# 15-state
count_active_15 = cbind(metadata[,c("Sample","chrY")],read.table("/scratch/ecp/TE_landscape/combine_marks/true_summit/count_active.txt"))
...
# 50-state
count_active_50 = cbind(c("E003","E004","E005","E006","E007","E008","E017"),read.table("/scratch/ecp/TE_landscape/state50/count_active.txt"))
...
Potential analysis (Fig 2A/C) using 18-state and 50-state chromHMM models.  
...
c = ggplot(state_sample_50state,aes(x=State50,y=Proportion,fill=State18)) + geom_bar(stat="identity",color="black") + scale_fill_manual(values=chromHMM_states_18,guide=FALSE) + coord_flip() + facet_wrap(~Sample) + labs(x="State with 50-state model",y="Proportion of TEs in state") + scale_x_discrete(limits=rev(levels(state_sample_50state$State50)))

grid.arrange(a,b,c,nrow=2,layout_matrix=rbind(c(1,2),c(3)),widths=c(0.55,0.45))
```
...
# Fisher's exact test
fisher.test(vista_val_pooled_fraction[which(vista_val_pooled_fraction$Set %in% c("restricted","shuffle_constitutive")),c("Pos","Neg")])
fisher.test(vista_val_pooled_fraction[which(vista_val_pooled_fraction$Set %in% c("constitutive","shuffle_constitutive")),c("Pos","Neg")])
fisher.test(vista_val_pooled_fraction[which(vista_val_pooled_fraction$Set %in% c("constitutive","restricted")),c("Pos","Neg")])
fisher.test(vista_val_pooled_fraction[which(vista_val_pooled_fraction$Set %in% c("restricted","not_active")),c("Pos","Neg")])

# Plot
ggplot(vista_val_pooled_fraction,aes(x=1,y=Fraction,shape=Set)) + geom_point() + ylim(0,1) + ylab("Proportion of VISTA enhancers\npositively validated") + theme(axis.text.x = element_blank(),axis.ticks.x = element_blank(),axis.title.x = element_blank()) + scale_shape_discrete(labels=orthogonal_labels)
```
...
# Number of TEs overlapping bin centers by class
...
grid.arrange(a,b,c,d,e,nrow=4,layout_matrix=rbind(c(1,2),c(3),c(4),c(5)),widths=c(0.45,0.55))
```
...
# Number of TEs annotated with each state in each sample
...
Uses the same filters as for the 15-state model.
